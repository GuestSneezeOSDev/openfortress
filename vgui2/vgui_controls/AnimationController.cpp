...
static AnimationController *__g_TestVar = NULL;
		...
		while (token[0])
		{
			...
			if (!stricmp(token, "animate"))
			{
				...
				if (cmdAnimate.variable == m_sPosition)
				{
					// Get first token
					if( token[0] == 'b' )
						cmdAnimate.target.bUseDefault[0] = true;
					...
					if( psz[0] == 'b' )
						cmdAnimate.target.bUseDefault[1] = true;

					SetupPosition( cmdAnimate, &cmdAnimate.target.b, psz, screenTall );
				}
				// Pos and ang use the same formatting
				else if( cmdAnimate.variable == m_sModelPos || cmdAnimate.variable == m_sModelAng )
				{
					float *pFl[] =
					{
						&cmdAnimate.target.a,
					...
					for( int i = 0; i < 3; i++ )
					{
						const char *psz = args[i];
						if( psz[0] == 'b' )
						{
							cmdAnimate.target.bUseDefault[i] = true;
							psz++;
						}
				...
				// fix up scale
				else if (cmdAnimate.variable == m_sSize)
				{
					if( IsProportional() )
					{
						char *psz = &token[0];
...
// 							}
				...
				if (cmdAnimate.variable == m_sWide ||
...
void AnimationController::UpdateActiveAnimations(bool bRunToCompletion)
{
	...
	for (int i = 0; i < m_ActiveAnimations.Count(); i++)
	{
		...
		{		
			...
			if( anim.panel.Get() == nullptr )
			{
				// panel is gone, remove the animation
				continue;
			}

			UtlSymId_t seqName = anim.seqName;
			Panel *pParent = anim.panel->GetParent();
			bool bAnimationEnded = true;

			// Check if there's still parts of this animation sequence left
			for( int i = 0; i < m_ActiveAnimations.Count() && pParent; i++ )
			{
				...
				Panel *foundPanel = pParent->FindChildByName(animPanel->GetName(), true);

				// The animation panel *has* to be the same pannel as the one from the parent
				if ( foundPanel != animPanel )
					continue;

				// If it is, that means the animation still has to end, don't sent an end signal
				bAnimationEnded = false;
				break;
			}

			// Since we can't add new functions to the base panel
			// we can just use the message function system
			if( bAnimationEnded )
			{
				KeyValues *pKV = new KeyValues( "AnimEnd", "name", g_ScriptSymbols.String(anim.seqName) );
				ivgui()->PostMessage( anim.panel->GetVParent(), pKV->MakeCopy(), NULL );
				pKV->deleteThis();
			}
		}
	}
...
void AnimationController::StartCmd_Animate(Panel *panel, UtlSymId_t seqName, AnimCmdAnimate_t &cmd)
{
	...
	AdjustDefaultValues( anim.panel, anim.variable, anim.endValue );
...
AnimationController::Value_t AnimationController::GetValue(ActiveAnimation_t& anim, Panel *panel, UtlSymId_t var)
{
	...
	{
		int x, y;
		AnimHandleHack(panel).GetAnimPos(x, y);
	...
	else if ( var == m_sModelPos )
	{
		...
		if( pPanel )
		{
			pPanel->GetHUDModelAnimPos( x, y, z );
			val.a = x;
			val.b = y;
			val.c = z;
		}
	...
	else if ( var == m_sModelAng )
	{
		...
		if( pPanel )
		{
			pPanel->GetHUDModelAnimAng( x, y, z );
			val.a = x;
			val.b = y;
			val.c = z;
		}
	...
	{
		int w, t;
		AnimHandleHack(panel).GetAnimSize(w, t);
		val.a = (float)w;
		val.b = (float)t;
	}
...
void AnimationController::SetValue(ActiveAnimation_t& anim, Panel *panel, UtlSymId_t var, Value_t &value)
{
	if( var == m_sPosition )
	{
		...
		AnimHandleHack(panel).SetAnimPos(x, y);
	}
	else if ( var == m_sModelPos )
	{
		AnimContModelPanel *pPanel = dynamic_cast<AnimContModelPanel*>(panel);
		if( pPanel )
		{
			pPanel->SetHUDModelAnimPos( value.a, value.b, value.c );
		}
	}
	else if ( var == m_sModelAng )
	{
		...
		if( pPanel )
		{
			pPanel->SetHUDModelAnimAng( value.a, value.b, value.c );
		}
	}	
	else if( var == m_sSize )
	{
		AnimHandleHack(panel).SetAnimSize((int)value.a, (int)value.b);
	}
	else if (var == m_sFgColor)
	{
		Color col = panel->GetFgColor();
...
void AnimationController::AdjustDefaultValues( Panel *panel, UtlSymId_t var, Value_t &value )
{
	if( var == m_sPosition )
	{
		int x, y;
		panel->GetPos( x, y );
		if( !value.bUseDefault[0] )
			value.a -= x;

		if( !value.bUseDefault[1] )
			value.b -= y;
...
// Purpose: removes an existing set of commands from the queue
//-----------------------------------------------------------------------------
bool IsHUDAnimationPlaying( AnimationController *pAnimationControler, const char *seqName, Panel *pWithinParent )
{
	UtlSymId_t seqToken = g_ScriptSymbols.Find(seqName);
	// if pWithinParent is specified, find only messages under that parent
	{
		for (int i = 0; i < pAnimationControler->m_PostedMessages.Count(); i++)
		{
			if ( ( pAnimationControler->m_PostedMessages[i].seqName == seqToken ) &&
				 ( !pWithinParent || ( pAnimationControler->m_PostedMessages[i].parent == pWithinParent ) ) )
			{
				return true;
			}
		}
	...
	// if pWithinParent is specified, find only animations under that parent
	for( int i = 0; i < pAnimationControler->m_ActiveAnimations.Count(); i++ )
	{
		if( pAnimationControler->m_ActiveAnimations[i].seqName != seqToken )
			continue;

		// panel this anim is on, m_ActiveAnimations[i].panel
		if( pWithinParent )
		{
			Panel *animPanel = pAnimationControler->m_ActiveAnimations[i].panel;
