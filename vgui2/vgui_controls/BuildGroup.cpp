...
void BuildGroup::LoadControlSettings(const char *controlResourceName, const char *pathID, KeyValues *pPreloadedKeyValues, KeyValues *pConditions)
{
	...
	// dont delete if we're recursive
	if( !pConditions || !pConditions->GetBool("recursive_include", false) )
		DeleteAllControlsCreatedByControlSettingsFile();

	// Allow us to include other res files in our files!
	// The main file still overrides these ones since it gets ApplySettings-t last
	KeyValues *pIncludeRes = rDat->FindKey( "include_res" );
	if( pIncludeRes )
	{
		FOR_EACH_VALUE( pIncludeRes, pInclude )
		{
			pConditions = pConditions ? pConditions : new KeyValues("temp");
			pConditions->SetBool("recursive_include", 1);
			LoadControlSettings(pInclude->GetString(), pathID, pPreloadedKeyValues, pConditions);
		}
	}
