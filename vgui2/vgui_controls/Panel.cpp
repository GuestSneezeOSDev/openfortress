...
// This is absolutely Fucking Stupid - Kay
CUtlMap<vgui::HPanel, vgui::IAnimHandlingHack> g_AnimHandles;
...
void Panel::Init( int x, int y, int wide, int tall )
{
	...
	if( !g_AnimHandles.Count() )
		g_AnimHandles.SetLessFunc( DefLessFunc(vgui::HPanel) );

	IAnimHandlingHack pNewHandle;
	int iIndex = g_AnimHandles.Insert(ToHandle(), pNewHandle);
	g_AnimHandles[iIndex].pOwner = this;

	SetPos(x, y);
	g_AnimHandles[iIndex].SetAnimPos(0, 0);
	SetSize(wide, tall);
	g_AnimHandles[iIndex].SetAnimSize(0, 0);
...
void Panel::SetPos(int x, int y)
{
	...
	IAnimHandlingHack *pAnimHandler = &AnimHandleHack(this);
	pAnimHandler->m_iDefaultXPos = x;
	pAnimHandler->m_iDefaultYPos = y;
	pAnimHandler->RefreshPos();
}
...
{
	IAnimHandlingHack *pAnimHandler = &AnimHandleHack(this);
	x = pAnimHandler->m_iDefaultXPos;
	y = pAnimHandler->m_iDefaultYPos;
}
...
void Panel::SetSize(int wide, int tall)
{
	...
	IAnimHandlingHack *pAnimHandler = &AnimHandleHack(this);
	pAnimHandler->m_iDefaultWide = wide;
	pAnimHandler->m_iDefaultTall = tall;
	pAnimHandler->RefreshSize();
}
...
{
	IAnimHandlingHack *pAnimHandler = &AnimHandleHack(this);
	wide = pAnimHandler->m_iDefaultWide;
	tall = pAnimHandler->m_iDefaultTall;
}
...
void Panel::OnCommand(const char *command)
{
	...
	else if( !Q_strncasecmp( "runanimation", command, sizeof("runanimation")-1 ) )
	{
		CCommand args;
		...
		int iLevel = args.ArgC() > 2 ? atoi(args[2]) : 1;
		...
		GetAnimationController()->StartAnimationSequence( pTarget, args[1] );
	}
	else
	{
...
IAnimHandlingHack::IAnimHandlingHack()
{
	m_iDefaultXPos = 0;
	m_iDefaultYPos = 0;
	m_iAnimXPos = 0;
...
// Purpose: 
//-----------------------------------------------------------------------------
void IAnimHandlingHack::SetAnimPos(int x, int y)
{
	Assert( abs(x) < 32768 && abs(y) < 32768 );
	m_iAnimXPos = x;
	m_iAnimYPos = y;
...
// Purpose: 
//-----------------------------------------------------------------------------
void IAnimHandlingHack::GetAnimPos(int &x, int &y)
{
	x = m_iAnimXPos;
	y = m_iAnimYPos;
}

void IAnimHandlingHack::RefreshPos()
{
	int x, y, xanim, yanim;
	pOwner->GetPos(x, y);
	GetAnimPos(xanim, yanim);
	ipanel()->SetPos(pOwner->GetVPanel(), x + xanim, y + yanim);
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void IAnimHandlingHack::SetAnimSize(int wide, int tall)
{
	// Assert(abs(x) < 32768 && abs(y) < 32768);
	m_iAnimWide = wide;
	m_iAnimTall = tall;
...
// Purpose: 
//-----------------------------------------------------------------------------
void IAnimHandlingHack::GetAnimSize(int &wide, int &tall)
{
	wide = m_iAnimWide;
	tall = m_iAnimTall;
}

void IAnimHandlingHack::RefreshSize()
{
	int wide, tall, wideanim, tallanim;
	pOwner->GetSize(wide, tall);
	GetAnimSize(wideanim, tallanim);
	ipanel()->SetSize(pOwner->GetVPanel(), wide + wideanim, tall + tallanim);
}

void vgui::SetupAnimHandlingHackForPanel( vgui::Panel *pPanel )
{
	if( !g_AnimHandles.Count() )
		g_AnimHandles.SetLessFunc( DefLessFunc(vgui::HPanel) );

	IAnimHandlingHack pNewHandle;
	int iIndex = g_AnimHandles.Insert(pPanel->ToHandle(), pNewHandle);
	g_AnimHandles[iIndex].pOwner = pPanel;
