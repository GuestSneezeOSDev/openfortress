...
// #endif
...
#endif
	{
		...
		vec_t flSize = pOther->GetAbsVelocity().Length();

		if ( std::isnan(flSize) )
		{
			Warning("flSize in %s isNan!\n", __FUNCTION__);
			flSize = 1.0f;
		}
		...
		VectorMultiply(vForward, flSize, vNewVel);
		...
		{
			Warning("vNewVel BUNK in %s!\n", __FUNCTION__);
			Vector setZeroVec(0, 0, 0);
			pVelocity = &setZeroVec;
		}
		else
		{
			pVelocity = &vNewVel;
	...
	if ( pOther->IsPlayer() && pOther->IsAlive())
	{
		...
		for ( int i = 0; i < targets; i++ )
		{
			...
			if ( ent->IsPlayer() && ent->IsAlive() && ent != pOther && ( ent->GetTeamNumber() != GetTeamNumber() || ent->GetTeamNumber() == TF_TEAM_MERCENARY ) )
			{
