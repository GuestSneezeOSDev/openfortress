...
//-----------------------------------------------------------------------------
// Purpose: Shake the screen for a single client
// Input  : pPlayer - Player to apply the shake to
//			amplitude - Amplitude of shake
//			frequency - 
//			duration - duration of shake in seconds.
//			command - One of the following values:
...
//			bAirShake - if this is false, then it will only shake players standing on the ground.
//-----------------------------------------------------------------------------
void UTIL_ScreenShakePlayer(CBasePlayer* pPlayer, float amplitude, float frequency, float duration, ShakeCommand_t eCommand, bool bAirShake)
{
	if (amplitude > MAX_SHAKE_AMPLITUDE)
	{
		amplitude = MAX_SHAKE_AMPLITUDE;
	...
	// Only start shakes for a player that is on the ground unless doing an air shake.
	if (!pPlayer || (!bAirShake && (eCommand == SHAKE_START) && !(pPlayer->GetFlags() & FL_ONGROUND)))
	{
		return;
	}

	TransmitShakeEvent(pPlayer, amplitude, frequency, duration, eCommand);
}
...
void UTIL_SetModel( CBaseEntity *pEntity, const char *pModelName )
{
	...
	if ( i == -1 )	
	{
		...
		if ( int newi = CBaseEntity::PrecacheModel( pModelName ) )
		{
			...
			DevWarning(1, "%s was not precached\n", pModelName );
		}
#else
