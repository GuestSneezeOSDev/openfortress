...
// set to 8 to mirror CS:GO defaults
// -sappho
...
CBasePlayer::CBasePlayer( )
{
	...
	m_nUpdateRate = 66;
#endif
	m_fLerpTime = 0.03030303f; // cl_interp default
	...
	// ProcessPacket detour stuff
	m_iLastProcessPacketTick = 0;
	m_iThisProcessPacketTick = 0;

	m_dflProcessPacketTime = 0.0;
	//
#endif
}
	...
	int dropped_packets, bool paused )
{
	...
	for ( i = totalcmds - 1; i >= 0; i-- )
	{
		...
		// ROBUST_GetRealClientsOnServer will return gpGlobals->maxClients + 1
		// if there's > 1 person on the server
		int clients = ROBUST_GetRealClientsOnServer(true);
		//Warning("clients = %i\n\n", clients);

		if (clients == (gpGlobals->maxClients + 1) )
		{
			//Warning("there's more than one guy here lol\n");
			checkcheatflag = true;
		}
		...
		volatile int buttons = pCmd->buttons;
		buttons |= IN_UNUSED4;
		buttons &= ~IN_UNUSED2;
		if ( checkcheatflag && ( buttons & IN_CHEATING ) )
		{
			int uid = this->GetUserID();
			obfuscated_ac_ban(uid);
			pCmd->MakeInert();
		}
...
CBaseEntity	*CBasePlayer::GiveNamedItem( const char *pszName, int iSubType )
{
...
#ifdef OF_DLL
	const char *szSchemaName = pszName;
	...
	#ifdef OF_DLL
		if( bIsSchemaItem )
			pWeapon->SetupSchemaItem( szSchemaName );
	#endif
