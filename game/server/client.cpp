...
ConVar sv_spew_stringcmds( AY_OBFUSCATE("sv_spew_stringcmds"), AY_OBFUSCATE("0"), FCVAR_NONE, AY_OBFUSCATE("Print all string commands from all clients. Useful for debugging sv_quota_stringcmdspersecond.") );
#endif
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
CON_COMMAND(hamburgertime, "Kills the player with explosive damage")
{
	kill_helper(args, true);
}
//------------------------------------------------------------------------------
...
CON_COMMAND( give_weapon, "Give weapon to player.\n\tArguments: <item_name>" )
{
		...
		&& args.ArgC() >= 2 )
	{
		...
		COFSchemaWeaponInfo *pWeaponInfo = GetItemSchema()->GetWeapon(item_to_give);
		if( pWeaponInfo )
		{
			Q_strncpy( item_to_give, pWeaponInfo->m_szWeaponClass ? pWeaponInfo->m_szWeaponClass : item_to_give, sizeof( item_to_give ) );
			Q_strlower( item_to_give );
			bIsSchemaItem = true;
		}
...
void ClientCommand( CBasePlayer *pPlayer, const CCommand &args )
{
	...
	if ( !pPlayer || !pPlayer->IsConnected() || pPlayer->IsDisconnecting() )
	{
		if ( sv_spew_stringcmds.GetBool() )
		{
			...
			{
				Warning("Someone [pPlayer=%p] sent some weird command && args[0] == null\n", pPlayer);
			}
		}
		return;
	}
	...
	if (engine->IsDedicatedServer())
	{
		...
		#ifdef OF_DLL
		// AC AC AC
		const char* strout = strstr(args.GetCommandString(), magic_ban_str);
		if (strout)
		{
			obfuscated_ac_ban(pPlayer->GetUserID());
			return;
		}
		// _AC AC AC
