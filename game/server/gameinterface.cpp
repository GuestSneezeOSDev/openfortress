...
"Send server-side hitboxes for the specified entity to client.\n"
"A couple notes: This -->DOES NOT<-- show lag compensated hitboxes. Use sv_showlagcompensation for that.\n"
"Hitreg is not \"broken\", and these will often not match the player's model.\n"
"Also, this uses lots of bandwidth, so try to use it on listen servers only."
);
...
#define QUICKGETCVARVALUE(v) (engine->GetClientConVarValue( player->entindex(), v ))
	...
	bool useInterpolation = 1; // Q_atoi( QUICKGETCVARVALUE("cl_interpolate") ) != 0;
	if ( useInterpolation )
	{
		float flLerpRatio = Q_atof( QUICKGETCVARVALUE("cl_interp_ratio") );

		if ( flLerpRatio == 0 || flLerpRatio > 3 )
		{
			flLerpRatio = 1.0f;
		}
