	...
	for ( int issueIndex = 0; issueIndex < m_potentialIssues.Count(); ++issueIndex )
	{
		...
		if ( FStrEq( pszTypeString, pCurrentIssue->GetTypeString() ) )
		{
			...
			if ( pCurrentIssue->CanCallVote( iEntIndex, pszDetailString, nErrorCode, nTime ) )
			{
				...
				CBasePlayer *pTargetPlayer = UTIL_PlayerByUserId(atoi(pszDetailString));
				pCurrentIssue->iIDTargetPlayer = NULL;
				if (pTargetPlayer && !pTargetPlayer->IsBot())
				{
					pCurrentIssue->iIDTargetPlayer = pTargetPlayer->GetSteamIDAsUInt64();
					DevMsg("%llu", pCurrentIssue->iIDTargetPlayer);
				}
				...
				// Force (if kick & against player) vote no - Bryson
				if ( pCurrentIssue->IsYesNoVote() && !bDedicatedServer && pTargetPlayer)
				{
					int iTargetPlayer = pTargetPlayer->entindex();
					TryCastVote( iTargetPlayer, "Option2" );
				}
...
CVoteController::TryCastVoteResult CVoteController::TryCastVote( int iEntIndex, const char *pszVoteString )
{
	...
	CBasePlayer *pPlayerVoter = m_potentialIssues[m_iActiveIssueIndex]->m_hPlayerTarget;

	if (pVoter->IsPlayer())
	{
		if (( pVoteHolder != NULL ) && ( pVoter != NULL ) && ( pPlayerVoter != NULL ) && ToBasePlayer(pVoter) == pPlayerVoter)
		return CAST_FAIL_NO_ACTIVE_ISSUE;	// Maybe make a custom CAST for this? -Bryson
	}
...
bool CVoteController::IsValidVoter( CBasePlayer *pWhom )
{
	...
	// If they are being voted against, make them auto-vote no instead -Bryson
	// if (pWhom->GetSteamIDAsUInt64() ==  m_potentialIssues[m_iActiveIssueIndex]->iIDTargetPlayer)
	//	return false;
