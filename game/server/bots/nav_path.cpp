...
void CStuckMonitor::Update( CImprov *improv )
{
	if( m_isStuck )
	{
		...
		const float unstuckRange = GetStuckVel(improv) / 4.0f;
	...
	else
	{
		...
		else
		{
			...
			float stuckVel = GetStuckVel( improv );
...
float CStuckMonitor::GetStuckVel( CImprov* improv )
{
	return (improv->IsUsingLadder()) ? 10.0f : m_stuckVel; 
}