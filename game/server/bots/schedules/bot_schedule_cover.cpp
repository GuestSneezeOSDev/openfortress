    ...
    if ( GetBot()->IsCombating() || GetBot()->IsAlerted() ) 
	{
        if ( GetProfile()->GetSkill() >= SKILL_HARD && GetDecision()->IsDangerousEnemy() ) 
		{
            if ( HasCondition( BCOND_LIGHT_DAMAGE ) )
				return BOT_DESIRE_COVER_DANGEROUS_ENEMY;
        }

        if ( HasCondition( BCOND_REPEATED_DAMAGE ) )
			return BOT_DESIRE_COVER_REPEATED_DAMAGE;

        if ( HasCondition( BCOND_HEAVY_DAMAGE ) )
			return BOT_DESIRE_COVER_HEAVY_DAMAGE;

		// still have a little desire so long as we're in combat
		// but not while we're alerted
		// Since once we're behind cover, we lose the IsCombating flag
		// This essentially creates cover peaking
		if( GetBot()->IsCombating() )
			return BOT_DESIRE_COVER_BASE_COMBAT;
