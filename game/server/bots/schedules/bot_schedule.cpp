...
void IBotSchedule::TaskStart()
{
    ...
    switch ( pTask->task ) {
        ...
        case BTASK_DECIDE_NEW_ROAM_LOCATION:
        {
            if ( !GetMemory() )
			{
                Fail( "Without memory" );
			...
			Vector vecFrom( GetHost()->GetAbsOrigin() );
			CNavArea *pArea = NULL;
			...
			while ( true ) 
			{
				...
				if ( !GetLocomotion()->IsTraversable( vecFrom, vecGoal ) )
					continue;

				GetMemory()->UpdateDataMemory( "FreeRoamLocation", pArea->GetRandomPoint() );
				break;
			}
        ...
        case BTASK_HUNT_ENEMY:
        {
            ...
            if ( HasCondition( BCOND_TOO_FAR_TO_ATTACK ) ) 
			{
                ...
                if ( pWeapon ) 
				{
					...
					{
						// Should prolly be 0, so they get as close as they can
                        tolerance = range;
