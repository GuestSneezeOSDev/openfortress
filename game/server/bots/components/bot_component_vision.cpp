...
#define LOOK_RANDOM_INTERVAL ( IsAlerted() ) ? RandomInt( 1, 3 ) : RandomInt( 3, 8 )
...
void CBotVision::Update()
{
    ...
    LookAtThreat(); // PRIORITY_CRITICAL
...
//================================================================================
bool CBotVision::GetEntityBestAimPosition( CBaseEntity *pEntity, Vector &vecLookAt )
{
    if ( pEntity == NULL )
        return false;
...
bool CBotVision::LookAt( const char *pDesc, CBaseEntity *pTarget, int priority, float duration, float cosTolerance )
{
    ...
    Vector vecLookAt;
    if( !GetEntityBestAimPosition( pTarget, vecLookAt ) )
		return false;
...
{
	// Not a really good check
    if ( !vecGoal.IsValid() )
	{
		DevMsg( "Vector invalid!! %f %f %f\n", vecGoal.x, vecGoal.y, vecGoal.z );
        return false;
	}
    //DevMsg( "%f %f %f | %f %f %f\n", m_vecLookGoal.x, m_vecLookGoal.y, m_vecLookGoal.z, vecGoal.x, vecGoal.y, vecGoal.z );
    if ( HasAimGoal() && m_vecLookGoal == vecGoal && !m_bAimReady )
        return false;
...
void CBotVision::LookAtThreat()
{
	...
	LookAt("Threat", memory->GetEntity(), PRIORITY_CRITICAL, 0.2f);
}
...
void CBotVision::LookRandomSpot()
{
    ...
    viewAngles.y += RandomInt( -120, 120 );

	viewAngles.x = min( max( viewAngles.x, -89 ), 89 );

    Vector vecForward;
    AngleVectors( viewAngles, &vecForward );

    Vector vecPosition = GetHost()->EyePosition();
    LookAt( "Random Spot", vecPosition + 30 * vecForward, PRIORITY_VERY_LOW, 2.0f );
}
