...
DECLARE_REPLICATED_COMMAND( tf_bot_locomotion_allow_teleport, "1", "Indica si los Bots pueden teletransportarse al quedar atascados." )
DECLARE_REPLICATED_COMMAND( tf_bot_locomotion_hiddden_teleport, "1", "Indica si los Bots pueden teletransportarse solo si ningun Jugador los esta mirando" )
DECLARE_REPLICATED_COMMAND( tf_bot_locomotion_tolerance, "60", "" )
DECLARE_REPLICATED_COMMAND( tf_bot_locomotion_allow_wiggle, "1", "" )
...
void CBotLocomotion::Reset()
{
    ...
    CTFPlayer *pPlayer = dynamic_cast<CTFPlayer*>(m_pBot);

    if( pPlayer )
    {
        m_NavPathFollower->m_stuckMonitor.SetStuckVel( pPlayer->GetPlayerMaxSpeed() * 0.1f );
    }
}
...
void CBotLocomotion::Update()
{
	...
	{
        GetPathFollower()->Debug( tf_bot_debug_locomotion_extra.GetBool() );
    }
    else 
	{
    ...
    if ( !IsUnreachable() ) 
	{
		...
		{
			GetHost()->SpeakConceptIfAllowed(MP_CONCEPT_PLAYER_HELP);
            Wiggle();
        }
    }
}
...
void CBotLocomotion::Wiggle()
{
	...
	{
        m_WiggleDirection = RandomInt( 0, 1 ) ? LEFT : RIGHT;
...
void CBotLocomotion::Strafe()
{
    if ( !m_StrafeTimer.HasStarted() || m_StrafeTimer.IsElapsed() ) 
	{
        m_StrafeDirection = RandomInt( 0, 1 ) ? LEFT : RIGHT;
        m_StrafeTimer.Start( RandomFloat( 0.2f, 0.5f ) );
    }
    ...
    if( GetSimpleGroundHeightWithFloor( vecPos, &flGround ) ) 
	{
        GetBot()->InjectMovement( m_StrafeDirection );
    }

    //// Sometimes we duck
    //if( RandomInt( 0, 100 ) > 90 ) 
	//{
	//	if ( !m_StrafeDuckTimer.HasStarted() || m_StrafeDuckTimer.IsElapsed() ) 
	//	{
	//		m_StrafeDuckTimer.Start( RandomFloat( 0.3f, 0.8f ) );
	//	}
    //}
	//
	//if( m_StrafeDuckTimer.HasStarted() && !m_StrafeDuckTimer.IsElapsed() )
...
float CBotLocomotion::GetTolerance() const
{
    ...
    float flTolerance = tf_bot_locomotion_tolerance.GetFloat();
    return flTolerance;
}
