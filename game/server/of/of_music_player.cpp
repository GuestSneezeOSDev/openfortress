...
void CTFMusicPlayer::ChangeMusic( const char *szName )
{
	szLoopingSong = MAKE_STRING(szName);
	NetworkStateChanged();
}
...
void CTFDMMusicManager::Spawn( void )
{
	...
	COFSoundParameters *pRoundMusic = OFSoundManifest()->GetSoundscript( STRING( szRoundMusic.GetForModify() ) );
	if( pRoundMusic )
	{
		PrecacheScriptSound( pRoundMusic->GetWinMusic() );
		//PrecacheScriptSound( pRoundMusic->GetString("lose") );
	}
}
	...
	if( !m_bDisableThink )
	{
		if ( TeamplayRoundBasedRules() && 
			(	
				...
				( TFGameRules()->IsInfGamemode() && TFGameRules()->InSetup() )
			) 
			)
		{
...
void CTFDMMusicManager::SetRoundMusic( const char *szName )
{
	szRoundMusic = MAKE_STRING( szName );
	if( pRoundMusicPlayer )
		pRoundMusicPlayer->ChangeMusic( szName );
}

void CTFDMMusicManager::SetWaitingMusic( const char *szName )
{
	szWaitingForPlayerMusic = MAKE_STRING( szName );

	if( pWaitingMusicPlayer )
		pWaitingMusicPlayer->ChangeMusic( szName );
}

void CTFDMMusicManager::StartWinMusic()
{
	m_bDisableThink = true;
	if( pRoundMusicPlayer )
	{
		pRoundMusicPlayer->m_bHardTransition = true;
		pRoundMusicPlayer->SetDisabled(true);
	}
}

void CTFDMMusicManager::EndWinMusic()
{
	m_bDisableThink = false;
	if( pRoundMusicPlayer )
	{
		pRoundMusicPlayer->m_bHardTransition = false;
		pRoundMusicPlayer->SetDisabled(false);
	}
}

void CTFDMMusicManager::SetDisabled( bool bDisabled )
{
	m_bDisableThink = bDisabled;
	if( pRoundMusicPlayer )
	{
		pRoundMusicPlayer->SetDisabled(bDisabled);
	}
	if( pWaitingMusicPlayer )
	{
		pWaitingMusicPlayer->SetDisabled(bDisabled);
	}
}