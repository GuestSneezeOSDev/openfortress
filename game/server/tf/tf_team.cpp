...
IMPLEMENT_SERVERCLASS_ST( CTFTeam, DT_TFTeam )
	...
	SendPropInt( SENDINFO( m_iNumQuotaBots ) ),
	SendPropInt( SENDINFO( m_iNumLockdownBots ) ),
...
void CTFTeam::AddPlayer( CBasePlayer *pPlayer )
{
	...
	if( pPlayer->IsBotOfType( BOT_TYPE_INSOURCE ) )
		m_iNumQuotaBots++;
	else if( pPlayer->IsBotOfType( BOT_TYPE_LOCKDOWN ) )
		m_iNumLockdownBots++;
}
...
void CTFTeam::RemovePlayer( CBasePlayer *pPlayer )
{
	...
	TFGameRules()->OnTeamLostPlayer( GetTeamNumber(), pPlayer );
