...
ConVar sv_vote_issue_kick_allowed("sv_vote_issue_kick_allowed", "0", FCVAR_NONE, "Allow kick votes to be called. (0 = Disabled)");
ConVar sv_vote_kick_maxplayers_required( "sv_vote_kick_maxplayers_required", "3", FCVAR_NONE, "Amount of players that must be present in a server before the kick vote can be called." );
// default is 0 as people can't behave themselves
...
public:
	...
	CUtlVector< MapAndPlaytime_t > m_MapVotePool;
};
...
{
	if ( !sv_vote_issue_kick_allowed.GetBool() || sv_vote_kick_ban_duration.GetInt() <= 0 )
		return false;
...
int __cdecl SortMapsAndTimes( const MapAndPlaytime_t *p, const MapAndPlaytime_t *q )
{
	int x = p->seconds;
	int y = q->seconds;
...
{	
	...
	{	
		...
		// Make sure the selection is properly cleared
		PlayedMaps()->m_MapVotePool.RemoveAll();
		PlayedMaps()->m_MapVotePool.Purge();
		...
		FOR_EACH_VEC( PlayedMaps()->m_MapsAndPlaytimes, i )
		{
			if ( PlayedMaps()->m_MapsAndPlaytimes.IsValidIndex( i ) )
			{
				...
				// skip current map
				if( FStrEq( mapname, gpGlobals->mapname.ToCStr() ) )
				{
				...
				else
				{
					...
					PlayedMaps()->m_MapVotePool.AddToTail( MapAndPlaytime );
				}
			}
		}

		// sort them for the final output
		PlayedMaps()->m_MapVotePool.Sort(SortMapsAndTimes);

		// and finally put them into the vote menu
		FOR_EACH_VEC(PlayedMaps()->m_MapVotePool, i )
		{
			if ( !PlayedMaps()->m_MapVotePool.IsValidIndex( i ) )
				continue;

			if ( vecNames.Count() >= 5 )
				break;

			vecNames.AddToTail( PlayedMaps()->m_MapVotePool[i].mapname );
		}
	}
