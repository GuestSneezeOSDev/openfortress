...
public:
	...
	virtual void		Spawn();
			...
			void		ForceEquipCosmetics( const float *flCosmetics, int iCount, int iClass );
			void		ForceEquipCosmetics( const char *szCosmetics, int iClass );
			void		EquipRandomCosmetics( int iClass, int iCount = 5 );
			void		StripWeapons();
	...
	virtual int			TakeArmorTFC(int iArmorTFC);
	...
	// Utility.
	void				RemoveOwnedEnt( const char *pEntName, bool bGrenade = false );
	...
	void TeamFortress_ClientConnected( void );
	void TeamFortress_ClientDisconnected();
    SERVERONLY_EXPORT void TeamFortress_RemoveEverythingFromWorld();
	...
	float PlayLegShotReceivedSound(void);
	...
	bool SetWeaponInSlot( WeaponHandle hWeapon, int iSlot, int iSlotPos );
	...
	bool ShouldWaitForRespawn( void );
	void SetWaitForRespawn( int bWaitForRespawn ) { m_bWaitForRespawn = bWaitForRespawn; }	
	
	bool IsPressingRespawnButton( void );
	
	bool ShouldQuickZoom( void );

	int GetTargetFOV() { return (!m_hZoomOwner) ? 0 : m_iFOV; };
	...
	Vector GetPlayerColor() { return m_vecPlayerColor; }
...
public:
	...
	int					RestockAmmo( float PowerupSize, bool bIgnoreModifiers = false );
	int					RestockAmmo( float PowerupSize, CTFWeaponBase *pWeapon );
...
private:
	...
	// TODO: Maybe make the positions a vector? Slots can stay arrays as i doubt we'll need more than 10
	WeaponHandle m_hWeaponInSlot[MAX_WEAPON_SLOTS][MAX_WEAPON_POSITIONS];
public:
...
private:
	...
	CountdownTimer	LoadoutRequestTimer;
	// Client commands.
public:
	...
	bool				SetClass( const char *pClassName );
	bool				SetClass( int iClassIndex );

	bool				IsClassAllowed( int iClassIndex );
	int					GetRandomClass( void );
private:
...
//	float				m_flPowerPlayTime;
	...
	void				InputSetAttributeOfWeapon( inputdata_t &inputdata );
	void				InputAddAttributeToWeapon( inputdata_t &inputdata );
	void				InputRemoveAttributeFromWeapon( inputdata_t &inputdata );
