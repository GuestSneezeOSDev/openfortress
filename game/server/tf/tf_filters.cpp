...
class CFilterTFCArmor : public CBaseFilter
{
    DECLARE_CLASS( CFilterTFCArmor, CBaseFilter)
public:
...
LINK_ENTITY_TO_CLASS(filter_tfc_armor, CFilterTFCArmor);

BEGIN_DATADESC(CFilterTFCArmor)
    DEFINE_KEYFIELD( m_iFilterMode, FIELD_INTEGER, "operator"),
    DEFINE_KEYFIELD( m_iArmorAmount, FIELD_INTEGER, "amount"),
END_DATADESC()

enum FilterArmorMode
{
    GREATERTHAN,
    LESSTHAN,
    SAME,
...
bool CFilterTFCArmor::PassesFilterImpl(CBaseEntity *pCaller, CBaseEntity *pEntity)
{
    CTFPlayer *p = ToTFPlayer(pEntity);
    if (!p)
        return false;
    int armor = p->m_Shared.GetTFCArmor();
    switch (m_iFilterMode)
    {
    case SAME:
        return armor == m_iArmorAmount;
        break;
