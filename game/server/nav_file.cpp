...
NavErrorType COFNavArea::Load( CUtlBuffer &fileBuffer, unsigned int version, unsigned int subVersion )
{
	// TF2 uses version 2
	if( subVersion == 2 )
	{
		...
		// Just skip the extra data
		// Hard to figure out what exactly it does without reverse engineering - Kay
		fileBuffer.GetUnsignedInt();
		if ( !fileBuffer.IsValid() )
		{
			// Error message you get in TF2
	...
	else if ( subVersion > TheNavMesh->GetSubVersionNumber() )
	{
		// Error message you get in TF2
