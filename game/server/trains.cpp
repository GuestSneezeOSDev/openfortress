...
public:
    ...
    COutputEvent OnReachTop;
    COutputEvent OnReachBottom;

    void PlatSpawnTrigger();
    ...
    bool m_bIgnorePlayerBelow;
    float m_flReturntime = 0;
    TOGGLE_STATE m_Intialstate;
private:
...
BEGIN_DATADESC( CFuncPlat )
    ...
    // Variables
    DEFINE_KEYFIELD( m_flHeight, FIELD_FLOAT, "TravelDistance" ),
    DEFINE_KEYFIELD( m_bIgnorePlayerBelow, FIELD_BOOLEAN, "ignoreplayersbelow"),
    ...
    DEFINE_OUTPUT(OnReachTop, "OnReachTop"),
    DEFINE_OUTPUT(OnReachBottom, "OnReachBottom"),
...
// CPlatTrigger
//==================================================
class CPlatTrigger : public CBaseTrigger
{
    DECLARE_CLASS( CPlatTrigger, CBaseTrigger );
...
void CFuncPlat::Setup( void )
{
    ...
    m_vecPosition1 = GetAbsOrigin();	//Top
    m_vecPosition2 = GetAbsOrigin();	//Bottom

    if (m_flHeight > 0)// ensure upper posistion is always pos1
    {
        m_vecPosition2.z -= abs(m_flHeight);
    }
    else
    {
        m_vecPosition1.z += abs(m_flHeight);
    }
...
void CFuncPlat::Spawn( )
{
    ...
    if (m_flHeight > 0)
    {
        ...
        m_Intialstate = TS_AT_TOP;
    }
    else
    {
        m_toggle_state = TS_AT_BOTTOM;
        m_Intialstate = TS_AT_BOTTOM;
    }
...
void CFuncPlat::PlatSpawnTrigger()
{
    // old code: //GetClassPtr( (CPlatTrigger *)NULL)->SpawnInsideTrigger( GetClassPtr( (CFuncPlat *)pevPlatform ) );
    plattrig = CREATE_UNSAVED_ENTITY(CPlatTrigger, "plat_trigger");
    plattrig->SpawnInsideTrigger(this);
}
...
void CPlatTrigger::SpawnInsideTrigger( CFuncPlat *pPlatform )
{
    ...
    // Create trigger entity, set bounds and pos, give it a touch method
    SetSolid( SOLID_BBOX );
    AddSolidFlags( FSOLID_TRIGGER | FSOLID_NOT_SOLID);
    SetMoveType( MOVETYPE_NONE );
    SetAbsOrigin( pPlatform->GetAbsOrigin() );

    model_t* pModel = pPlatform->GetModel();
    modelinfo->GetModelBounds(pModel, mns, mxs);
    if (pPlatform->m_flHeight > 0)
    {
        mns.z -= pPlatform->m_flHeight;
        mxs.z += 96;
    }
    else
    {
        mxs.z += abs(pPlatform->m_flHeight) + 96;
    }
    Vector pos = GetAbsOrigin();
    UTIL_SetSize ( this, mns, mxs );
    SetCollisionBounds(mns, mxs);
    SetTouch(&CPlatTrigger::Touch);
}
...
void CPlatTrigger::Touch( CBaseEntity *pOther )
{
    ...
    if (pOther->GetAbsOrigin().z >= m_pPlatform->WorldSpaceCenter().z && pOther->GetAbsOrigin().z <= m_pPlatform->WorldSpaceCenter().z + 96 && m_bTriggerable)// on platform
    {
        // Make linked platform go up/down.
        if (m_pPlatform->m_toggle_state == TS_AT_BOTTOM)
        {
            m_pPlatform->SetThink(&CFuncPlat::UpThink);
            m_pPlatform->SetNextThink(gpGlobals->curtime);
            m_bTriggerable = false;// here so the think isnt infinitely pushed back
        }
        if (m_pPlatform->m_toggle_state == TS_AT_TOP)
        {
            m_pPlatform->SetThink(&CFuncPlat::DownThink);
            m_pPlatform->SetNextThink(gpGlobals->curtime);
...
void CFuncPlat::Touch(CBaseEntity *pOther) {
    if (HasSpawnFlags(SF_TRIGGER_ON_TOUCH)) {
        if (pOther) {
            if (pOther->IsPlayer()) {
                if (m_flHeight < 0) {
...
void CFuncPlat::DownThink()// at the top waiting until we can go down
{
    CBaseEntity* list[64];
    Vector pos = plattrig->GetAbsOrigin();
    int touching = UTIL_EntitiesInBox(list, 64, pos + plattrig->mns, pos + plattrig->mxs, 0);
    bool godown = true;

    if (!m_bIgnorePlayerBelow)
    {
        for (int i = 0; i < touching; i++)
        {
            CBasePlayer* player = dynamic_cast<CBasePlayer*>(list[i]);
            if (player)
            {
                if (player->GetAbsOrigin().z < GetAbsOrigin().z)//theres a player below us, dont go down
                    { godown = false;}
                if (player->GetAbsOrigin().z >= WorldSpaceCenter().z && player->GetAbsOrigin().z < WorldSpaceCenter().z + 96 && m_Intialstate == TS_AT_BOTTOM)//we started at the bottom and theres a player on top of us,
                    {godown = false;}
    ...
    else
    {
        for (int i = 0; i < touching; i++)
        {
            ...
            if (player)
            {
                if (player->GetAbsOrigin().z > WorldSpaceCenter().z && player->GetAbsOrigin().z < WorldSpaceCenter().z + 96 && m_Intialstate == TS_AT_BOTTOM)//we started at the bottom and theres a player on top of us, dont go down
                {godown = false;}
            }
        }
    }

    if (godown)// we're all good to go down
    {
        if (m_flReturntime < gpGlobals->curtime && m_Intialstate == TS_AT_BOTTOM || m_Intialstate == TS_AT_TOP)// needs to be nested otherwise returning is infinitely pushed back
        {
            GoDown();
            SetThink(NULL);
            SetNextThink(NULL);
            if (m_Intialstate == TS_AT_BOTTOM)
            {
                plattrig->m_bTriggerable = true;
            }
            return;
        }
    }
    else if (m_Intialstate == TS_AT_BOTTOM)
    {
        m_flReturntime = gpGlobals->curtime + 1;
    }
    SetNextThink(gpGlobals->curtime + 0.1);
}

void CFuncPlat::UpThink()//at bottom waiting to go up
{
    CBaseEntity* list[64];
    Vector pos = plattrig->GetAbsOrigin();
    int touching = UTIL_EntitiesInBox(list, 64, pos + plattrig->mns, pos + plattrig->mxs, 0);
    ...
    // cycle through all currently touching entitys, if its a player are they on the platform? if so prevent from going up if our first position was the higher one
    for (int i = 0; i < touching; i++)
    {
        CBasePlayer* player = dynamic_cast<CBasePlayer*>(list[i]);
        if (player)
        {
            if (player->GetAbsOrigin().z >= WorldSpaceCenter().z && player->GetAbsOrigin().z < WorldSpaceCenter().z + 96)//started at top and theres a player on us, dont go back up yet
                goup = false;
        }
    }
    ...
    if (goup)
    {
        if (m_flReturntime < gpGlobals->curtime && m_Intialstate == TS_AT_TOP || m_Intialstate == TS_AT_BOTTOM)
        {
            GoUp();
            SetThink(NULL);
            SetNextThink(NULL);
            if (m_Intialstate == TS_AT_TOP)
            {
                plattrig->m_bTriggerable = true;
            }
            return;
    ...
    else if (m_Intialstate == TS_AT_TOP)
    {
        m_flReturntime = gpGlobals->curtime + 1;
    }
        SetNextThink(gpGlobals->curtime + 0.1);
...
void CFuncPlat::HitBottom( void )
{
    ...
    m_toggle_state = TS_AT_BOTTOM;
    if (!IsTogglePlat() && m_Intialstate == TS_AT_TOP)
    {
        // After a delay, the platform will automatically go back to its original pos
        SetThink(&CFuncPlat::UpThink);
        SetNextThink(gpGlobals->curtime + 3);
    }
    OnReachBottom.FireOutput(this, this);
...
void CFuncPlat::HitTop( void )
{
    ...
    if ( !IsTogglePlat() && m_Intialstate == TS_AT_BOTTOM)
    {
        // After a delay, the platform will automatically go back to its original pos
        SetThink(&CFuncPlat::DownThink);
        SetNextThink(gpGlobals->curtime + 3);
    }
    OnReachTop.FireOutput(this, this);
}
