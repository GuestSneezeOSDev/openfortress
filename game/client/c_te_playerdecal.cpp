...
void ClearPlayerSpray( int player ) 
{
	C_BasePlayer *pPlayer = UTIL_PlayerByIndex( player );
	...
	// last 0 makes it transparent
	color32 rgbaColor = { 255, 255, 255, 0 };
...
void cc_SprayMutePlayerHandler_f( const CCommand& args,  bool bUnMute = false )
{
	char		*pszName = NULL;
	int			i = 0;
	char		name[64];
	...
	if ( args.ArgC() <= 1 )
	{
		ConMsg( "Usage:  %smute_player_spray < name >\n", bUnMute ? "un" : "");
		return;
	}
	...
	if ( pszName && pszName[0] != 0 )
	{
		...
		for ( i = 1 ; i <= gpGlobals->maxClients ; i++ )
		{
			...
			// found!
			if( Q_strcasecmp( pPlayer->GetPlayerName(), pszName ) )
				continue;

			GetClientVoiceMgr()->SetPlayerSprayBlockedState( pPlayer->entindex(), !bUnMute );
			Msg("%s player %s %s spray mute list.\n", bUnMute ? "Removed" : "Added", pPlayer->GetPlayerName(), bUnMute ? "from" : "to");

			if( !bUnMute )
				ClearPlayerSpray(pPlayer->entindex());
...
void cc_SprayMutePlayer_f( const CCommand& args )
{
	cc_SprayMutePlayerHandler_f( args, false );
}

void cc_SprayUnMutePlayer_f(const CCommand& args)
{
	cc_SprayMutePlayerHandler_f(args, true);
}

ConCommand mute_player_spray( "mute_player_spray", cc_SprayMutePlayer_f, "Prevent a players spray from appearing", FCVAR_NONE );
ConCommand unmute_player_spray( "unmute_player_spray", cc_SprayUnMutePlayer_f, "Allow a players spray to appear", FCVAR_NONE );
#endif
//-----------------------------------------------------------------------------
// Purpose: 
// Input  : filter - 
...
#ifdef OF_CLIENT_DLL
	// Don't show spray if they're muted
	if( GetClientVoiceMgr()->IsPlayerSprayBlocked(player) )
		return;
#endif
