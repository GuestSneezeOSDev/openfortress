...
void LoadHudTextures( CUtlDict< CHudTexture *, int >& list, KeyValues *pKV )
{
	KeyValues *pTemp, *pTextureSection;
	...
	// Read "*file"-to-prefix mapping.
	KeyValues *pTextureFileRefs = pKV->FindKey( "TextureFileRefs" );
	if ( pTextureFileRefs )
	{
		pTemp = pTextureFileRefs->GetFirstSubKey();
		while ( pTemp )
	...
	// Read our individual HUD texture data blocks.
	pTextureSection = pKV->FindKey( "TextureData" );
	if ( pTextureSection )
	{
		// Read the sprite data
		pTemp = pTextureSection->GetFirstSubKey();
