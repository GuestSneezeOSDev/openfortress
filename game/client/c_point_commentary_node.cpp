...
void CHudCommentary::Paint()
{
	...
	if ( COMMENTARY_BUTTONS & IN_ATTACK )
	{
		...
		if ( pszText )
		{
			wzFinal = UTIL_ReplaceKeyBindings( std::wstring(pszText) );
			vgui::surface()->DrawSetTextPos( m_iSpeakersX, iY );
			vgui::surface()->DrawPrintText( wzFinal.c_str(), wcslen(wzFinal.c_str()) );
		}
		...
		if ( pszText )
		{
			...
			wzFinal = UTIL_ReplaceKeyBindings(std::wstring(pszText));
			vgui::surface()->GetTextSize( hFont, wzFinal.c_str(), w, h);
