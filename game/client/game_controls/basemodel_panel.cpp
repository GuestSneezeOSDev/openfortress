...
CBaseModelPanel::CBaseModelPanel( vgui::Panel *pParent, const char *pName ): BaseClass( pParent, pName )
{
	...
	m_angPlayerDefault.Init();
	m_vecPlayerPosDefault.Init();

	m_angPlayerAnim.Init();
	m_vecPlayerPosAnim.Init();
}
...
void CBaseModelPanel::ParseModelResInfo( KeyValues *inResourceData )
{
	...
	m_angPlayerDefault = m_BMPResData.m_angModelPoseRot;
	m_vecPlayerPosDefault = m_BMPResData.m_vecOriginOffset;
...
void CBaseModelPanel::PerformLayout()
{
	...
	if ( m_bAllowFullManipulation )
	{
		...
		ResetCameraPivot();
		SetCameraOffset( -(GetFullPos() + kVecFocalPoint));
		...
		BaseClass::SetModelAnglesAndPosition( GetFullAngles(), vec3_origin);
	...
	if ( m_bForcePos )
	{
		...
		SetModelAnglesAndPosition( GetFullAngles(), GetFullPos() );
	}
...
void CBaseModelPanel::OnMousePressed ( vgui::MouseCode code )
{
	...
	int width, height;
	ipanel()->GetSize( GetVPanel(), width, height );
	int x = width / 2;
	int y = height / 2;
...
{
	m_angPlayerDefault.y += flDelta;
	if ( m_angPlayerDefault.y > 360.0f )
	{
		m_angPlayerDefault.y = m_angPlayerDefault.y - 360.0f;
	}
	else if ( m_angPlayerDefault.y < -360.0f )
	{
		m_angPlayerDefault.y = m_angPlayerDefault.y + 360.0f;
	}
...
{
	m_angPlayerDefault.x += flDelta;
	if ( m_angPlayerDefault.x > m_flMaxPitch )
	{
		m_angPlayerDefault.x = m_flMaxPitch;
	}
	else if ( m_angPlayerDefault.x < -m_flMaxPitch )
	{
		m_angPlayerDefault.x = -m_flMaxPitch;
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
Vector CBaseModelPanel::GetFullPos() const
{
	return m_vecPlayerPosDefault + m_vecPlayerPosAnim;
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
QAngle CBaseModelPanel::GetFullAngles() const
{
	return m_angPlayerDefault + m_angPlayerAnim;
}
...
void CBaseModelPanel::LookAtBounds( const Vector &vecBoundsMin, const Vector &vecBoundsMax )
{
	...
	m_angPlayerDefault = m_BMPResData.m_angModelPoseRot;
//	m_vecPlayerPosDefault = vecModelPos;
...
{
	Vector vPos = m_vecPlayerPosDefault;
	x = vPos.x;
	y = vPos.y;
	z = vPos.z;
}
...
{
	m_vecPlayerPosDefault.Init( x, y, z );
	SetModelAnglesAndPosition( GetFullAngles(), GetFullPos() );
}

void CBaseModelPanel::GetHUDModelAng( float &x, float &y, float &z )
{
	QAngle vAng = m_angPlayerDefault;
	x = vAng.x;
	y = vAng.y;
	z = vAng.z;
}
...
{
	m_angPlayerDefault.Init( x, y, z );
	SetModelAnglesAndPosition( GetFullAngles(), GetFullPos() );
}

void CBaseModelPanel::GetHUDModelAnimPos( float &x, float &y, float &z )
{
	Vector vPos = m_vecPlayerPosAnim;
	x = vPos.x;
	y = vPos.y;
	z = vPos.z;
}

void CBaseModelPanel::SetHUDModelAnimPos( float x, float y, float z )
{
	m_vecPlayerPosAnim.Init( x, y, z );
	SetModelAnglesAndPosition( GetFullAngles(), GetFullPos() );
}

void CBaseModelPanel::GetHUDModelAnimAng( float &x, float &y, float &z )
{
	QAngle vAng = m_angPlayerAnim;
	x = vAng.x;
	y = vAng.y;
	z = vAng.z;
}

void CBaseModelPanel::SetHUDModelAnimAng( float x, float y, float z )
{
	m_angPlayerAnim.Init( x, y, z );
	SetModelAnglesAndPosition( GetFullAngles(), GetFullPos() );
}

void CBaseModelPanel::ResetAnim()
{
	SetSequence( GetRootMDL().m_MDL.m_nSequence, true );
}