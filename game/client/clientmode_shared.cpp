...
SoundData_t *ClientModeShared::PlayAnnouncerLine( const char *szSound, bool bAnnouncer )
{
	char szFullSound[128];
	Q_snprintf( szFullSound, sizeof(szFullSound), "%s", szSound );
	
	if ( bAnnouncer )
	{
		Q_snprintf( szFullSound, sizeof(szFullSound), "%s.%s", ClientModeShared::GetAnnouncer(), szSound );
		FMODManager()->StopAllAnnouncerSounds();
	}
	return FMODManager()->PlaySound( szFullSound, bAnnouncer ? OF_CHANNEL_ANNOUNCER : OF_CHANNEL_GAME );
}

SoundData_t* ClientModeShared::PlayTeamAnnouncerLine( const char* szSound, int iTeam, bool bAnnouncer, int iPlayer )
{
	// Bail if excluded player...
	if (GetLocalPlayerIndex() == iPlayer)
		return NULL;

	bool bValidTeam = false;

	if ( (GetLocalTeam() && GetLocalTeam()->GetTeamNumber() == iTeam) )
	{
		bValidTeam = true;
	}
	...
	if ( bValidTeam == false )
	{
		...
		{
			if ( pSpectatorTarget->GetTeamNumber() == iTeam )
			{
				bValidTeam = true;
			}
		}
	...
	if ( bValidTeam == true )
	{
		return ClientModeShared::PlayAnnouncerLine( szSound, bAnnouncer );
	}

	return NULL;
}

SoundData_t* ClientModeShared::PlayFFAAnnouncerLine( const char* szSound, const char *szSoundRest, int iPlayer, bool bAnnouncer )
{
	...
	if ( pLocalPlayer->entindex() == iPlayer )
	{
	...
	{
		if ( GetSpectatorTarget() && (GetSpectatorMode() == OBS_MODE_IN_EYE || GetSpectatorMode() == OBS_MODE_CHASE || GetSpectatorMode() == OBS_MODE_POI) )
		{
			if ( GetSpectatorTarget() == GetSpectatorTarget() )
			{
				bTargetedPlayer = true;
			}
		}
	...
	if ( bTargetedPlayer == true )
		return ClientModeShared::PlayAnnouncerLine( szSound, bAnnouncer );
	else if ( szSoundRest && strcmp(szSoundRest, "") && bTargetedPlayer == false )
		return ClientModeShared::PlayAnnouncerLine( szSoundRest, bAnnouncer );
...
void ClientModeShared::FireGameEvent( IGameEvent *event )
{
	...
	{
		PlayTeamAnnouncerLine( 
			(char*)event->GetString( "sound" ),
			event->GetInt( "team" ),
			event->GetBool( "announcer" ),
			event->GetInt("exclude_players")
		);
	}
#ifdef OF_CLIENT_DLL
	else if (Q_strcmp( "ffa_broadcast_audio", eventname ) == 0 )
	{
		PlayFFAAnnouncerLine( 
			(char*)event->GetString( "sound" ),
			(char*)event->GetString("sound_rest"),
			event->GetInt( "player" ),
			event->GetBool( "announcer" )
		);
	}
...
//		DONE! - Kay
		...
		if ( pSupport )
		{
			COFAnnouncerInfo *pAnnouncer = pSupport->GetAnnouncer( of_announcer_override.GetString() );
			if( pAnnouncer && TFGameRules() )
			{
				if( pAnnouncer->SupportsCurrentGamemodes() )
