...
#ifdef OF_CLIENT_DLL
CRagdollExplosionEnumerator::CRagdollExplosionEnumerator( Vector origin, float radius, float magnitude, int damage )
#else
...
#ifdef OF_CLIENT_DLL
		pModel->ImpactTrace( &tr, DMG_BLAST, NULL, m_iDamage );
#else
...
#ifdef OF_CLIENT_DLL
	CRagdollExplosionEnumerator	ragdollEnum( m_vecOrigin, m_nRadius, m_nMagnitude, m_iDamage );
#else
...
void C_TEExplosion::RecordExplosion( )
{
	...
	if ( clienttools->IsInRecordingMode() )
	{
		...
		msg->SetInt( "attachment", m_iDamage );
...
#ifdef OF_CLIENT_DLL
	__g_C_TEExplosion.m_iDamage = iDamage;
#endif
...
#ifdef OF_CLIENT_DLL
	int iDamage = pKeyValues->GetInt( "attachment" );
#endif
