...
#endif // GLOWS_ENABLE
#else
	m_bServersideWeaponSwitch = false;
#endif
}
...
void C_BaseCombatCharacter::OnDataChanged( DataUpdateType_t updateType )
{
	...
	{
		// a bit scuffed but there isnt really a better way to make Deploy and Holster happen clientside
		if( m_bServersideWeaponSwitch )
		{
			EHANDLE m_hSwitchTo = m_hActiveWeapon;
			m_hActiveWeapon = m_hOldActiveWeapon;
			Weapon_Switch( (C_BaseCombatWeapon*)m_hSwitchTo.Get() );
		}
