...
#ifdef OF_CLIENT_DLL
C_BaseEntity *ParseImpactData( const CEffectData &data, Vector *vecOrigin, Vector *vecStart, Vector *vecShotDir, short &nSurfaceProp, int &iMaterial, int &iDamageType, int &iHitbox, int &iDamage );
#else
...
#ifdef OF_CLIENT_DLL
bool Impact( Vector &vecOrigin, Vector &vecStart, int iMaterial, int iDamageType, int iHitbox, C_BaseEntity *pEntity, trace_t &tr, int nFlags = 0, int maxLODToDecal = ADDDECAL_TO_ALL_LODS, int iDamage = 0 );
#else
...
public:
	// Forced constructor
#ifdef OF_CLIENT_DLL
	CRagdollEnumerator( Ray_t& shot, int iDamageType, int iDamage );
#else
