...
ConVar of_color_r( "of_color_r", "127", FCVAR_ARCHIVE | FCVAR_USERINFO, "Sets merc color's red channel value", true, -1, true, 255 );
ConVar of_color_g( "of_color_g", "71", FCVAR_ARCHIVE | FCVAR_USERINFO, "Sets merc color's green channel value", true, -1, true, 255 );
ConVar of_color_b( "of_color_b", "127", FCVAR_ARCHIVE | FCVAR_USERINFO, "Sets merc color's blue channel value", true, -1, true, 255 );
...
{
	COFCosmeticInfo *pCosmetic = GetItemSchema()->GetCosmetic( abs( atoi(args[3]) ) );
...
{
	COFSchemaWeaponInfo *pWpn = GetItemSchema()->GetWeapon( args[3] );
...
void LoadoutUnEquip( const CCommand& args )
{
	...
	COFSchemaWeaponInfo *pWpn = NULL;
	COFCosmeticInfo *pCosmetic = NULL;

	switch( iCategory )
	{
		default:
		case 0:
			pCosmetic = GetItemSchema()->GetCosmetic( abs( atoi(args[3]) ) );
		break;
		case 1:
			pWpn = GetItemSchema()->GetWeapon( args[3] );
		break;
	}
...
void ApplyCosmeticsToModel( C_BaseAnimating *pModel, int iPlayerIndex, CUtlVector<CosmeticHandle> *m_hCosmetic = NULL, int iTeamOverride = -1 )
{
	...
	{
		COFCosmeticInfo *pCosmetic = GetItemSchema()->GetCosmetic( g_TFPR->GetPlayerCosmetic( iPlayerIndex, i ) );
		if( !pCosmetic )
		{
		...
		// Hide back cosmetics if gameplay items are attached to the player's back
		if (Q_strcmp(pCosmetic->m_szRegion, "back") == 0)
		{
			if (pPlayer->m_Shared.InCond(TF_COND_JETPACK_POWERUP))
			{
				continue;
			}
			...
			bool bHasBackSuperWeapon = false;

			CTFWeaponBase* pWeapon = (CTFWeaponBase*) pPlayer->GetWeapon(0);
			for (int w = 0; w < pPlayer->WeaponCount(); ++w)
			{
				pWeapon = dynamic_cast<CTFWeaponBase*>(pPlayer->GetWeapon(w) );
				if (pWeapon && pWeapon->GetTFWpnData().m_bAppearOnBack && pWeapon != pPlayer->GetActiveTFWeapon())
				{
					bHasBackSuperWeapon = true;
					break;
				}
			}

			if (bHasBackSuperWeapon)
			{
				continue;
			}
		}
		...
		if( pCosmetic->m_Bodygroups.Count() && pPlayer && pPlayer->m_Shared.IsZombie() )
		{
			continue;
		}
		...
		{
			int m_Bodygroup = pModel->FindBodygroupByName( pCosmetic->m_Bodygroups.GetElementName(i) );

			if ( m_Bodygroup >= 0 )
			{
				pModel->SetBodygroup( m_Bodygroup, pCosmetic->m_Bodygroups.Element(i) );
			}
		}

		if( pCosmetic->m_szModel && Q_strcmp( pCosmetic->m_szModel, "BLANK" ) )
		{
			CosmeticHandle handle = C_PlayerAttachedModel::Create( pCosmetic->m_szModel, pModel, 0, vec3_origin, PAM_PERMANENT, 0, EF_BONEMERGE, false );	
...
void ApplyCosmeticsToModelPanel( CModelPanel *pModel, int iPlayerIndex, int iTeamOverride = -1 )
{
	...
	for( int i = 0; i < g_TFPR->GetPlayerCosmeticCount( iPlayerIndex ); i++ )
	{
		...
		FOR_EACH_DICT( pCosmetic->m_Bodygroups, i )
		{
			pModel->SetBodygroup( pCosmetic->m_Bodygroups.GetElementName(i), pCosmetic->m_Bodygroups.Element(i) );
		}
		...
		if( pCosmetic->m_szModel && Q_strcmp( pCosmetic->m_szModel, "BLANK" ) )
		{
			...
			m_nSkin = m_nSkin + (iTeamCount * pCosmetic->m_iSkinOffset);
			
			pModel->AddAttachment( pCosmetic->m_szModel ? pCosmetic->m_szModel : "models/empty.mdl", m_nSkin );
		}
	}	
}
...
// Head is weak and squishy, pop it like a watermelon
#define OF_GORE_HEAD_HEALTH 15
#define OF_GORE_BODY_HEALTH 30
...
C_TFRagdoll::C_TFRagdoll()
{
	...
	m_iHealthHead = OF_GORE_HEAD_HEALTH;

	// Ends of limbs get dismembered easier than the top parts
	m_iHealthLeftArm = OF_GORE_BODY_HEALTH / 2;
	m_iHealthRightArm = OF_GORE_BODY_HEALTH / 2;
	m_iHealthLeftLeg = OF_GORE_BODY_HEALTH / 2;
	m_iHealthRightLeg = OF_GORE_BODY_HEALTH / 2;
...
{
	m_iHealthLeftArm = OF_GORE_BODY_HEALTH;
	...
	m_iGoreLeftArm = 2 + bLevel;
}

void C_TFRagdoll::DismemberRightArm( bool bLevel )
{
	m_iHealthRightArm = OF_GORE_BODY_HEALTH;
...
//			*pCustomImpactName - 
//-----------------------------------------------------------------------------
void C_TFRagdoll::ImpactTrace( trace_t *pTrace, int iDamageType, const char *pCustomImpactName, int iDamage )
{
	...
	if ( m_bGoreEnabled )
	{
		...
		{
			case HITGROUP_HEAD:
				m_iHealthHead -= iDamage;
				if( m_iHealthHead <= 0 && m_iGoreHead <= 1 )
					DismemberHead();
				break;
			case HITGROUP_LEFTARM:
				m_iHealthLeftArm -= iDamage;
				if( m_iHealthLeftArm <= 0 && m_iGoreLeftArm < 3 )
					DismemberLeftArm( m_iGoreLeftArm == 2 );
				break;
			case HITGROUP_RIGHTARM:
				m_iHealthRightArm -= iDamage;
				if( m_iHealthRightArm <= 0 && m_iGoreRightArm < 3 )
					DismemberRightArm( m_iGoreRightArm == 2 );
				break;
			case HITGROUP_LEFTLEG:
				m_iHealthLeftLeg -= iDamage;
				if( m_iHealthLeftLeg <= 0 && m_iGoreLeftLeg < 3 )
					DismemberLeftLeg( m_iGoreLeftLeg == 2 );
				break;
			case HITGROUP_RIGHTLEG:
				m_iHealthRightLeg -= iDamage;
				if( m_iHealthRightLeg <= 0 && m_iGoreRightLeg < 3 )
					DismemberRightLeg( m_iGoreRightLeg == 2 );
				break;
			default:
				break;
			}
...
public:
	void OnBind( void *pC_BaseEntity )
	{
		...
		if( of_tennisball.GetInt() == 1 )
		{	
			float r = floorf( COLOR_OF_TENNISBALL[0] ) / 255.0f;
			float g = floorf( COLOR_OF_TENNISBALL[1] ) / 255.0f;
			float b = floorf( COLOR_OF_TENNISBALL[2] ) / 255.0f;	
...
void C_TFPlayer::OnDataChanged( DataUpdateType_t updateType )
{
	...
	// If the observer mode has changed, update the attached models on the spectator target, if it exists
	if (m_iOldObserverMode != GetObserverMode())
	{
		C_TFPlayer* pTarget = ToTFPlayer(GetObserverTarget());
		if (pTarget)
		{
			ToTFPlayer(pTarget)->UpdatePlayerAttachedModels();
		}
	}
	...
	//if (bAliveNotStealth && m_Shared.InCond(TF_COND_PIERCED_LEGS) && !m_pPiercedLegsEffect)
	//	CreatePiercedLegsEffect();
	...
	if( m_bUpdateCosmetics || m_bUpdatePlayerAttachments )
		UpdateVisibility();
...
void C_TFPlayer::UpdateGameplayAttachments( void )
{
	...
	bool bJetpackShouldExist = false;

	if (IsAlive() && GetTeamNumber() >= FIRST_GAME_TEAM && !IsPlayerClass(TF_CLASS_UNDEFINED))
	{
		// Spawn the jetpack even when in first person so that the particles have something to attach to
		if (m_Shared.InCond(TF_COND_JETPACK_POWERUP))
		{
			bJetpackShouldExist = true;

			if (!m_hJetpack)
			{
				m_hJetpack = C_PlayerAttachedModel::Create(JETPACK_MODEL, this, LookupAttachment("flag"), vec3_origin, PAM_PERMANENT, 0, EF_BONEMERGE, false);
				m_bUpdateCosmetics = true;
			}

			if (InFirstPersonView())
			{
				m_hJetpack->AddEffects(EF_NODRAW);
			}
			else
			{
				m_hJetpack->RemoveEffects(EF_NODRAW);
			}
		}
			...
			for (int i = 0; i < WeaponCount(); ++i)
			{
				...
				if (pWeapon && pWeapon->GetTFWpnData().m_bAppearOnBack && pWeapon != GetActiveTFWeapon())
				{
					...
					if (handle)
					{
						...
						if (GetSpectatorMode() == OBS_MODE_IN_EYE) // SourceTV wont have super weapons clip in first person view -Bry
							handle->AddEffects(EF_NODRAW);
	...
	if (!bJetpackShouldExist && m_hJetpack)
	{
		// Do not release the jetpack attachment on every update as this breaks the jetpack particles
		m_hJetpack->Release();
		m_bUpdateCosmetics = true;
	}
}
...
bool C_TFPlayer::UseThirdPersonCam( void )
{
	return 
		m_Shared.IsLoser()
	||	( m_Shared.InCond( TF_COND_TAUNTING ) && tf_taunt_first_person.GetBool() )
	||	( cl_thirdperson.GetBool() && (of_allow_third_person.GetBool() || sv_cheats->GetBool()) );
}
...
void C_TFPlayer::TurnOnThirdPersonCam( void )
{
	...
	// Don't need double third person
...
void C_TFPlayer::ClientThink()
{
	...
	if ( 
		(TFGameRules() && TFGameRules()->IsESCGamemode() && pLocalPlayer->GetTeamNumber() == GetTeamNumber() && IsPlayerClass( TF_CLASS_CIVILIAN )) || 
		(TFGameRules() && TFGameRules()->IsInfGamemode() && pLocalPlayer->GetTeamNumber() != GetTeamNumber() && pLocalPlayer->m_Shared.IsZombie() && m_Shared.IsLastSurvivor())
		)
	{
		m_bGlowEnabled = true;
		UpdateGlowEffect();
	...
	if ( ( pLocalPlayer != this ) && (pLocalPlayer->GetObserverMode() == OBS_MODE_CHASE && pLocalPlayer->GetObserverTarget() == this ) )
	{
		UpdateVisibility();
	}
	...
	// Kill the effect if either the player is dead, the spy is now invisible, if player is no longer tranqed
	//if (m_pPiercedLegsEffect && (bRemoveEffect || !m_Shared.InCond(TF_COND_PIERCED_LEGS)))
	//{
	//	ParticleProp()->StopEmissionAndDestroyImmediately(m_pPiercedLegsEffect);
	//	m_pPiercedLegsEffect = NULL;
	//}
}
...
void C_TFPlayer::ClientPlayerRespawn( void )
{
	...
	if ( of_respawn_particles.GetBool() )
	{
		if ( TFGameRules() && TFGameRules()->IsDMGamemode() && ( !InFirstPersonView() || !IsLocalPlayer() || of_first_person_respawn_particles.GetBool() ) )
		{
			char pEffectName[32];
			pEffectName[0] = '\0';
			Q_snprintf( pEffectName, sizeof( pEffectName ), "dm_respawn_%02d", m_Shared.GetSpawnEffects() );
...
// Purpose:  Creation Of Tranq Overhead Effect
//-----------------------------------------------------------------------------
void C_TFPlayer::CreatePiercedLegsEffect(void)
{
	// Don't create them for the local player
	if (IsLocalPlayer())
		return;

	m_pPiercedLegsEffect = ParticleProp()->Create("mark_for_death", PATTACH_POINT_FOLLOW, "head");
}
*/
//-----------------------------------------------------------------------------
// Purpose: 
...
void C_TFPlayer::ValidateModelIndex( void )
{
	if ( m_Shared.InCond( TF_COND_DISGUISED ) && IsEnemyPlayer() )
	{
		TFPlayerClassData_t *pData = GetPlayerClassData( m_Shared.GetDisguiseClass(), m_Shared.GetDisguiseClassMod() );
...
void C_TFPlayer::FireEvent(const Vector& origin, const QAngle& angles, int event, const char *options)
{
	...
	case AE_CL_BODYGROUP_SET_VALUE:
		...
		{
			// hacky!
			bool bUsingViewModel = InFirstPersonView() && !ShouldDrawThisPlayer();
			if( bUsingViewModel )
			{
				pWeapon = GetViewModel();
				if( !pWeapon )
					pWeapon = pTFWeapon;
			}
			else
