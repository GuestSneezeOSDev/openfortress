		...
		if (pEnt)
		{
			if (IsPlayerIndex(m_iTargetEntIndex))
			{
				...
				if (pPlayer->m_Shared.InCond(TF_COND_DISGUISED) && // they're disguised
					!pPlayer->m_Shared.InCond(TF_COND_DISGUISING) && // and they're not in the process of disguising
					!pPlayer->m_Shared.InCondInvis()) // and they're not cloaked
				{
				...
				bReturn = ( 
					...
					|| bDisguisedEnemy 
					|| pLocalTFPlayer->GetPlayerClass()->GetClassIndex() == TF_CLASS_SPY );

				if ( !( pLocalTFPlayer->GetPlayerClass()->GetClassIndex() == TF_CLASS_SPY ) && ( pPlayer->m_Shared.InCondInvis() && ( pLocalTFPlayer->GetTeamNumber() == TF_TEAM_MERCENARY && pPlayer->GetTeamNumber() == TF_TEAM_MERCENARY ) ) )
					bReturn = false;
			}
	...
	if ( m_iTargetEntIndex )
	{
		...
		if ( IsPlayerIndex( m_iTargetEntIndex ) )
		{
			...
			if ( pLocalTFPlayer->GetTeamNumber() == TF_TEAM_MERCENARY && !TFGameRules()->IsTeamplay() )
			{
				...
				if (bDisguisedEnemy || pLocalTFPlayer->IsPlayerClass(TF_CLASS_SPY)) bShowHealth = true;
