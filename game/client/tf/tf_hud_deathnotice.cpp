...
void CTFHudDeathNotice::Paint()
{
	...
	for ( int i = 0; i < iCount; i++ )
	{
		...
		// Draw victims name
		// Default color is default text color, in case of Capture points
		Color clr = GetInfoTextColor(i, msg.bLocalPlayerInvolved);

		if( TFGameRules()->IsDMGamemode() && !TFGameRules()->IsTeamplay() )
			clr = msg.Victim.iColor;
		else if( msg.Victim.iTeam != TEAM_UNASSIGNED )
			clr = GetTeamColor(msg.Victim.iTeam);
...
void CTFHudDeathNotice::FireGameEvent( IGameEvent *event )
{
	...
	if ( FStrEq( pszEventName, "player_death" ) || FStrEq( pszEventName, "object_destroyed" ) )
	{
		...
		if ((event->GetInt("damagebits") & DMG_CRITICAL) || (event->GetInt("customkill") == TF_DMG_CUSTOM_RAILGUN_HEADSHOT))
		{
...
void CTFHudDeathNotice::PlayRivalrySounds( int iKillerIndex, int iVictimIndex, int iType )
{
	...
	if ( iType == TF_DEATH_DOMINATION )
	{
		...
		)
		{
			...
			TeamplayRoundBasedRules()->BroadcastSoundFFA( iKillerIndex, "IsDominating" );
			TeamplayRoundBasedRules()->BroadcastSoundFFA( iVictimIndex, "GotDominated" );
		}
	}
	else if ( iType == TF_DEATH_REVENGE )
	{
		...
		if ( TeamplayRoundBasedRules() && TFGameRules() && TFGameRules()->IsDMGamemode() && !TFGameRules()->DontCountKills() )
		{
			...
			TeamplayRoundBasedRules()->BroadcastSoundFFA( iKillerIndex, "GotRevenge" );
...
void CTFHudDeathNotice::OnGameEvent( IGameEvent *event, int iDeathNoticeMsg )
{
	...
	if ( FStrEq( pszEventName, "player_death" ) || FStrEq( pszEventName, "object_destroyed" ) )
	{
		...
		case TF_DMG_CUSTOM_CROC:
			...
			msg.wzInfoText[0] = 0;
			break;
		case TF_DMG_CUSTOM_TAUNTATK_BOND:
			Q_strncpy(msg.szIcon, "d_taunt_mercenary", ARRAYSIZE(msg.szIcon));
			msg.wzInfoText[0] = 0;
			break;
		case TF_DMG_CUSTOM_REFLECT:
			if (killedwith && *killedwith)
			{
				DevMsg("Reflect Type = %s\n", killedwith);

				Q_snprintf(msg.szIcon, sizeof(msg.szIcon), "d_reflect_%s", killedwith);
				msg.wzInfoText[0] = 0;
				break;
			}
			else
			{
				Q_strncpy(msg.szIcon, "d_reflect_generic", ARRAYSIZE(msg.szIcon));
				msg.wzInfoText[0] = 0;
				break;
			}
		default:
