...
void CTFWinPanelDM::StartPanel( KeyValues *event )
{
	...
	if( DMMusicManager() )
	{
		COFSoundParameters *pScript = OFSoundManifest()->GetSoundscript(DMMusicManager()->szRoundMusic);
		if( pScript && ( pScript->GetOutro()[0] != '\0' || pScript->GetWinMusic()[0] != '\0' ) )
			...
			if (pPlayerModel)
			{
				...
				if (!of_disable_cosmetics.GetBool())
				{
					for (int i = 0; i < tf_PR->GetPlayerCosmeticCount(pPlayer->entindex()); i++)
					{
						...
						{
							COFCosmeticInfo* pCosmetic = GetItemSchema()->GetCosmetic( tf_PR->GetPlayerCosmetic(pPlayer->entindex(), i) );
							if( !pCosmetic )
								continue;
							...
							if( Q_strcmp(pCosmetic->m_szModel, "BLANK") )
							{
								...
								pPlayerModel->AddAttachment( pCosmetic->m_szModel ? pCosmetic->m_szModel : "models/empty.mdl", iSkin);
							}

							FOR_EACH_DICT( pCosmetic->m_Bodygroups, i )
							{
								pPlayerModel->SetBodygroup(pCosmetic->m_Bodygroups.GetElementName(i), pCosmetic->m_Bodygroups.Element(i));
							}
						}
					}
				}
