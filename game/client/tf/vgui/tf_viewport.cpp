...
CON_COMMAND( changeclass, "Choose a new class" )
{
	...
	if ( TFGameRules() && !TFGameRules()->IsClassSelectAllowed(iTeam) )
	{
		return;
	}
...
{
	// Fixes animations not updating when you have the in game menu open
	GetAnimationController()->UpdateAnimations( gpGlobals->curtime );
	BaseClass::OnTick();
}