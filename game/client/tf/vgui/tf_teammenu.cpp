...
void CTFTeamMenu::OnTick()
{
	...
	C_TFTeam *pMercenary = GetGlobalTFTeam(TF_TEAM_MERCENARY);

	if (!pRed || !pBlue || !pMercenary)
		return;

	int iBluePlayers = pBlue->Get_Number_Players();
	int iRedPlayers  = pRed->Get_Number_Players();
	int iMercPlayers = pMercenary->Get_Number_Players();

	if( TeamplayRoundBasedRules()->IgnoreBotsWhenBalancing() )
	{
		iBluePlayers -= pBlue->GetNumQuotaBots();
		iRedPlayers  -= pRed->GetNumQuotaBots();
		iMercPlayers -= pMercenary->GetNumQuotaBots();
	}

	// set our team counts
	SetDialogVariable( "bluecount", iBluePlayers );
	SetDialogVariable( "redcount", iRedPlayers );
	SetDialogVariable( "mercenarycount", iMercPlayers );
...
{
	const char *GameType = "Deathmatch";
	if ( !TFGameRules() )
		return GameType;


	if ( TFGameRules()->InGametype( TF_GAMETYPE_DM ) )
	{
		switch( TFGameRules()->GetMutator() )
		{
			...
			//}
			default:
				GameType = TFGameRules()->IsTDMGamemode() ? "TDM" : "Deathmatch";
		}
	}

	if ( TFGameRules()->InGametype( TF_GAMETYPE_DUEL ) )
		GameType = "Duel";
	...
	if ( TFGameRules()->InGametype( TF_GAMETYPE_CTF ) )
		GameType = TFGameRules()->Is3WaveGamemode() ? "3Wave" : "CTF";
...
void CTFDMTeamMenu::ShowPanel(bool bShow)
{
	...
	if (bShow)
	{
		...
		if( pPlayer && pPlayer->m_Shared.InState( TF_STATE_WELCOME ) )
		{
			ClientModeShared::PlayFFAAnnouncerLine( (char*)GetGamemodeMessage(), NULL, pPlayer->entindex(), true );
		}
