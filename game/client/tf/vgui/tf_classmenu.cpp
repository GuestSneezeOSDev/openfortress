...
CTFClassMenu::CTFClassMenu( IViewPort *pViewPort ) : CClassMenu( pViewPort )
{
	...
	EditablePanel* pRandom = new EditablePanel(this, "RandomPanel");

	m_pClassButtons[TF_CLASS_RANDOM] = new CTFImageMouseOverButton<CTFClassInfoPanel>(pRandom, "randompc", m_pClassInfoPanel, this);
...
void CTFClassMenu::UpdateNumClassLabels( int iTeam )
{
	...
	{
		int iClassCount = tf_PR->GetCountForPlayerClass( iTeam, g_sClassDefines[i], true );
		int iClassLimit = TFGameRules()->GetClassLimit(iTeam, i);

		std::string strClassCount = "";
		bool isSelectable = true;
	
		if ( iClassLimit == 0 )
		{
			strClassCount = "Blocked";
			isSelectable = false;
		}
		else if ( iClassLimit != -1 && iClassCount >= iClassLimit )
		{
			strClassCount = fmt::format(FMT_STRING("{:d} (Full)"), iClassCount);
			isSelectable = false;
		} 
		else if ( iClassCount > 0 )
		{
			strClassCount = fmt::format(FMT_STRING("{:d}"), iClassCount);
		}

		if (i < TF_CLASS_COUNT_ALL)
		{
			if (isSelectable)
			{
		...
		SetDialogVariable(g_sDialogVariables[i], strClassCount.c_str());
		...
		if ( nTotalCount < CLASS_COUNT_IMAGES )
		{
			for ( int j = 0 ; j < iClassCount; ++j )
			{
				...
				if ( pImage )
				{
					TFPlayerClassData_t *pClassData = GetPlayerClassData(i, 0);
