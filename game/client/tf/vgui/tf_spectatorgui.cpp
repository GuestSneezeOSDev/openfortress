...
void CTFSpectatorGUI::UpdateReinforcements( void )
{
	...
	else
	{
		...
		if ( 
			(!flNextRespawn && !pPlayer->ShouldWaitForRespawn()) || 
			( TFGameRules()->IsDMGamemode() || TFGameRules()->IsTDMGamemode() || TFGameRules()->IsInfGamemode() || TFGameRules()->IsGGGamemode() ) 
			)
		{
			m_pReinforcementsLabel->SetVisible( false );
			return;
		...
		if ( iRespawnWait <= 0 )
		{
			...
			if( pPlayer->ShouldWaitForRespawn() )
			{
...
void CTFSpectatorGUI::UpdateKeyLabels( void )
{
	...
	if ( m_pClassOrTeamLabel )
	{
		...
		if ( pPlayer )
		{
			...
			if ( wzTemp )
			{
				wsFinal = UTIL_ReplaceKeyBindings( std::wstring( wzTemp ) );
				m_pClassOrTeamLabel->SetText(wsFinal.c_str());
			}
		}
	}
	...
	if ( m_pSwitchCamModeKeyLabel )
	{
		...
		else
		{
			...
			wsLabel = UTIL_ReplaceKeyBindings( std::wstring(wzTemp) );
			m_pSwitchCamModeKeyLabel->SetText(wsLabel.c_str());
		}
	}
	...
	if ( m_pCycleTargetFwdKeyLabel )
	{
		...
		else
		{
			...
			wLabel = UTIL_ReplaceKeyBindings( std::wstring(wzTemp) );
			m_pCycleTargetFwdKeyLabel->SetText( wLabel.c_str() );
		}
	}
	...
	if ( m_pCycleTargetRevKeyLabel )
	{
		...
		else
		{
			...
			wLabel = UTIL_ReplaceKeyBindings( std::wstring(wzTemp) );
			m_pCycleTargetRevKeyLabel->SetText( wLabel.c_str() );
		}
	}
