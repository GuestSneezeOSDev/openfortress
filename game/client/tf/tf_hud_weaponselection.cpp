...
ConVar of_old_subweapon_select( "of_old_subweapon_select", "0", FCVAR_CLIENTDLL | FCVAR_ARCHIVE, "When enabled, weapon subcategories go from right to left.");
ConVar of_weaponswitch_show_binds( "of_weaponswitch_show_binds", "0", FCVAR_CLIENTDLL | FCVAR_ARCHIVE, "When enabled, numbers are replaced with keybindings for slots.");
...
private:
	...
	CPanelAnimationVar( int, m_iMaxRowSubSlots, "MaxRowSubSlots", "4" );
...
bool CHudWeaponSelection::ShouldDraw()
{
	...
	if (pPlayer && pPlayer->m_Shared.InCond( TF_COND_BERSERK ))
		return false;
...
//-------------------------------------------------------------------------
// Purpose: Calculates how many weapons sub slots need to be displayed
//-------------------------------------------------------------------------
int CHudWeaponSelection::GetNumVisibleSubSlots()
{
	...
	case HUDTYPE_FASTSWITCH:
			pSelectedWeapon = player->GetActiveTFWeapon();
		break;
	default:
	...
	for ( int i = 0; i < MAX_WEAPONS; i++ )
	{
		...
		if ( ( pWeapon->GetSlot() == iActiveSlot ) && ( pWeapon->VisibleInWeaponSelection() ) )
		{
			nCount++;
		}
	}
...
void CHudWeaponSelection::PostChildPaint()
{
	...
	int nNumSubSlots = GetNumVisibleSubSlots();
	if( nNumSubSlots == -1 )
		return;
	...
	default:
		{
			bool bLeftToRight = !of_old_subweapon_select.GetBool();
			// calculate where to start drawing
			int nAddedPosRows = ceil((float)nNumSubSlots / (float)m_iMaxRowSubSlots) - 1;
			int nTotalHeight = (nNumSlots - 1) * (m_flSmallBoxTall + m_flBoxGap) + m_flLargeBoxTall + ( (m_flLargeBoxTall + m_flBoxGap) * nAddedPosRows);
			...
			for ( int i = 0; i < m_iMaxSlots; i++ )
			{
				...
				if ( i == iActiveSlot )
				{
					int iPosLeft = nNumSubSlots;
					int iStartPos = xStartPos - m_flLargeBoxWide;

					if( bLeftToRight )
						iStartPos -= (min(iPosLeft-1, m_iMaxRowSubSlots - 1)) * (m_flLargeBoxWide + m_flBoxGap);

					xpos = iStartPos;

					bool bFirstItem = !bLeftToRight;
					int iLastPos = 0;
					for ( int slotpos = 0; slotpos < MAX_WEAPON_POSITIONS; slotpos++ )
					{
						...
						if( iLastPos >= m_iMaxRowSubSlots )
						{
							if( bLeftToRight && iPosLeft < m_iMaxRowSubSlots )
								iStartPos = (xStartPos - m_flLargeBoxWide) - ((iPosLeft-1) * (m_flLargeBoxWide + m_flBoxGap));
						...
						if( bLeftToRight )
							bFirstItem = nNumSubSlots - iPosLeft + 1 == min(m_iMaxRowSubSlots, nNumSubSlots);
						
						int shortcut = bFirstItem ? i + 1 : -1;
						if( of_weaponswitch_flat.GetInt() && bFirstItem)   
						{
						...
						if ( IsPC() && shortcut >= 0 )
						{
							...
							if( of_weaponswitch_show_binds.GetBool() )
							{
								const char *key = engine->Key_LookupBinding( VarArgs("slot%d",shortcut) );
								if( !key )
								{
									key = "< not bound >";
								}
								g_pVGuiLocalize->ConvertANSIToUnicode(key, wch, sizeof(wch));
							}
						
							DrawString(wch, xStartPos - XRES(4) - m_flSelectionNumberXPos, ypos + YRES(6) + m_flSelectionNumberYPos, numberShadowColor);
							DrawString(wch, xStartPos - XRES(5) - m_flSelectionNumberXPos, ypos + YRES(5) + m_flSelectionNumberYPos, numberColor);
						}
