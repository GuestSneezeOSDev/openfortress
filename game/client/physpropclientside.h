...
#ifdef OF_CLIENT_DLL
	virtual	void			ImpactTrace( trace_t *pTrace, int iDamageType, const char *pCustomImpactName, int iDamage );
#else
...
public:
...
#ifdef OF_CLIENT_DLL
	virtual void ImpactTrace(trace_t *pTrace, int iDamageType, char *pCustomImpactName, int iDamage)
#else
...
#endif
	{
		...
		if (GetRenderColor().a >= 255)
		{
			...
			// register the impact
#ifdef OF_CLIENT_DLL
			BaseClass::ImpactTrace(pTrace, iDamageType, pCustomImpactName, iDamage);
#else
