...
void UTIL_ImpactTrace( trace_t *pTrace, int iDamageType, const char *pCustomImpactName )
{
	...
	if ( pTrace->surface.flags & SURF_NODRAW )
		return;
#ifdef OF_CLIENT_DLL
	pEntity->ImpactTrace( pTrace, iDamageType, pCustomImpactName, 0 );
#else
...
// Output  :
//------------------------------------------------------------------------------
void UTIL_ImpactTraceDamage( trace_t *pTrace, int iDamageType, int iDamage, const char *pCustomImpactName )
{
	C_BaseEntity *pEntity = pTrace->m_pEnt;
	...
	pEntity->ImpactTrace( pTrace, iDamageType, pCustomImpactName, iDamage );
}
#endif
...
void UTIL_MakeSafeName( const char *oldName, char *newName, int newNameBufSize )
{
	...
	std::string std_string_name(oldName);

	if (std_string_name.at(0) == '#')
	{
		std_string_name.at(0) = '*';
	}
	UTIL_StripCharsFromSTDString(std_string_name, '%');
	UTIL_ReplaceAll(std_string_name, "&", "&&");
	memset(newName, 0x0, newNameBufSize);
	snprintf(newName, newNameBufSize, "%s", std_string_name.c_str());
}
...
std::vector<std::wstring> GetTokensFromWStr(const std::wstring& i_str, const std::wstring& i_delim)
{
	std::vector<std::wstring> result;
	size_t startIndex = 0;

	for (size_t found = i_str.find(i_delim); found != std::wstring::npos; found = i_str.find(i_delim, startIndex))
	{
...
#ifdef _DEBUG
CON_COMMAND(testKeyBindings, "run UTIL_ReplaceKeyBindings with custom text")
{
	const char* sargs = args.ArgS();
	std::wstring ws(sargs, sargs + strlen(sargs));
	std::wstring wsLabel = UTIL_ReplaceKeyBindings(ws);
	// Some test strings:
	// Press [ %changeclass% ] to Change Class 100%% of the time %%%%
	// ^ should give you:
	// Press [ <token> ] to Change Class 100% of the time %%
	// Press [ %changeclass% ] to Change Class
	Warning("out string = %S", wsLabel.c_str());
}
#endif


const std::wstring ConvertCmdTokenToFriendlyKeyWStr(const std::wstring& inToken)
{
	std::wstring outToken = {};

	// lookup key names
	char binding[64] = {};
	g_pVGuiLocalize->ConvertUnicodeToANSI(inToken.c_str(), binding, sizeof(binding));
	...
	//!! Since we don't use localised names for not bound, dont use this
	// char friendlyName[64] = {};
	// V_snprintf(friendlyName, sizeof(friendlyName), "%s", key);
	// Q_strupr(key);
	
	wchar_t* locName = g_pVGuiLocalize->Find(key);
	// no localized/friendly name
	if (!locName || wcslen(locName) <= 0)
	{
		wchar_t wToken[64] = {};
		g_pVGuiLocalize->ConvertANSIToUnicode(key, wToken, sizeof(wToken));
		outToken = std::wstring(wToken);
	}
	else
	{
		outToken = std::wstring(locName);
	}
	return outToken;
}

//-----------------------------------------------------------------------------
// Purpose: Looks up key bindings for commands and replaces them in the returned string
//			%<commandname>% will get replaced with its bound control,
//			e.g. %attack2% gets replaced with MOUSE2
//-----------------------------------------------------------------------------
std::wstring UTIL_ReplaceKeyBindings(const std::wstring &instring)
{
	Assert(!instring.empty());
	if (instring.empty())
	{
		return std::wstring(L"empty string?");
	}
	...
	{
		auto thischar = instring[pos];
		auto nextchar = instring[pos+1];
		// percent signs
		if (thischar == '%' && nextchar == '%')
		{
			newstring << '%';
			// skip to next non percent sign - notice it's only + 1 since we're pre-incrementing in our for loop
			pos++;
			continue;
		}
		else if (thischar == '%' /* && nextchar != '%' */)
		{
			size_t nextpos = instring.find_first_of('%', pos+1);
			if (nextpos == std::wstring::npos)
			{
				// bug/bad string (there are an odd # of percent signs, so we're missing or have an extra one
				Warning("Malformed token in \n\n%S\n\n(stopped at character %i)\n", instring.c_str(), pos);
				return std::wstring(L"malformed string, see console");
			}

			size_t size_of_substr	= (nextpos - pos) - 1;						// -1 whacks the ending percent sign
			std::wstring token		= instring.substr(pos+1, size_of_substr);	// pos+1 whacks the beginning percent sign
			std::wstring friendly	= ConvertCmdTokenToFriendlyKeyWStr(token);
