		...
		if ( iSelectedItem >= 0 )
		{
			...
			else if ( !V_stricmp( "ChangeMutator", pszIssueRaw ) )
			{
				...
				if ( m_pVoteParameterList->GetItemCount() == 0 )
				{
					...
					KeyValues *pMutator7 = new KeyValues("Arsenal");
					pMutator7->SetString("Name", "#TF_vote_mutator_arsenal");
					pMutator7->SetInt("index", 8);
					int iMutator7 = m_pVoteParameterList->AddItem(0, pMutator7);

					KeyValues *pMutator8 = new KeyValues("Eternal Shotguns");
					pMutator8->SetString("Name", "#TF_vote_mutator_eternalshotguns");
					pMutator8->SetInt("index", 9);
					...
					pMutator7->deleteThis();
					pMutator8->deleteThis();
					...
					if ( m_hIssueFont != INVALID_FONT )
					{
						...
						m_pVoteParameterList->SetItemFont(iMutator7, m_hIssueFont);
						m_pVoteParameterList->SetItemFgColor(iMutator7, m_IssueFGColor);
						m_pVoteParameterList->SetItemFont(iMutator8, m_hIssueFont);
						m_pVoteParameterList->SetItemFgColor(iMutator8, m_IssueFGColor);
					}
				}
			}
#endif			
...
void CHudVote::MsgFunc_VoteStart( bf_read &msg )
{
	...
	if ( m_iVoteCallerIdx != DEDICATED_SERVER )
	{
		...
		// In most circumstances, this is actually just the server, so lets remove this for now - Bryson
		//else
		//{
	...
	if ( m_bIsYesNoVote )
	{
		...
		if ( pszText )
		{
			...
			wsFinal = UTIL_ReplaceKeyBindings(std::wstring( pszText ));
			if ( m_pVoteActive )
				m_pVoteActive->SetControlString( "LabelOption1", wsFinal.c_str() );
		}
		...
		if ( pszText )
		{
			wsFinal = UTIL_ReplaceKeyBindings(std::wstring(pszText));
			if ( m_pVoteActive )
				m_pVoteActive->SetControlString( "LabelOption2", wsFinal.c_str() );
		}
	}
	else
	{
