...
public:
	...
	virtual void	CalcShake( bool bThirdPerson );
...
//-----------------------------------------------------------------------------
void CViewEffects::CalcShake( bool bThirdPerson )
{
	float	fraction, freq;
	...
	float fThirdPersonShakeMult = 1.0;

	// Reduce the amount of screen shake when in third-person
	if (bThirdPerson)
	{
		fThirdPersonShakeMult = 0.6;
	}
	...
	for ( int nShake = nShakeCount - 1; nShake >= 0; nShake-- )
	{
		...
		fraction = (( pShake->endtime - gpGlobals->curtime ) / pShake->duration) * fThirdPersonShakeMult;
