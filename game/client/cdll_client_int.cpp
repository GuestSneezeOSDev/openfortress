...
#include "gameui/VLoadingProgress.h"
...
int CHLClient::Init(CreateInterfaceFn appSystemFactory, CreateInterfaceFn physicsFactory, CGlobalVarsBase *pGlobals)
{
	...
	pBaseModPanel->SetSize( width, height );

	BaseModUI::LoadingProgress* pLoadingProgress = new BaseModUI::LoadingProgress( NULL, "LoadingProgress", BaseModUI::LoadingProgress::LWT_LOADINGPLAQUE );
	pLoadingProgress->InvalidateLayout( false, true );
	pLoadingProgress->SetVisible( false );
	pLoadingProgress->MakePopup( false );
	...
	*/
	...
	if ( mat_dxlevel )
	{
		...
		mat_dxlevel->AddFlags(FCVAR_DEVELOPMENTONLY);
	}
	...
	if ( mat_hdr_level )
	{
		...
		mat_hdr_level->AddFlags(FCVAR_DEVELOPMENTONLY);
	}
...
{
	// BONK: Cheats are using g_pClientMode to do stupid bullshit!
	...
	// UNDONE: Sporadic crashes with this func???
...
bool CHLClient::DisconnectAttempt( void )
{
	...
	// lol, we gotta do this or malicious servers will set
	// host_timescale to tiny values and break people's games
	// this will be de-kludged when I get engine detours set up
	// -sappho
