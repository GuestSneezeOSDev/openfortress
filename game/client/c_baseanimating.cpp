...
#ifdef OF_CLIENT_DLL
void C_ClientRagdoll::ImpactTrace( trace_t *pTrace, int iDamageType, const char *pCustomImpactName, int iDamage )
#else
...
void C_BaseAnimating::InitModelEffects( void )
{
	...
	TermRopes();
	FOR_EACH_VEC( m_hInitParticles, i )
	{
		if( !m_hInitParticles[i] )
			continue;

		ParticleProp()->StopEmissionAndDestroyImmediately( m_hInitParticles[i] );
	}
	m_hInitParticles.Purge();
}
...
void C_BaseAnimating::DelayedInitModelEffects( void )
{
	...
	if ( modelKeyValues->LoadFromBuffer( modelinfo->GetModelName( GetModel() ), modelinfo->GetModelKeyValueText( GetModel() ) ) )
	{
 		...
 		if ( !m_bNoModelParticles )
		{
			...
			if ( pkvAllParticleEffects && ShouldDraw() )
			{
				// Start grabbing the sounds and slotting them in
...
bool C_BaseAnimating::GetAttachment( int number, Vector &origin, QAngle &angles )
{
	...
	if ( !pData->m_bAnglesComputed )
	{
		...
		// _baseanimating.cpp(2315,30): warning C4463: overflow; assigning 1 to bit-field that can only hold values from -1 to 0
		// -sappho
...
void C_BaseAnimating::UpdateVisibility()
{
	...
	else if ( m_hInitParticles.Count() )
	{
		FOR_EACH_VEC( m_hInitParticles, i )
		{
			if( !m_hInitParticles[i] )
