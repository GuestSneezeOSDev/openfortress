...
private:
	...
	float cyflymder_ddoe;
	float cyflymder_echdoe;
	bool groundedInPreviousFrame;

	float FOVScale;

	Color colourDefault; // "Off-white"
	...
	Color *playerColour;	// The colour we'll use if the player-colour convar is 1 (This references one of the above two)
	
	Color playerColourShadowbase;
	Color *playerColourShadow;

	Color defaultVelVectorCol;
	Color defaultInputVectorCol;
	Color *vectorColor_vel;
	Color *vectorColor_input;
...
CHudSpeedometer::CHudSpeedometer(const char *pElementName) : CHudElement(pElementName), BaseClass(NULL, "HudSpeedometer")
{
	...
	cyflymder_ddoe = 0.0f;
	cyflymder_echdoe = 0.0f;
	groundedInPreviousFrame = false;

	FOVScale = 1.0f;

	colourDefault = Color(251, 235, 202, 255);
	playerColour = &colourDefault;
	
	playerColourShadowbase = Color(0, 0, 0, 255);
	playerColourShadow = &playerColourShadowbase;

	defaultVelVectorCol = Color(255, 0, 0, 255);
	defaultInputVectorCol = Color(0, 0, 255, 255);
