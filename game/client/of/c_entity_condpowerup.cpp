...
private:
	...
	private:
		...
		bool	m_bDisableIdleSound;
		bool	m_bRespawning;
		bool	m_bInitialDelay;
		bool	m_bShouldGlow;
		bool	m_bShouldIdleSoundPlay;
...
RecvPropBool( RECVINFO( m_bDisableIdleSound ) ),
...
void C_CondPowerup::Spawn( void )
{
	...
	//m_bDisableIdleSound = TFGameRules()->IsDuelGamemode() ? true : false;
...
void C_CondPowerup::ClientThink( void )
{
	...
	/*bool bShouldIdleSoundPlay = !m_bRespawning;

	if (bShouldIdleSoundPlay != m_bShouldIdleSoundPlay)
	{
	m_bShouldIdleSoundPlay = bShouldIdleSoundPlay;
	}

	if (!m_bDisableIdleSound && m_bShouldIdleSoundPlay && !building_cubemaps.GetBool())
	{
...
int C_CondPowerup::DrawModel( int flags )
{
	...
	pMesh->Draw();
	float RespawnTime = m_bInitialDelay ? fl_RespawnDelay : fl_RespawnTime;
