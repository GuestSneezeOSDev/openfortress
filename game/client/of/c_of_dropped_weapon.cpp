...
void C_TFDroppedWeapon::ClientThink( void )
{
	...
	if ( !pPlayer )
	{
		...
		// don't glow...
	if (!of_droppedweapons_glow.GetBool() || //..if glow is disabled...
		( !m_iReserveAmmo || !m_iClip ) || //...or if there's no ammo...
		( !of_allow_allclass_pickups.GetBool() && !pPlayer->GetPlayerClass()->IsClass( TF_CLASS_MERCENARY ) ) || //...or if not merc...
		(pPlayer->m_Shared.IsZombie())  	//...or if we're a zombie...
		)
	{
		if (m_bShouldGlow)
		{
