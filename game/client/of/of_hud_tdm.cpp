...
void CTFHudTDM::OnThink()
{
	...
	if ( m_flNextThink < gpGlobals->curtime )
	{
		...
		else
		{
			...
			int nLivesLeft = tf_PR->GetLives(iIndex);
			
			hudlcd->SetGlobalStat( "(kills)", VarArgs( "%d", nKills ) );
			if ( TFGameRules() && TFGameRules()->IsGGGamemode() )
				m_nKills = nGGLevel;
			else if (TFGameRules() && TFGameRules()->IsArenaGamemode())
				m_nKills = nLivesLeft;
			else
				m_nKills = nKills;
			...
			{
				SetDialogVariable("FragLimit", of_lives.GetInt());
				g_pVGuiLocalize->ConstructString(string1, sizeof(string1), g_pVGuiLocalize->Find("#TF_ScoreBoard_Lives"), 1, 1);
			}
			else
			{
