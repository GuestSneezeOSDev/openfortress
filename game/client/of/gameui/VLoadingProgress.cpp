	...
	m_LoadingWindowType( eLoadingType )
{
	...
	m_nLoadingInfoRef = 0;
	m_nLoadingProgressRef = 0;

	m_pOriginalProgressBar = NULL;
...
//-----------------------------------------------------------------------------
void LoadingProgress::OnActivate()
{
	m_bActive = true;

	LoadingProgress::LoadingType type = LoadingProgress::LT_POSTER;
	SetupControlStates();
...
//-----------------------------------------------------------------------------
void LoadingProgress::OnDeactivate()
{
	m_bActive = false;

	vgui::Label *pLabel = dynamic_cast<Label *>(FindChildByName("MapLabel"));
	...
	// Makes subsequent "map" commands open the loading menu
...
void LoadingProgress::FetchLoadingInfo()
{
	Panel *root_panel = GetGameUIRootPanel();

	if( !root_panel )
		return;

	VPANEL loading_panel = GetGameUIChildByName(root_panel, "LoadingDialog");
	Frame *loading_dialog = static_cast<Frame*>(GetGameUIPanel(loading_panel));

	if( !loading_dialog )
		return;

	m_nLoadingInfoRef = GetGameUIChildByName( loading_dialog, "InfoLabel" );
	m_nLoadingProgressRef = GetGameUIChildByName( loading_dialog, "Progress" );

	m_pOriginalProgressBar	= static_cast<ProgressBar*>(GetGameUIPanel(m_nLoadingProgressRef));
	m_pOriginalLabel		= static_cast<Label*>(GetGameUIPanel(m_nLoadingInfoRef));
}
...
void LoadingProgress::OnThink()
{
	...
	// Fetch info again if we failed at the start

	// UPDATE: Unfortunately, we can't check if the loading dialog has been deleted
	// And sometimes, it does in the middle of loading
	// Meaning all the pointers below are garbage...
	// We don't know of a way to have something like a handler, to check if they're valid
	// So we are forced to fetch all of the pannels from the ground up...
	// If anyone knows a better solution, feel free to tell us - Kay
//	if( m_nLoadingInfoRef == 0 || m_nLoadingProgressRef == 0 || GetGameUIRootPanel() == NULL )
	{
		FetchLoadingInfo();
	}
	...
	FetchAndSetProgressBar();

	CloseIfDisconnected();
}

void LoadingProgress::FetchAndSetStatus()
{
	if( m_pOriginalLabel )
	{
		char szInfo[MAX_PATH] = "";
		m_pOriginalLabel->GetText( szInfo, sizeof( szInfo ) );
		SetStatusText( szInfo );
	}
}

void LoadingProgress::FetchAndSetProgressBar()
{
	if( m_pOriginalProgressBar )
	{
		// Note: This is rather obsurd, however
		// for some reason pProgressBar->GetProgress() does not fetch the correct data
		// so we get the settings instead, which DOES return the correct values
		// a little more resource intensive but not too bad - Kay

		KeyValues* test = new KeyValues("test");
		m_pOriginalProgressBar->GetSettings(test);

		float flProgress = test->GetFloat("progress", 0.0f);
...
void LoadingProgress::CloseIfDisconnected()
{
	Panel *root_panel = GetGameUIRootPanel();
	...
	const wchar_t *loading_title = surface()->GetTitle(loading_panel);
	if( !Q_wcscmp(loading_title, g_pVGuiLocalize->Find("#GameUI_Disconnected")) )
		Close();
}
...
void LoadingProgress::PaintBackground()
{
	...
	if ( m_bDrawPoster && m_pFooter )
	{
		...
		ipanel()->GetSize( CBaseModPanel::GetSingleton().GetVPanel(), screenWidth, screenHeight );

		int x, y, wide, tall;
		ipanel()->GetPos( m_pFooter->GetVPanel(), x, y );
		ipanel()->GetSize( m_pFooter->GetVPanel(), wide, tall );
