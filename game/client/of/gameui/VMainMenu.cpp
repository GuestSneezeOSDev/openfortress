...
extern void LeaveGameOkCallback();
...
void MainMenu::OnCommand( const char *command )
{
	...
	else if (!Q_strcmp(command, "showcompendium"))
	{
		CBaseModPanel::GetSingleton().OpenWindow(WT_LORE_COMPENDIUM, this, false);
	}
...
void MainMenu::OnThink()
{
	...
	if( !CBaseModPanel::GetSingletonPtr()->IsInLevel() && m_bInGame )
	{
		m_bInGame = false;
		InvalidateLayout(true, true);
	}
	else if( CBaseModPanel::GetSingletonPtr()->IsInLevel() && !m_bInGame )
	{
		m_bInGame = true;
		InvalidateLayout(true, true);
	}
...
void MainMenu::ApplySchemeSettings( IScheme *pScheme )
{
	...
	// Makes the first "map" command properly open the loading screen
	...
	int iVersion = atoi(engine->GetProductVersionString());
				
	iVersion -= 10000000;

	SetDialogVariable( "PatchVersion", iVersion );

	//SetFooterState();

	if( m_bInGame )
	{
		pSettings = "Resource/UI/BaseModUI/IngameMainMenu.res";
	}
	else
	{
