...
#define REFERENCE_SCREEN_HEIGHT 1080
...
private:
	...
	float	m_flLerpTime;			// Timer
	float	m_flTargetFadeTime;		// The fade time (different for turning blur on and off, two directions)
	bool	m_bBlurEnabled;			// Controls the lerp direction
	bool	m_bLerpDone;
...
#define CURSOR_VERTS 4
	...
	float	m_flScreenScaleFactor;
	bool	bLayoutInvalidated;
};
...
ConVar hud_weaponwheel_hold( "hud_weaponwheel_hold", "1", FCVAR_ARCHIVE, "When true, holding down last inv will trigger the weapon wheel, otherwise only +weaponwheel opens it." );
...
CHudWeaponWheel::CHudWeaponWheel( const char *pElementName ) : CHudElement( pElementName ), BaseClass( NULL, "HudWeaponwheel" ), m_rawinput( "m_rawinput" )
{
	...
	m_flLerpTime = 0.0f;
	m_flTargetFadeTime = -1.0f;
	m_bBlurEnabled = false;
	m_bLerpDone = true;

	m_iLastScreenHeight = -1;
	m_flScreenScaleFactor = 1.0f;
	...
	{
		segments[i].bHasIcon = false;
		segments[i].bucketSelected = 0;
		segments[i].defaultBucket = 0;
	}
	...
	if ( of_weaponswitch_flat.GetBool() && bucket < 0 )
	{
		...
		for ( int i = 0; i < id; i++ )
		{
			...
			if ( hudSelection->GetFirstPos( i ) )
			{
				...
				if( !pWeapon->VisibleInWeaponSelection() ) continue;
	...
	if ( GetHudWeaponSelection() )
	{
		for ( int slot = 0; slot < numberOfSegments; slot++ )
		{
			...
			for ( int bucketSlot = 0; bucketSlot < MAX_WEAPON_POSITIONS; bucketSlot++ )
			{
				...
				if ( weaponInSlot && weaponInSlot->VisibleInWeaponSelection() )
				{
...
void CHudWeaponWheel::RefreshWheelVerts( void )
{
	...
	m_flScreenScaleFactor = (float)m_iLastScreenHeight / (float)REFERENCE_SCREEN_HEIGHT;
...
void CHudWeaponWheel::OnTick( void )
{
	...
	if ( m_bLastInvHeld )
	{
		if ( gpGlobals->curtime >= m_flLastInvHoldTimer && !m_bWeaponWheelOpen && hud_weaponwheel_hold.GetBool() )
		{
			IN_WeaponWheelDown();
		}
	...
	if ( weaponSelect )
	{
		for ( int slot = 0; slot < numberOfSegments; slot++ )
		{
			...
			{
				CBaseCombatWeapon *pWeapon = weaponSelect->GetWeaponInSlot( slot, bucket );
				if( pWeapon && pWeapon->VisibleInWeaponSelection() )
					weaponsThisTick++;
			}
		}
		...
		if( pSelectedWeapon && pSelectedWeapon->VisibleInWeaponSelection() )
		{
...
void CHudWeaponWheel::OnMouseWheeled( int delta )
{
	...
	if ( bWheelActive )
	{
		...
		if ( slotSelected >= 0 )
		{
			...
			{
				CBaseCombatWeapon *pWeapon = pHUDSelection->GetWeaponInSlot( slotSelected, i );
				if ( pWeapon && pWeapon->VisibleInWeaponSelection() )
				{
					wepsCurrentlyInSlot++;
					lastValidIndex = i;
			...
			if ( wepsCurrentlyInSlot > 0 )
			{
				...
				while ( iterations < MAX_WEAPON_POSITIONS )
				{
					...
					CBaseCombatWeapon *pWeapon = pHUDSelection->GetWeaponInSlot( slotSelected, bucket );
					if ( pWeapon && pWeapon->VisibleInWeaponSelection() )
						break;
