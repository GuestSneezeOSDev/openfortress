...
C_EntityDissolve::C_EntityDissolve( void )
{
	...
	m_vParticleColor = Vector( 255, 255, 255 );
}
...
void C_EntityDissolve::OnDataChanged( DataUpdateType_t updateType )
{
	...
	if ( updateType == DATA_UPDATE_CREATED )
	{
		...
		if( GetMoveParent() )
		{
			C_TFRagdoll *pRagdoll = dynamic_cast<C_TFRagdoll*>(GetMoveParent());
			if( pRagdoll )
			{
				C_TFPlayer *pPlayer = ToTFPlayer(pRagdoll->GetPlayerHandle());
				if( pPlayer && GetTFPlayerResource() )
					m_vParticleColor = GetTFPlayerResource()->GetPlayerColorVector(pPlayer->entindex()) * 255.0f;
			}
		}
	}
}
...
int C_EntityDissolve::DrawModel( int flags )
{
	...
	for ( int i = 0; i < set->numhitboxes; ++i )
	{
		...
		for ( int j = 0; j < iTempParts; j++ )
		{
			...
			sParticle->m_uchColor[0]	= m_vParticleColor.x;
			sParticle->m_uchColor[1]	= m_vParticleColor.y;
			sParticle->m_uchColor[2]	= m_vParticleColor.z;
