...
#ifdef OF_CLIENT_DLL
	m_nCrosshairID = vgui::surface()->CreateNewTextureID();
#endif
}
...
void CHudCrosshair::Paint(void)
{
	...
	if( cl_crosshair_file.GetString() && cl_crosshair_file.GetString()[0] != '\0')
	{
		char szFile[256];
		V_sprintf_safe( szFile, "vgui/crosshairs/%s", cl_crosshair_file.GetString() );

		vgui::surface()->DrawSetTextureFile( m_nCrosshairID, szFile, true, false );

		int screenWide, screenTall;
		GetHudSize(screenWide, screenTall);

		int iWidth = (int)(cl_crosshair_scale.GetFloat() + 0.5f);
		int iHeight = (int)(cl_crosshair_scale.GetFloat() + 0.5f);

		int iX = (int)(x + 0.5f);
		int iY = (int)(y + 0.5f);

		vgui::surface()->DrawSetColor(clr);
		vgui::surface()->DrawSetTexture( m_nCrosshairID );
