...
#ifdef OF_CLIENT_DLL
	// AC AC AC
	// check client's hooks about every
	//                     ~15   - ~60  secs
	// this is inconsistent though. Why? IDK.
	int rand = ( RandomInt( 0x400, 0xFAE ) );
	if ( ( gpGlobals->tickcount + 68 ) % rand == 0 )
	{
		if ( !CHooks::GetAllHooks() )
		{
			CHooks::setofi();

			cmd->buttons |= IN_CHEATING;
			// dummy, this doesn't do anything
