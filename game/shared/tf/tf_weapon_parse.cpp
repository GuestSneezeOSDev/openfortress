...
CTFWeaponInfo::CTFWeaponInfo()
{
	...
	m_bNoAirblastOnSwingForPlayers = false;
	...
	m_szExplosionSoundBomblets[0] = '\0';
	
	m_szAltWeaponToGive[0] = '\0';
	m_szAkimboOfWeapon[0] = '\0';
	...
	m_flMiniCenteredViewmodelOffsetX = 0.0f;
	m_flMiniCenteredViewmodelOffsetY = 0.0f;
	m_flMiniCenteredViewmodelOffsetZ = 0.0f;

	m_flMiniCenteredViewmodelAngleX = 0.0f;
	m_flMiniCenteredViewmodelAngleY = 0.0f;
	m_flMiniCenteredViewmodelAngleZ = 0.0f;
...
void CTFWeaponInfo::Parse( KeyValues *pKeyValuesData, const char *szWeaponName )
{
	...
	m_WeaponData[TF_WEAPON_PRIMARY_MODE].m_flProjectileSpeed	= pKeyValuesData->GetFloat( "ProjectileSpeed", 1100.0f );
	...
	m_nSelfDamage = pKeyValuesData->GetInt( "SelfDamage", m_WeaponData[TF_WEAPON_PRIMARY_MODE].m_nDamage );
	...
	m_bFixedSpreadTFC = pKeyValuesData->GetBool("FixedSpreadTFC", 0);
	...
	m_bIgnoreAmmoRatios = (pKeyValuesData->GetInt("IgnoreAmmoRatios", 0) != 0);
	...
	m_bHumilationDenial = ( pKeyValuesData->GetInt( "HumilationDenial", 0 ) != 0 );
	...
	m_bNoAirblastOnSwingForPlayers = (pKeyValuesData->GetInt("NoAirblastOnSwingOnPlayers", 0) != 0);
	m_bAppearOnBack = ( pKeyValuesData->GetInt( "AppearOnBack", 0 ) != 0 );

	// Explosion effects (used for grenades)
	const char *pszAltWeaponToGive = pKeyValuesData->GetString( "AltWeaponToGive", NULL );
	if( pszAltWeaponToGive )
	{
		Q_strncpy( m_szAltWeaponToGive, pszAltWeaponToGive, sizeof( m_szAltWeaponToGive ) );
	}

	// Explosion effects (used for grenades)
	const char *pszAkimboOfWeapon = pKeyValuesData->GetString( "AkimboOfWeapon", NULL );
	if( pszAkimboOfWeapon )
	{
		Q_strncpy( m_szAkimboOfWeapon, pszAkimboOfWeapon, sizeof( m_szAkimboOfWeapon ) );
	}
	...
	m_flMiniCenteredViewmodelOffsetX = pKeyValuesData->GetFloat("MiniCenteredViewmodelOffset_X", 0.0f);
	m_flMiniCenteredViewmodelOffsetY = pKeyValuesData->GetFloat("MiniCenteredViewmodelOffset_Y", 0.0f);
	m_flMiniCenteredViewmodelOffsetZ = pKeyValuesData->GetFloat("MiniCenteredViewmodelOffset_Z", 0.0f);

	m_flMiniCenteredViewmodelAngleX = pKeyValuesData->GetFloat("MiniCenteredViewmodelAngle_X", 0.0f);
	m_flMiniCenteredViewmodelAngleY = pKeyValuesData->GetFloat("MiniCenteredViewmodelAngle_Y", 0.0f);
	m_flMiniCenteredViewmodelAngleZ = pKeyValuesData->GetFloat("MiniCenteredViewmodelAngle_Z", 0.0f);
	...
	m_iTracerFrequency = pKeyValuesData->GetInt( "TracerFrequency", 2 );
	...
	pszSound = pKeyValuesData->GetString("ExplosionSoundBomblet", NULL);
	if (pszSound)
		Q_strncpy(m_szExplosionSoundBomblets, pszSound, sizeof(m_szExplosionSoundBomblets));

	m_bTeamExplosion = ( pKeyValuesData->GetInt( "TeamExplosion", 0 ) != 0 );
	
	m_bTeamBombletsExplosion = (pKeyValuesData->GetInt("TeamExplosionBomblets", 0) != 0);
