...
ConVar of_zombie_lunge_crit("of_zombie_lunge_crit", "0", FCVAR_REPLICATED | FCVAR_NOTIFY, "Enables crits on lung's autohit.");
...
ConVar of_berserk_knockback("of_berserk_knockback", "0.25", FCVAR_NOTIFY | FCVAR_REPLICATED, "Multiplies damage impulse for all damage types when you have berserk.");
...
RecvPropEHandle( RECVINFO( m_pWeaponSpawner ) ),
...
RecvPropBool( RECVINFO( m_bIsLastSurvivor)),
...
RecvPropBool( RECVINFO( m_bLaunchedFromJumppad) ),
// Spy.
...
RecvPropInt( RECVINFO( m_nDisguiseClassMod ) ),
...
DEFINE_PRED_FIELD( m_bLaunchedFromJumppad, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE),
// These three dont really need a prediction field tbh? TODO: Remove this and test if anything breaks
DEFINE_PRED_FIELD(m_bIsZombie, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE),
DEFINE_PRED_FIELD(m_bIsLastSurvivor, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE),
...
SendPropEHandle( SENDINFO( m_pWeaponSpawner ) ),
...
SendPropInt( SENDINFO( m_bIsLastSurvivor ), 1, SPROP_UNSIGNED | SPROP_CHANGES_OFTEN ),
...
SendPropInt( SENDINFO( m_iArmor ), -1, SPROP_CHANGES_OFTEN ),
SendPropBool( SENDINFO (m_bLaunchedFromJumppad)),

// Spy
SendPropTime( SENDINFO( m_flInvisChangeCompleteTime ) ),
SendPropInt( SENDINFO( m_nDisguiseTeam ), 3, SPROP_UNSIGNED ),
SendPropInt( SENDINFO( m_nDisguiseClass ),  Q_log2( TF_CLASS_COUNT_ALL )+1, SPROP_UNSIGNED ),
SendPropInt( SENDINFO( m_nDisguiseClassMod ), Q_log2( TF_CLASSMOD_LAST )+1, SPROP_UNSIGNED ),
...
//-----------------------------------------------------------------------------
SERVERONLY_DLL_EXPORT void CTFPlayerShared::AddCond( int nCond, float flDuration /* = PERMANENT_CONDITION */ )
{
...
//-----------------------------------------------------------------------------
SERVERONLY_DLL_EXPORT void CTFPlayerShared::RemoveCond( int nCond )
{
	Assert( nCond >= 0 && nCond < TF_COND_LAST );
...
void CTFPlayerShared::OnDataChanged( void )
{
	...
	if( m_bOldIsLoser != IsLoser() )
	{
		if( IsLoser() )
			m_pOuter->TurnOnThirdPersonCam();
		else
			m_pOuter->TurnOffThirdPersonCam();
	}
}
...
void CTFPlayerShared::OnConditionAdded( int nCond )
{
	...
	case TF_COND_PIERCED_LEGS:
		OnAddPiercedLegs();
		break;

	case TF_COND_JETPACK_POWERUP:
		OnAddJetpackPowerup();
		break;

	case TF_COND_JETPACK_ENGAGED:
		OnAddJetpackEngaged();
		break;
...
void CTFPlayerShared::OnConditionRemoved( int nCond )
{
	...
	case TF_COND_JETPACK_POWERUP:
		OnRemoveJetpackPowerup();
		break;

	case TF_COND_JETPACK_ENGAGED:
		OnRemoveJetpackEngaged();
		break;
...
//-----------------------------------------------------------------------------
// Purpose:Runs Health related server sided stuff, such as Overheal and Health Decay
//-----------------------------------------------------------------------------
void CTFPlayerShared::ConditionHealthBuff(void)
{
#ifdef GAME_DLL
	...
	if (InCond(TF_COND_HEALTH_BUFF))
	{
		...
		if (InCond(TF_COND_PIERCED_LEGS))
			m_flPiercedLegsRemoveTime -= 2.f * gpGlobals->frametime;
	}
...
void CTFPlayerShared::OnAddCritBoosted( int iCond )
{
	...
	UpdateCritParticle();
#else
	m_pOuter->EmitSound("Weapon_General.CritPower");
#endif
}
...
void CTFPlayerShared::OnRemoveBerserk( void )
{
	...
	if ( pTFPlayer && pTFPlayer->IsAlive() )
	{
		...
		CTFWeaponBase *pWeapon = (CTFWeaponBase *)pTFPlayer->Weapon_OwnsThisType( "TF_WEAPON_BERSERK" );
		if( pWeapon )
		{
...
void CTFPlayerShared::OnAddPiercedLegs(void)
{
	m_pOuter->TeamFortress_SetSpeed();
}

void CTFPlayerShared::OnRemovePiercedLegs(void)
{
	m_pOuter->TeamFortress_SetSpeed();
}

void CTFPlayerShared::OnAddJetpackPowerup(void)
{
#ifdef CLIENT_DLL
	if (m_pOuter->IsLocalPlayer() || (IsLocalPlayerSpectator() && GetSpectatorTarget() == m_pOuter->entindex()))
	{
		m_pOuter->EmitSound("Powerup.Jetpack.Added");
	}
...
void CTFPlayerShared::OnRemoveJetpackPowerup(void)
{
	if (InCond(TF_COND_JETPACK_ENGAGED))
	{
		RemoveCond(TF_COND_JETPACK_ENGAGED);
	}

#ifdef CLIENT_DLL
	if (m_pOuter->IsLocalPlayer() || (IsLocalPlayerSpectator() && GetSpectatorTarget() == m_pOuter->entindex()))
	{
		m_pOuter->EmitSound("Powerup.Jetpack.Removed");
	}
...
void CTFPlayerShared::OnAddJetpackEngaged(void)
{
	SetJetpackEngaged(true);

#ifdef GAME_DLL
	UTIL_ScreenShakePlayer(m_pOuter, 1.5, 150.0, 5.0, SHAKE_START, true);
#endif

#ifdef CLIENT_DLL
	if (m_pOuter->m_hJetpack)
	{
		m_pOuter->m_hJetpack->ParticleProp()->Create("rockettrail", PATTACH_POINT_FOLLOW, "thruster");
		m_pOuter->m_hJetpack->ParticleProp()->Create("rocketbackblast", PATTACH_POINT_FOLLOW, "thruster");
	}

	m_pOuter->StopSound("Powerup.Jetpack.Loop");
	m_pOuter->EmitSound("Powerup.Jetpack.Loop");
#endif
}

void CTFPlayerShared::OnRemoveJetpackEngaged(void)
{
	SetJetpackEngaged(false);

#ifdef GAME_DLL
	UTIL_ScreenShakePlayer(m_pOuter, 0.0, 0.0, 0.0, SHAKE_STOP);
#endif

#ifdef CLIENT_DLL
	if (m_pOuter->m_hJetpack)
	{
		m_pOuter->m_hJetpack->ParticleProp()->StopParticlesNamed("rockettrail", false);
		m_pOuter->m_hJetpack->ParticleProp()->StopParticlesNamed("rocketbackblast", false);
	}

	m_pOuter->StopSound("Powerup.Jetpack.Loop");
	m_pOuter->EmitSound("Powerup.JetPack.Disengage");
#endif
}
...
// Purpose: 
//-----------------------------------------------------------------------------
SERVERONLY_DLL_EXPORT void CTFPlayerShared::Burn( CTFPlayer *pAttacker, float flTime )
{
#ifdef GAME_DLL
...
void CTFPlayerShared::FuckUpLegs(CTFPlayer *pAttacker, float flTime, float flSpeed)
{
	...
	if (!InCond(TF_COND_PIERCED_LEGS) && !InCondUber())
	{
		// Start sloweness
		m_flPiercedLegsSlowness = flSpeed;
		AddCond(TF_COND_PIERCED_LEGS, flTime);
	}
	else if (InCond(TF_COND_PIERCED_LEGS) && !InCondUber())
	{
		if (m_flPiercedLegsSlowness - (1.0 - flSpeed) < 0.5)
		{
			m_flPiercedLegsSlowness = 0.5;
		}
		else
		{
			float m_flPiercedLegsSlownessAddTogether = m_flPiercedLegsSlowness - (1.0 - flSpeed);
			m_flPiercedLegsSlowness = m_flPiercedLegsSlownessAddTogether;
		}
		float m_flNewPiercedLegsTime = (flTime / 2) + GetConditionDuration(TF_COND_PIERCED_LEGS);
		//Msg("m_flNewPiercedLegsTime is %\n", m_flNewPiercedLegsTime);
		//Msg("TF_COND_PIERCED_LEGS Condition Duration is %f\n", GetConditionDuration(TF_COND_PIERCED_LEGS));
		AddCond(TF_COND_PIERCED_LEGS, m_flNewPiercedLegsTime);
	}
	else
		return;
#endif
...
void CTFPlayerShared::OnRemoveDisguised( void )
{
	...
	{
		TFPlayerClassData_t *pData = GetPlayerClassData( m_pOuter->GetPlayerClass()->GetClassIndex(), m_pOuter->GetPlayerClass()->GetModifier() );
	...
	m_nDisguiseClassMod.Set( TF_CLASSMOD_NONE );
...
//-----------------------------------------------------------------------------
SERVERONLY_DLL_EXPORT void CTFPlayerShared::Disguise( int nTeam, int nClass )
{
#ifdef GAME_DLL
...
void CTFPlayerShared::CompleteDisguise( void )
{
	...
	if( GetDisguiseTarget() )
		m_nDisguiseClassMod = TFGameRules()->GetPlayerClassMod( (CTFPlayer*)GetDisguiseTarget().Get() );
	else
		m_nDisguiseClassMod = TFGameRules()->GetTeamClassMod( m_nDisguiseTeam );
...
void CTFPlayerShared::RecalcDisguiseWeapon( void )
{
	...
	CTFWeaponInfo *pLastValidInfo = NULL;

	TFPlayerClassData_t *pData = GetPlayerClassData( m_nDisguiseClass, m_nDisguiseClassMod );

	Assert( pData );

	// Find the weapon in the same slot
	int i;
	for ( i = 0; i < pData->m_iWeaponCount; i++ )
	{
		if ( pData->m_aWeapons[i] != TF_WEAPON_NONE )
		{
			const char *pWpnName = GetItemSchema()->GetWeaponName( pData->m_aWeapons[i] );
			if( !pWpnName )
				continue;

			UTIL_PrecacheSchemaWeapon( pWpnName );
			WEAPON_FILE_INFO_HANDLE	hWpnInfo = LookupWeaponInfoSlot( pWpnName );
			Assert( hWpnInfo != GetInvalidWeaponInfoHandle() );
			CTFWeaponInfo *pWeaponInfo = dynamic_cast<CTFWeaponInfo*>( GetFileWeaponInfoFromHandle( hWpnInfo ) );
			pLastValidInfo = pWeaponInfo ? pWeaponInfo : pLastValidInfo;
	...
	if( !pDisguiseWeaponInfo )
		pDisguiseWeaponInfo = pLastValidInfo;
...
bool CTFPlayerShared::UpdateParticleColor( CNewParticleEffect *pParticle )
{
	...
	{
		Vector vecColor = m_pOuter->m_vecPlayerColor;
#ifdef OF_CLIENT_DLL
...
#endif
		pParticle->SetControlPoint( CUSTOM_COLOR_CP1, vecColor );
		return true;
	}
	return false;
}
...
void CTFPlayerShared::SetInvulnerable( bool bState, bool bInstant )
{
	...
	if ( bState )
	{
		...
		if (InCond(TF_COND_PIERCED_LEGS))
			RemoveCond(TF_COND_PIERCED_LEGS);
...
bool CTFPlayerShared::IsLastSurvivor(void)
{
	return m_bIsLastSurvivor;
}

void CTFPlayerShared::SetSurvivor(bool bSurvivor)
{
	m_bIsLastSurvivor = bSurvivor;
}
//-----------------------------------------------------------------------------
// Purpose: Team check.
//-----------------------------------------------------------------------------
...
void CTFPlayerShared::SetSwapWeaponSpawner( CBaseEntity *pEnt )
{
	m_pWeaponSpawner = pEnt;
}

CBaseEntity *CTFPlayerShared::GetSwapWeaponSpawner( void )
{
	return m_pWeaponSpawner;
}
...
void CTFPlayer::FireBullet( const FireBulletsInfo_t &info, bool bDoEffects, int nDamageType, int nCustomDamageType /*= TF_DMG_CUSTOM_NONE*/ )
{
	...
	// Trace didn't hit anything, let's loop thru players and test this trace against hitboxes this time
	// to make 100% sure we didn't hit anything
	// fixes https://www.youtube.com/watch?v=q0l0mRV7UC8
	// -sappho
	if ( !trace.startsolid && !trace.DidHitNonWorldEntity() )
	{
		// ignore shooter
		CTraceFilterSimple filter( this, COLLISION_GROUP_NONE );
		// sets the closest playertrace (in &trace), if we hit any client at all
		// if we didn't, trace.fraction will be 1.0
		UTIL_ClipTraceToPlayers( vecStart, vecEnd, ( MASK_SOLID | CONTENTS_HITBOX ), &filter, &trace );
	}
	...
	if ( trace.fraction < 1.0 )
	{
		...
		if ( bDoEffects )
		{
			...
			else
			{
				...
				{
					UTIL_ImpactTraceDamage( &trace, nDamageType, info.m_flDamage );
				}
			}
...
void CTFPlayer::TeamFortress_SetSpeed()
{
	...
	if ( IsObserver() )
	{
		if ( GetObserverMode() == OBS_MODE_ROAMING )
			SetMaxSpeed( GetPlayerClassData( TF_CLASS_SCOUT, 0 )->m_flMaxSpeed );
		else
			SetMaxSpeed( 0 );
		return;
	}
	...
	{
		float flMaxDisguiseSpeed = GetPlayerClassData( m_Shared.GetDisguiseClass(), m_Shared.GetDisguiseClassMod() )->m_flMaxSpeed;
	...
	if (m_Shared.InCond(TF_COND_AIMING_TFC))
	{
		if (maxfbspeed > 80)
			maxfbspeed = 80;
	}
	...
	if (m_Shared.InCond(TF_COND_TRANQ) && m_Shared.InCond(TF_COND_PIERCED_LEGS))
	{
		//1...if it passes, check to see if PiercedLegslown is 50% slowdown...A
		if (m_Shared.m_flPiercedLegsSlowness == 0.5)
		{
		...
		else
		{
			...
			if ( 1 - ( (1 - m_Shared.m_flPiercedLegsSlowness) + ( 1 - m_Shared.m_flTranqMovementSlowness) ) <= 0.5 )
			{
			...
			else
			{
				...
				maxfbspeed *= 1 - ( (1 - m_Shared.m_flPiercedLegsSlowness) + (1 - m_Shared.m_flTranqMovementSlowness) );
			}
		}
	}
	...
	else
	{
		...
		else if (m_Shared.InCond(TF_COND_PIERCED_LEGS))
		{
			maxfbspeed *= m_Shared.m_flPiercedLegsSlowness;
		}
	}
...
#ifdef GAME_DLL
	const char *pszWeaponName = GetObjectInfo(iObjectType)->m_bVisibleInWeaponSelection ? GetObjectInfo(iObjectType)->m_pObjectName : "tf_weapon_builder";
	CTFWeaponBuilder *pObj = (CTFWeaponBuilder*) WeaponSchema_OwnsThisID( GetItemSchema()->GetWeaponID(pszWeaponName) );
	if( !pObj )
		return CB_CANNOT_BUILD;

	if( !pObj->m_iBuildableObjects.HasElement( iObjectType ) )
		return CB_CANNOT_BUILD;
#endif
...
bool CTFPlayer::ShouldWaitForRespawn( void )
{
	// Dont make bots wait indefinitely
#ifdef OF_DLL
	if( IsFakeClient() )
		return false;
#endif
	
	if( TFGameRules() && TFGameRules()->IsDuelGamemode() )
		return false;
	
	// Serverside, take the player var
#ifdef OF_DLL
	return m_bWaitForRespawn;
	// clientside, just take the convar directly
#else
	return of_wait_for_respawn.GetBool();
#endif
}

bool CTFPlayer::IsPressingRespawnButton( void )
{
	return  ( m_nButtons & IN_USE		)
...
/*
int CTFPlayer::GetMaxAmmo( int iAmmoIndex, int iClassNumber = -1, int iModifier = -1 )
{
	...
	if ( iClassNumber != -1 && iModifier != -1 )
	{
		iMaxAmmo = GetPlayerClassData( iClassNumber, iModifier )->m_aAmmoMax[iAmmoIndex];
	}
	else
	{
