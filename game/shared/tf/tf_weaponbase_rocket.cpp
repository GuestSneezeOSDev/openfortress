...
CTFBaseRocket *CTFBaseRocket::Create( CTFWeaponBase *pWeapon, const char *pszClassname, const Vector &vecOrigin, 
									  const QAngle &vecAngles, CBaseEntity *pOwner, float flSpeed )
{
	...
	if ( pWeapon )
	{
		pRocket->m_nWeaponID = pWeapon->GetWeaponID();
		...
		pRocket->SetExplosionVisualInfo( &visuals );

		visuals = pWeapon->GetBombletExplosionVisualInfo();
		pRocket->SetBombletExplosionVisualInfo( &visuals );
	}
	// Spawn.
	pRocket->Spawn();
...
#ifdef OF_DLL
void CTFBaseRocket::SetExplosionVisualInfo( TFExplosion_Visuals_t* pInfo )
{
	m_ExplosionVisualInfo = *pInfo;
	m_ExplosionVisualInfo.m_iTeam = GetTeamNumber();
}

void CTFBaseRocket::SetBombletExplosionVisualInfo( TFExplosion_Visuals_t* pInfo )
{
...
void CTFBaseRocket::ExplodeManualy( trace_t *pTrace, int bitsDamageType, int bitsCustomDamageType )
{
	...
	{
		TE_TFExplosion( filter, 0.0f, vecOrigin, pTrace->plane.normal, -1, GetBombletExplosionVisualInfo() );
	}
...
void CTFBaseRocket::Explode( trace_t *pTrace, CBaseEntity *pOther )
{
	...
	int iEntIndex = 0;
	if ( pOther )
		iEntIndex = pOther->entindex();

	TE_TFExplosion( filter, 0.0f, vecOrigin, pTrace->plane.normal, iEntIndex, GetExplosionVisualInfo() );
