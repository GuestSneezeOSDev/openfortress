...
ConVar  of_lives					( "of_lives", "1", FCVAR_NOTIFY | FCVAR_REPLICATED, "How many lives a player has in the Arena mode." );
ConVar  of_lives_frags_score		( "of_lives_frags_score", "0", FCVAR_NOTIFY | FCVAR_REPLICATED, "Allow Frags and Lives to be combined for winning score." );
ConVar  of_arena_lives_multiplier	( "of_arena_lives_multiplier", "1", FCVAR_NOTIFY | FCVAR_REPLICATED, "The multiplier number for Lives when of_lives_frags_score is enabled." );
ConVar  of_arena_frags_multiplier	( "of_arena_frags_multiplier", "2", FCVAR_NOTIFY | FCVAR_REPLICATED, "The multiplier number for Frags when of_lives_frags_score is enabled." );
...
ConVar of_merc_only_arsenal			( "of_merc_only_arsenal", "0", FCVAR_NOTIFY | FCVAR_REPLICATED, "If Arsenal is enabled and so is this command. Merc will be only the one who uses the Arsenal Loadout." );
...
ConVar of_spawners_resupply			("of_spawners_resupply", "0", FCVAR_NOTIFY | FCVAR_REPLICATED, "If you gain ammo from weapons spawners of weapons you already own.");
ConVar of_spawners_ammo_ratio_resupply	("of_spawners_ammo_ratio_resupply", "0.75", FCVAR_NOTIFY | FCVAR_REPLICATED, "How much ammo you gain from Weapon Spawners on weapons already owned.");

ConVar of_pickups_ammo_ratio			("of_pickups_ammo_ratio", "0.25", FCVAR_NOTIFY | FCVAR_REPLICATED, "How much ammo you gain from Dropped Weapons Pickup.");
ConVar of_spawners_ammo_ratio			("of_spawners_ammo_ratio", "0.50", FCVAR_NOTIFY | FCVAR_REPLICATED, "How much ammo you gain from Weapon Spawners on first pickup.");

ConVar of_allow_civ_pickups			("of_allow_civ_pickups", "0", FCVAR_NOTIFY | FCVAR_REPLICATED, "Allow Civillians to pickup dropped weapons.");
ConVar of_civ_pickups_ammo_ratio	("of_civ_pickups_ammo_ratio", "0.10", FCVAR_NOTIFY | FCVAR_REPLICATED, "How much ammo Civillian pickup from dropped weapons.");
...
ConVar of_mutator			( "of_mutator", "0", FCVAR_NOTIFY | FCVAR_REPLICATED,
							...
							true, 0, true, 8 );
ConVar of_dom_scorelimit_multiplier  ("of_dom_scorelimit_multiplier", "1", FCVAR_NOTIFY | FCVAR_REPLICATED, "Multiplies points need to win in (mercenary) Domination");
...
ConVar of_armor_on_spawn("of_armor_on_spawn", "-1", FCVAR_NOTIFY | FCVAR_REPLICATED, "If armor is allowed, sets if you spawn with or without armor.");
ConVar of_multiweapons( "of_multiweapons", "1", FCVAR_NOTIFY | FCVAR_REPLICATED,
			"Toggles the Quake-like multi weapon system." );
ConVar of_weapon_slot_count( "of_weapon_slot_count", "4", FCVAR_NOTIFY | FCVAR_REPLICATED,
	"If more than 0, makes the weapon system functon like an inventory, where you can have up to x weapons dependant on the setting.", true, 0, true, 9 );
...
ConVar of_fix_random_fall_damage("of_fix_random_fall_damage", "1", FCVAR_NOTIFY | FCVAR_REPLICATED,
	"If 1, fall damage will be consistent from the same height. If 0, fall damage will have a +/-20% random spread.",
	true, 0, true, 1);
...
ConVar of_classlimit_scout("of_classlimit_scout", "-1", FCVAR_REPLICATED, "Class limit for Scouts. -1 indicates no limit, 0 blocks the class from being selected. Usage: <any team limit>[,<blue>][,<red>].");
ConVar of_classlimit_sniper("of_classlimit_sniper", "-1", FCVAR_REPLICATED, "Class limit for Snipers. -1 indicates no limit, 0 blocks the class from being selected. Usage: <any team limit>[,<blue>][,<red>].");
ConVar of_classlimit_soldier("of_classlimit_soldier", "-1", FCVAR_REPLICATED, "Class limit for Soldiers. -1 indicates no limit, 0 blocks the class from being selected. Usage: <any team limit>[,<blue>][,<red>].");
ConVar of_classlimit_demoman("of_classlimit_demoman", "-1", FCVAR_REPLICATED, "Class limit for Demomen. -1 indicates no limit, 0 blocks the class from being selected. Usage: <any team limit>[,<blue>][,<red>].");
ConVar of_classlimit_medic("of_classlimit_medic", "-1", FCVAR_REPLICATED, "Class limit for Medics. -1 indicates no limit, 0 blocks the class from being selected. Usage: <any team limit>[,<blue>][,<red>].");
ConVar of_classlimit_heavy("of_classlimit_heavy", "-1", FCVAR_REPLICATED, "Class limit for Heavies. -1 indicates no limit, 0 blocks the class from being selected. Usage: <any team limit>[,<blue>][,<red>].");
ConVar of_classlimit_pyro("of_classlimit_pyro", "-1", FCVAR_REPLICATED, "Class limit for Pyros. -1 indicates no limit, 0 blocks the class from being selected. Usage: <any team limit>[,<blue>][,<red>].");
ConVar of_classlimit_spy("of_classlimit_spy", "-1", FCVAR_REPLICATED, "Class limit for Spies. -1 indicates no limit, 0 blocks the class from being selected. Usage: <any team limit>[,<blue>][,<red>].");
ConVar of_classlimit_engineer("of_classlimit_engineer", "-1", FCVAR_REPLICATED, "Class limit for Engineers. -1 indicates no limit, 0 blocks the class from being selected. Usage: <any team limit>[,<blue>][,<red>].");
ConVar of_classlimit_mercenary("of_classlimit_mercenary", "-1", FCVAR_REPLICATED, "Class limit for Mercenaries. -1 indicates no limit, 0 blocks the class from being selected. Usage: <any team limit>[,<blue>][,<red>].");
ConVar of_classlimit_civilian("of_classlimit_civilian", "-1", FCVAR_REPLICATED, "Class limit for Civilians. -1 indicates no limit, 0 blocks the class from being selected. Usage: <any team limit>[,<blue>][,<red>].");
ConVar of_classlimit_juggernaut("of_classlimit_juggernaut", "-1", FCVAR_REPLICATED, "Class limit for Juggernauts. -1 indicates no limit, 0 blocks the class from being selected. Usage: <any team limit>[,<blue>][,<red>].");
...
ConVar of_armor("of_armor", "-1", FCVAR_REPLICATED | FCVAR_NOTIFY, \
	"Enables armor.\n-1 = Depends on Retro mode\n 0 = Forced off\n 1 = Forced on");

ConVar of_armor_ammo_repair("of_armor_ammo_repair", "0", FCVAR_REPLICATED | FCVAR_NOTIFY, \
	"Allows ammo packs to repair armor.\n 0 = Forced off\n 1 = Forced on");
...
BEGIN_NETWORK_TABLE_NOBASE( CTFGameRules, DT_TFGameRules )
	...
	RecvPropBool(RECVINFO(m_bArmorOnSpawn)),
	...
	SendPropInt( SENDINFO( m_nHuntedCount_red ), 10, SPROP_UNSIGNED ),
	SendPropInt( SENDINFO( m_nMaxHunted_red ), 10, SPROP_UNSIGNED ),
	SendPropInt( SENDINFO( m_nHuntedCount_blu ), 10, SPROP_UNSIGNED ),
	SendPropInt( SENDINFO( m_nMaxHunted_blu ), 10, SPROP_UNSIGNED ),
    SendPropInt( SENDINFO( m_nDomScore_limit ), 16, SPROP_UNSIGNED ),
    SendPropInt( SENDINFO( m_nDomScore_blue ), 16, SPROP_UNSIGNED ),
    SendPropInt( SENDINFO( m_nDomScore_red ), 16, SPROP_UNSIGNED ),
	...
	SendPropBool(SENDINFO(m_bArmorOnSpawn)),
...
BEGIN_DATADESC( CTFGameRulesProxy )
	...
	DEFINE_KEYFIELD( m_bArmorOnSpawn, FIELD_BOOLEAN, "ArmorOnSpawn"),
...
void CTFGameRulesProxy::Activate()
{
	...
	TFGameRules()->m_bArmorOnSpawn = m_bArmorOnSpawn;
...
void CTFLogicDOM::Spawn( void )
{
	if ( TFGameRules() )
	{
        TFGameRules()->m_nDomScore_limit = (int)(m_nDomScore_limit * of_dom_scorelimit_multiplier.GetFloat());
...
//	}
	...
	m_bHasRedSpawns = false;
	m_bHasBlueSpawns = false;
	...
	char szCommand[MAX_MAP_NAME + 12];
	...
	ListenForGameEvent( "infection_lms" );
#endif
...
void CTFGameRules::Activate()
{
	...
	else if ( ( TFGameRules()->IsDMGamemode() && !TFGameRules()->IsTeamplay() ) || ( TFGameRules()->IsInfGamemode() || TFGameRules()->IsCoopEnabled()  || TFGameRules()->IsArenaGamemode()) )
...
//	m_nGameType.Set(TF_GAMETYPE_UNDEFINED);
	...
	if ( gEntList.FindEntityByClassname(NULL, "of_logic_inf") || !Q_strncmp(STRING(gpGlobals->mapname), "inf_", 4) || of_infection.GetBool() )
	{
		...
		if( !DMMusicManager() )
		{
			CBaseEntity::CreateNoSpawn( "dm_music_manager", vec3_origin, vec3_angle );
			bCreated = true;
		}

		DMMusicManager()->SetRoundMusic( "InfectionMusic.Risen" );
		DMMusicManager()->SetWaitingMusic( "InfectionMusic.Warmup" );

		if( bCreated )
			DMMusicManager()->Spawn();
...
void CTFGameRules::PrecacheClass( int iClass )
{
	for( int iMutator = TF_CLASSMOD_NONE; iMutator < TF_CLASSMOD_LAST; iMutator++ )
	{
		switch( iMutator )
		{
			case TF_CLASSMOD_TFC:
			{
				if( !IsRetroModeEnabled() )
					continue;
				break;
			}
			case TF_CLASSMOD_ZOMBIE:
			{
				if( !IsInfGamemode() )
					continue;
				break;
		...
		TFPlayerClassData_t *pData = GetPlayerClassData( iClass, iMutator );

		const char *pszModel = pData->GetModelName();
		...
		const char *pszModelArm = pData->GetArmModelName();
		if ( pszModelArm && pszModelArm[0] )
		{
			CBaseEntity::PrecacheModel( pszModelArm );
		}
	
		CBaseEntity::PrecacheScriptSound( pData->m_szDeathSound );
		CBaseEntity::PrecacheScriptSound( pData->m_szCritDeathSound );
		CBaseEntity::PrecacheScriptSound( pData->m_szMeleeDeathSound );
		CBaseEntity::PrecacheScriptSound( pData->m_szExplosionDeathSound );
...
void CTFGameRules::PrecacheGameMode()
{
	...
	// undone the undone: that causes so much stuttering, so this has to be done - Unknown, probably ficool

	// TODO: Loading the items game directly like this seems a bit unreasonable
	// look into using the items game manager class directly - Kay
	...
	CBaseEntity::PrecacheModel(DM_SHIELD_MODEL);
	CBaseEntity::PrecacheModel(JETPACK_MODEL);
	...
	if ( TFGameRules() && TFGameRules()->IsBirthday() )
	{
		...
		CBaseEntity::PrecacheModel(BDAY_HAT_MODEL);
	}
}
...
bool CTFGameRules::DoesArenaCountLivesAndFrags(void)
{
	return of_lives_frags_score.GetBool();
}
...
void CTFGameRules::SetupOnRoundStart( void )
{
	...
	while ( pSpot )
	{
		...
		if ( pTFSpawn )
		{
			...
			if (pTFSpawn->GetTeamNumber() == TF_TEAM_RED)
				m_bHasRedSpawns = true;

			if (pTFSpawn->GetTeamNumber() == TF_TEAM_BLUE)
				m_bHasBlueSpawns = true;
		}
	...
	else
	{
		...
		{
			pWeaponSpawner = static_cast<CWeaponSpawner *>( IWeaponSpawnerAutoList::AutoList()[i] );
			pWeaponSpawner->SetDisabled( bIsDuel && pWeaponSpawner->IsSuperWeapon() ? true : false );
		}
	}
	...
	else
	{
		for ( int i = 0; i < ICondPowerupAutoList::AutoList().Count(); ++i )
		{
			...
			pPowerup->SetDisabled( ( !(bIsDuel) && pPowerup->m_iCondition == TF_COND_SHIELD_DUEL) ? true : false ); //duel only shield powerup removed in all other gamemodes
		}
	}	
	...
	if (TFGameRules()->IsArenaGamemode())
	{
		for (int i = 1; i <= gpGlobals->maxClients; i++)
		{
			...
			CTFPlayer *pPlayerLives = ToTFPlayer(pPlayer);

			if (pPlayer)
			{
				pPlayerLives->m_Shared.SetTFLives(of_lives.GetInt());
				pPlayerLives->SetLives(of_lives.GetInt());
			}
		}
	...
	if ( TFGameRules()->IsInfGamemode() )
	{
		...
		for ( int i = 1; i <= gpGlobals->maxClients; i++ )
		{
			...
			if ( pPlayer && pPlayer->GetTeamNumber() == TF_TEAM_BLUE )
			{
				...
				pPlayer->m_Shared.SetSurvivor( false );
				pPlayer->ChangeTeam( TF_TEAM_RED, true );
				if (!TFGameRules()->IsClassSelectAllowed(TF_TEAM_RED))
					pPlayer->SetDesiredPlayerClassIndex(GetForcedClassIndex(TF_TEAM_RED));
			}
		}
		...
		{
			CreateInfectionRoundTimer();

			// all teams
			for ( int i = FIRST_GAME_TEAM; i < GetNumberOfTeams(); i++ )
			{
				BroadcastSound( i, "InfectionMusic.Warmup", false );
				BroadcastSound( i, "InfRoundStart" );
			}
		}
	}
}

void CTFGameRules::CreateInfectionRoundTimer( void )
{
	if( GetInfectionRoundTimer() )
		UTIL_RemoveImmediate( GetInfectionRoundTimer() );

	// create the timer
	SetInfectionRoundTimer( ( CTeamRoundTimer* )CBaseEntity::CreateNoSpawn( "team_round_timer", vec3_origin, vec3_angle ) );

	if( GetInfectionRoundTimer() )
	{
		GetInfectionRoundTimer()->SetName( MAKE_STRING( "zz_infection_timer" ) );
		GetInfectionRoundTimer()->SetInfectionBeginning( true );

		GetInfectionRoundTimer()->SetShowInHud( true );
		GetInfectionRoundTimer()->ChangeTeam( TF_TEAM_RED );

		GetInfectionRoundTimer()->SetSetupTimeRemaining( of_infection_preparetime.GetFloat() );
		GetInfectionRoundTimer()->SetBaseTime( of_infection_roundtime.GetFloat() ); 

		GetInfectionRoundTimer()->Spawn();

		GetInfectionRoundTimer()->SetDisabled( false );
		GetInfectionRoundTimer()->ResumeTimer();
	}
}
...
//	float flHalfRadiusSqr = Square( flRadius / 2.0f );
	...
	for ( CEntitySphereQuery sphere( vecSrc, flRadius ); (pEntity = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity() )
	{
		...
		// Take damage handles checking if the player themselves are a teammate
		// However it doesnt check if the *projectile* is on their team
		// So do that here
		if( !CanEntityHitEntity( pInflictor, pEntity ) )
			continue;
		...
		float flInitialDamage = info.GetDamage();
		float flBlastJumpDamage = flInitialDamage;

		if( info.GetAttacker() == pEntity && pWeapon )
		{
			flInitialDamage = pWeapon->GetTFWpnData().m_nSelfDamage;
			flBlastJumpDamage = pWeapon->GetTFWpnData().m_nBlastJumpDamageForce;
		}

		if ( tf_fixedup_damage_radius.GetBool() )
		{
			flAdjustedDamage = RemapValClamped(flDistanceToEntity, 0, flRadius, flInitialDamage, flInitialDamage * falloff);
			flBlastJumpDamage = RemapValClamped(flDistanceToEntity, 0, flRadius, flBlastJumpDamage, flBlastJumpDamage * falloff);
		}
		else
		{
			flAdjustedDamage = flDistanceToEntity * falloff;
			
			flBlastJumpDamage = flBlastJumpDamage - flAdjustedDamage;
			
			flAdjustedDamage = flInitialDamage - flAdjustedDamage;			
		}
		...
		if( info.GetAttacker() == pEntity && pWeapon )
		{
			adjustedInfo.SetDamageForForceCalc( flBlastJumpDamage );
			float flFormerDamage = adjustedInfo.GetBaseDamage();
			adjustedInfo.SetDamage( adjustedInfo.GetDamageForForceCalc() );
			CalculateExplosiveDamageForce( &adjustedInfo, dir, vecSrc );
			adjustedInfo.SetDamage( flFormerDamage );		
		}
		...
		else
		{
			...
			if (pEntity)
			{
				...
				if ((pWeapon != NULL) && (pTFOther != NULL))
				{
					...
					if (GetEnemyTeam(pTFOther) == pInflictor->GetTeamNumber() && !(pTFOther->m_Shared.InCondUber()))
					{
					...
					if( pPlayer )
					{
						...
						for ( int iWeapon = 0; iWeapon < TF_WEAPON_COUNT; iWeapon++ )
						{
							...
							if ( pWeapon )
							{
								...
								if( hHandle )
								{
									CTFWeaponBase* pOverlappingWeapon = pPlayer->GetWeaponInSlot(pWeapon->GetSlot(), pWeapon->GetPosition());
									if( pOverlappingWeapon
										&& pOverlappingWeapon != hHandle )
									{
										pPlayer->DropWeapon( pOverlappingWeapon,
										true, false, 
										pOverlappingWeapon->Clip1(),
										pOverlappingWeapon->ReserveAmmo() );
										UTIL_Remove( pOverlappingWeapon );
									}
									pPlayer->SetWeaponInSlot( hHandle, pWeapon->GetSlot(), pWeapon->GetPosition() );
								}
							}
						}
					}
				...
				if ( CountActivePlayers() > 1 && State_Get() == GR_STATE_RND_RUNNING )
				{
					...
					for ( int i = LAST_SHARED_TEAM+1; i < GetNumberOfTeams(); i++ )
					{
						...
						if ( iPlayers )
						{
							...
							{
								if (pTeam->GetPlayer(player) && (pTeam->GetPlayer(player)->IsAlive() || (pTeam->GetPlayer(player)->Lives() > 0)))
								{
									bFoundLiveOne++;
				...
				for ( int iTeam = LAST_SHARED_TEAM+1; iTeam < GetNumberOfTeams(); iTeam++ )
				{
					...
					if ( iPlayers )
					{
						...
						for ( int player = 0; player < iPlayers; player++ )
						{
							...
							if( pCiv && pCiv->IsConnected() )
							{
								if( pCiv->IsPlayerClass( TF_CLASS_CIVILIAN ) )
								{
									iFoundCivs++;
									if( iFoundCivs > GetMaxHunted( iTeam ) && !pCiv->IsClassAllowed( TF_CLASS_CIVILIAN ) )
									{
										pCiv->SetClass( TF_CLASS_RANDOM );
						...
						if ( GetMaxHunted( iTeam ) <= 0 )
							continue;

						int iDesiredHuntedCount = GetMaxHunted( iTeam );
						if( iPlayers < GetMaxHunted( iTeam ) )
							iDesiredHuntedCount = iPlayers;

						while( iFoundCivs < iDesiredHuntedCount )
						{
							int iPlayerIndex;
							...
							if( pTeam->GetPlayer(iPlayerIndex) && pTeam->GetPlayer(iPlayerIndex)->IsConnected() )
							{
								...
								{
									// Skips all the checks if the class is allowed
...
bool CTFGameRules::IsSpawnPointValid( CBaseEntity *pSpot, CBasePlayer *pPlayer, bool bIgnorePlayers, bool bForgiving )
{
	...
	if(	!pTFPlayer->m_Shared.IsZombie() &&	// Zombies can spawn anywhere
		...
		(pSpot->GetTeamNumber() != TF_TEAM_MERCENARY) &&  // Neutral spawns if there's red or blue spawns
		pSpot->GetTeamNumber() != pPlayer->GetTeamNumber() )
		return false;

	if ((pSpot->GetTeamNumber() == TF_TEAM_MERCENARY) && (TFGameRules()->HasRedSpawns() && TFGameRules()->HasBlueSpawns()) && TFGameRules()->IsTeamplay())
		return false;
...
void CTFGameRules::ClientSettingsChanged( CBasePlayer *pPlayer )
{
	...
	pTFPlayer->SetJumpSoundOption( Q_atoi( engine->GetClientConVarValue( pPlayer->entindex(), "of_jumpsound" ) ) );
	
	pTFPlayer->SetWaitForRespawn( Q_atoi( engine->GetClientConVarValue( pPlayer->entindex(), "of_wait_for_respawn" ) ) > 0 );

	// First person/Viewmodel stuff
...
{
#ifdef FIXED_CLIENT_KVS_BEING_BROKEN
	...
	else if ( Q_strcasecmp( pKeyValues->GetName(), "LoadoutInfo" ) == 0 )
	{
		//uint64 nID = LoadoutManager()->GetPlayerID( pPlayer );
		//Assert( nID != 0 );

		COFPlayerLoadout *pLoadout = LoadoutManager()->GetLoadout( pPlayer );
		if( !pLoadout )
		{
			LoadoutManager()->RegisterPlayer(pPlayer);

			pLoadout = LoadoutManager()->GetLoadout( pPlayer );
		}
		
		Assert( pLoadout );

		for ( int iClass = TF_CLASS_UNDEFINED+1; iClass < TF_CLASS_COUNT_ALL; ++iClass )
		{
			KeyValues *pCosmetics = pKeyValues->FindKey( "cosmetics" );
			if( pCosmetics )
			{
				KeyValues *pClass = pCosmetics->FindKey(g_aPlayerClassNames_NonLocalized[iClass]);
				if( pClass )
				{
					FOR_EACH_VALUE( pClass, pValue )
					{
						const char *szCategory = pValue->GetName();
						// Don't allow spoofed categories to function
						if( !GetItemSchema()->CategoryExists(szCategory) )
							return;
						...
						if( handle == pLoadout->m_ClassLoadout[iClass].m_Cosmetics.InvalidIndex() )
							pLoadout->m_ClassLoadout[iClass].m_Cosmetics.Insert( pValue->GetName(), pValue->GetFloat() );
						else
							pLoadout->m_ClassLoadout[iClass].m_Cosmetics[handle] = pValue->GetFloat();
					}
				}
			}

			KeyValues *pWeapons = pKeyValues->FindKey( "weapons" );
			if( pWeapons )
			{
				...
				if( pClass )
				{
					for( int i = 0; i < 3; i++ )
					{
						const char *szWeapon = pClass->GetString( UTIL_VarArgs("%d", i+1), NULL );
						ItemsGameHandle iWeaponID = GetItemSchema()->GetWeaponID(szWeapon);
						if( !GetItemSchema()->WeaponIDExists(iWeaponID) )
							continue;

						if( i < pLoadout->m_ClassLoadout[iClass].m_Weapons.Count() )
							pLoadout->m_ClassLoadout[iClass].m_Weapons[i] = iWeaponID;
						else
							pLoadout->m_ClassLoadout[iClass].m_Weapons.AddToTail( iWeaponID );
					}
				}
			}
		}
...
void CTFGameRules::PlayerKilled( CBasePlayer *pVictim, const CTakeDamageInfo &info )
{
	...
	// DM Kill stuff here
	// Don't count any Kill stuff for DM during waiting for players or setup
	if( IsInWaitingForPlayers() || InSetup() )
		return;
	
	if( IsGGGamemode() || (IsDMGamemode() && !DontCountKills()) )
	{
		int iScoreLimit = IsGGGamemode() ? m_iMaxLevel : fraglimit.GetInt();
		
		if( iScoreLimit > 0 ) 
		{
			if( IsTDMGamemode() && !IsGGGamemode() )
			{
				int iRedScore = TFTeamMgr()->GetTeam(TF_TEAM_RED)->GetScore();
				int iBluScore = TFTeamMgr()->GetTeam(TF_TEAM_BLUE)->GetScore();

				if( m_nCurrFrags < iRedScore )
				{
					m_nCurrFrags = iRedScore;
					FireTargets( "game_fragincrease", TFTeamMgr()->GetTeam(TF_TEAM_RED), TFTeamMgr()->GetTeam(TF_TEAM_RED), USE_TOGGLE, 0 );
				}
				else if( m_nCurrFrags < iBluScore )
				{
					m_nCurrFrags = iBluScore;
					FireTargets( "game_fragincrease", TFTeamMgr()->GetTeam(TF_TEAM_BLUE), TFTeamMgr()->GetTeam(TF_TEAM_BLUE), USE_TOGGLE, 0 );
				}			

				if( iRedScore >= iScoreLimit || iBluScore >= iScoreLimit )
				{
					SendTeamScoresEvent();
					GoToIntermission();
				}
				...
				if ( of_votenearend.GetBool() && !m_bStartedVote && ( iRedScore >= ( (float)iScoreLimit * 0.8 ) ||
					( iBluScore >= ( (float)iScoreLimit * 0.8 ) ) ) 
			...
			{
				int iScore = IsGGGamemode() ? pTFPlayerScorer->GGLevel() : pTFPlayerScorer->FragCount();

				if ( m_nCurrFrags < iScore )
				{
					m_nCurrFrags = iScore;
					FireTargets( "game_fragincrease", pTFPlayerScorer, pTFPlayerScorer, USE_TOGGLE, 0 );

					// We only support 10 and 5->1 but this is faster
					// and just supports more stuff if custom announcers want it
					if( !IsGGGamemode() && iScoreLimit - m_nCurrFrags <= 10 )
						BroadcastSound( TEAM_UNASSIGNED, UTIL_VarArgs( "FragsLeft%d", iScoreLimit - m_nCurrFrags ) );
				}

				if ( iScore >= iScoreLimit )
				{
					if( IsDuelGamemode() )
					...
					// TDM Gun game
					if( IsTDMGamemode() )
						SetWinningTeam( pTFPlayerScorer->GetTeamNumber(), 0, true, true );
					else
						SetWinningTeam( TF_TEAM_MERCENARY, WINREASON_POINTLIMIT, true, true, false);
				}

				CalcFFALead( pTFPlayerScorer );

				// one of our players is at 80% of the fragcount, start voting for next map
				if( of_votenearend.GetBool() && !m_bStartedVote
					&& m_nRoundsPlayed >= mp_maxrounds.GetInt() - 1 
					&& ( iScore >= ( (float)iScoreLimit * 0.8 ) )
...
void CTFGameRules::CalcFFALead( CTFPlayer *pTFPlayerScorer )
{
	CTFPlayer *pLead = ToTFPlayer( UTIL_PlayerByIndex( m_iLastLead ) );

	int iLeadScore = pLead ? (IsGGGamemode() ? pLead->GGLevel() : pLead->FragCount()) : -1;
	int iScore = IsGGGamemode() ? pTFPlayerScorer->GGLevel() : pTFPlayerScorer->FragCount();

	int iLead = iLeadScore < iScore ? pTFPlayerScorer->entindex() : m_iLastLead;
	// If the current scorer just overtook the lead
	if( iLead != m_iLastLead )
	{
		TeamplayRoundBasedRules()->BroadcastSoundFFA( pTFPlayerScorer->entindex(), "DM.LeadTaken" );
		TeamplayRoundBasedRules()->BroadcastSoundFFA( m_iLastLead, "DM.LeadLost" );
	}
	// Or if the killers score equals the lead score
	// AND the killer isnt the lead ( so the lead doesnt tie with themselves )
	else if( iLeadScore == iScore && m_iLastLead != pTFPlayerScorer->entindex() )
	{
		TeamplayRoundBasedRules()->BroadcastSoundFFA( pTFPlayerScorer->entindex(), "DM.LeadTied" );
		TeamplayRoundBasedRules()->BroadcastSoundFFA( m_iLastLead, "DM.LeadTied" );
	}
	m_iLastLead = iLead;
}

void CTFGameRules::CalcTDMLead( void )
{
	int iLead = -1;
	int iLeadScore = -1;
	int iSecond = -1;
	...
	for( int i = TF_TEAM_RED; i < TF_TEAM_COUNT; i++ )
	{
		...
		if( pTeam )
		{
			if( pTeam->GetScore() > iLeadScore )
			{
				iSecond = iLead;
				iSecondScore = iLeadScore;
				iLead = i;
				iLeadScore = pTeam->GetScore();
			}
			else if( pTeam->GetScore() > iSecondScore )
			{
				iSecond = i;
				iSecondScore = pTeam->GetScore();
			}
		}
	}

	// If the current scorer just overtook the lead
	if( iLeadScore > iSecondScore && m_iLastLead != iLead )
	{
		TeamplayRoundBasedRules()->BroadcastSound( iLead, "DM.YourTeamTookLead" );

		// Can't really lose lead when you dont have any points
		if( iSecondScore > 0 )
			TeamplayRoundBasedRules()->BroadcastSound( iSecond, "DM.YourTeamLostLead" );

		m_iLastLead = iLead;
	}
	// Or if the killers score equals the lead score
	// AND the killer isnt the lead ( so the lead doesnt tie with themselves )
	else if( iLeadScore == iSecondScore && m_iLastLead != (1 << iLead | 1 << iSecond) )
	{
		TeamplayRoundBasedRules()->BroadcastSound( iLead, "DM.YourTeamTiedLead" );
		TeamplayRoundBasedRules()->BroadcastSound( iSecond, "DM.YourTeamTiedLead" );

		m_iLastLead = 1 << iLead | 1 << iSecond;
	}
}
...
const char *CTFGameRules::GetKillingWeaponName( const CTakeDamageInfo &info, CTFPlayer *pVictim, int *weaponType )
{
	...
	else if ( pScorer && pInflictor && ( pInflictor == pScorer ) )
	{
		...
		if (pWeapon)
		{
			...
			{
				if (!(pWeapon->GetTFWpnData().m_bHumilationDenial))
				{
					*weaponType = 1;
				}
			}
...
void CTFGameRules::DeathNotice(CBasePlayer *pVictim, const CTakeDamageInfo &info)
{
	...
	if (event)
	{
		...
		if (!IsInWaitingForPlayers() && State_Get() > GR_STATE_PREGAME)
		{
			...
			if (info.GetDamageType() & DMG_BLAST) //inflictor is an explosive projectile
			{
				//Midair
...
bool CTFGameRules::PlayVoiceCommand(CBaseMultiplayerPlayer* pPlayer, const SVoiceCommand& voiceCommand, bool bIsCalledFromBind) const
{
	if (!BaseClass::PlayVoiceCommand(pPlayer, voiceCommand, bIsCalledFromBind))
	{
		return false;
	}

	int iActivity = ActivityList_IndexForName(voiceCommand.m_GestureActivity);
	CTFPlayer* pTFPlayer = ToTFPlayer(pPlayer);
	...
	if (iActivity != ACT_INVALID && pTFPlayer)
	{
		...
		// While it can be other voiceline, it still is potentially spammable - so keep the timer
		if (bIsCalledFromBind)
		{
...
#ifdef GAME_DLL
void CTFGameRules::OnTeamLostPlayer( int iTeamNum, CBasePlayer *pPlayer )
{
	// If RED is dead, BLU wins, if BLU is dead
	if( IsInfGamemode() && !InSetup() && !IsInWaitingForPlayers() && m_iRoundState == GR_STATE_RND_RUNNING )
	{
		...
		if( pTeam )
		{
			...
			{
				SetWinningTeam( 
					iTeamNum == TF_TEAM_RED ? TF_TEAM_BLUE : TF_TEAM_RED,
					WINREASON_OPPONENTS_DEAD, m_bForceMapReset );
			}
			else if( iTeamNum == TF_TEAM_RED )
			{
				// one guy left alive? give him haste
				if( iPlayers == 1 && !m_bInfectionLastManAlive )
				{
					CTFPlayer* pTFPlayer = ToTFPlayer( pTeam->GetPlayer( 0 ) );
					// The only alive player should be the only player left
					if( pTFPlayer )
					{
						...
						pTFPlayer->m_Shared.SetSurvivor( true );

						if( DMMusicManager() )
							DMMusicManager()->SetDisabled( true );
									
						IGameEvent * event = gameeventmanager->CreateEvent( "infection_lms" );
						if ( event )
						{
							gameeventmanager->FireEvent( event );
						}
...
{
	CTFPlayer *pPlayer = ToTFPlayer( GetContainingEntity( pEntity ) );
	if ( pPlayer )
	{
		pPlayer->TeamFortress_ClientConnected();
	}
...
float CTFGameRules::FlPlayerFallDamage( CBasePlayer *pPlayer )
{
	if ( pPlayer->m_Local.m_flFallVelocity > TF_PLAYER_MAX_SAFE_FALL_SPEED )
	{
		...
		if (!of_fix_random_fall_damage.GetBool())
		{
...
void CTFGameRules::SendWinPanelInfo( void )
{
	...
	if ( winEvent )
	{
		...
		for( iPlayerIndex = 1 ; iPlayerIndex <= MAX_PLAYERS; iPlayerIndex++ )
		{
			...
			if ( pStats )
			{
				if ( TFGameRules()->IsDMGamemode() && !TFGameRules()->DontCountKills()  && !TFGameRules()->IsArenaGamemode())
				{
						iRoundScore = iTotalScore = pTFPlayer->FragCount();
				}
				else if (TFGameRules()->IsArenaGamemode())
				{
					if (of_lives_frags_score.GetBool())
					{
						iRoundScore = iTotalScore = ((pTFPlayer->Lives() * of_arena_lives_multiplier.GetInt()) + (pTFPlayer->FragCount() * of_arena_frags_multiplier.GetInt()));
					}
					else
					{
						iRoundScore = iTotalScore = pTFPlayer->Lives();
					}
				}
				else
				{
...
void CTFGameRules::RoundRespawn( void )
{
	...
	for( int i = 0; i < gpGlobals->maxClients; i++ )
	{
		...
		if( pPlayer->GetBotType() != BOT_TYPE_LOCKDOWN )
			continue;
...
//-----------------------------------------------------------------------------
// Purpose: Returns gametype ID used for voice command menu set selection
//-----------------------------------------------------------------------------
int CTFGameRules::ActiveVoiceCommandSetId() const
{
	return m_nGameType.Get();
}

#if defined( OF_DLL ) || defined ( OF_CLIENT_DLL )
//---------------------------------------------------------------
// Purpoise: Takes a weapon name and player and returns which slot the weapon is supposed to be in
//---------------------------------------------------------------
void CTFGameRules::GetWeaponSlot( char *szName, int &iSlot, int &iPos, CTFPlayer *pPlayer )
{
	if( LookupWeaponInfoSlot(szName) == GetInvalidWeaponInfoHandle() )
		return;

	GetWeaponSlot( LookupWeaponInfoSlot(szName), iSlot, iPos, pPlayer );
}

//---------------------------------------------------------------
// Purpoise: Takes a weapon handle and player and returns which slot the weapon is supposed to be in
//---------------------------------------------------------------
void CTFGameRules::GetWeaponSlot( WEAPON_FILE_INFO_HANDLE pHandle, int &iSlot, int &iPos, CTFPlayer *pPlayer )
{
	if( pHandle == GetInvalidWeaponInfoHandle() )
		return;

	CTFWeaponInfo *pWeaponInfo = static_cast<CTFWeaponInfo*>( GetFileWeaponInfoFromHandle(pHandle) );

	ETFInventoryType nInvType = GetPlayerInventorySystem( pPlayer );
	...
	switch( nInvType )
	{
		...
		case OF_INVENTORY_DEFAULT:
			if( iClass != -1 && pWeaponInfo->m_iClassSlot[iClass] != -1 )
				iSlot = pWeaponInfo->m_iClassSlot[iClass];
			else
				iSlot = pWeaponInfo->iSlot;

			iPos = pWeaponInfo->iPosition;
			break;
		...
		case OF_INVENTORY_SLOTS:
			...
			if( pPlayer )
			{
				iPos = 0;
				iSlot = 1;
				CTFWeaponBase *pWeapon = pPlayer->GetWeaponInSlot( 1, 0 );
				while( pWeapon != NULL && iSlot < of_weapon_slot_count.GetInt() - 1 )
				{
					iSlot = pWeapon->GetSlot() + 1;
					pWeapon = pPlayer->GetWeaponInSlot( iSlot, 0 );
				}

				if( pWeapon )
				{
					iSlot = pPlayer->GetActiveWeapon()->GetSlot();
					iPos = 0;
				}
			}
#endif
...
ETFInventoryType CTFGameRules::GetPlayerInventorySystem( CTFPlayer *pPlayer )
{
	if( TFGameRules() && TFGameRules()->UsesDMBuckets() && !TFGameRules()->IsGGGamemode() )
	{
		int iIndex = pPlayer->GetPlayerClass()->GetClassIndex();

		if( of_allow_allclass_spawners.GetBool() || iIndex == TF_CLASS_MERCENARY )
			return of_multiweapons.GetInt() == 2 && of_weapon_slot_count.GetBool() ? OF_INVENTORY_SLOTS : OF_INVENTORY_MULTI;
	}

	return OF_INVENTORY_DEFAULT;
	
}

int CTFGameRules::GetPlayerClassMod( CTFPlayer *pPlayer )
{
	if( pPlayer->m_Shared.IsZombie() )
		return TF_CLASSMOD_ZOMBIE;

	if( pPlayer->IsRetroModeOn() )
		return TF_CLASSMOD_TFC;

	return GetTeamClassMod( pPlayer->GetTeamNumber() );
}

int CTFGameRules::GetTeamClassMod( int iTeam )
{
	if( IsInfGamemode() && iTeam == TF_TEAM_BLUE )
		return TF_CLASSMOD_ZOMBIE;

	if(		( TFGameRules()->IsRetroMode( RETROMODE_ON )  )
		||	( TFGameRules()->IsRetroMode( RETROMODE_BLUE_ONLY ) && iTeam == TF_TEAM_BLUE ) 
		||	( TFGameRules()->IsRetroMode( RETROMODE_RED_ONLY ) && iTeam == TF_TEAM_RED) )
		return TF_CLASSMOD_TFC;

	return TF_CLASSMOD_NONE;
}

int CTFGameRules::PlayerRelationship( CBaseEntity* pPlayer, CBaseEntity *pTarget, bool bCheckDisguise )
{
	// If either is on merc team, only consider them teammates if the entity is owned by the player
	if( pPlayer->GetTeamNumber() == TF_TEAM_MERCENARY 
		|| pTarget->GetTeamNumber() == TF_TEAM_MERCENARY )
		return pTarget->GetOwnerEntity() == pPlayer ? GR_TEAMMATE : GR_ENEMY;
	
	if( bCheckDisguise && pTarget->IsPlayer() )
	{
		CTFPlayer *pSpy = ToTFPlayer( pTarget );
		if( pSpy->m_Shared.InCond( TF_COND_DISGUISED ) && pPlayer->GetTeamNumber() != pTarget->GetTeamNumber())
		{
			if( pPlayer->GetTeamNumber() == pSpy->m_Shared.GetDisguiseTeam() )
				return GR_TEAMMATE;
		}
	}
...
bool IsSameEntityOrOwnedByEntity( CBaseEntity *pEntity, CBaseEntity *pTarget )
{
	while( pEntity )
	{
		if( pEntity == pTarget )
...
bool CTFGameRules::CanEntityHitEntity( CBaseEntity *pPlayer, CBaseEntity *pTarget )
{
	if( IsSameEntityOrOwnedByEntity( pPlayer, pTarget )  )
		return true;

	if( pPlayer->GetOwnerEntity() && !CanEntityHitEntity( pPlayer->GetOwnerEntity(), pTarget ) )
		return false;

	if( PlayerRelationship( pPlayer, pTarget ) == GR_TEAMMATE )
		return friendlyfire.GetInt();
...
void CTFGameRules::FireGameEvent( IGameEvent *event )
{
	...
	else if( !Q_strcmp( eventName, "infection_lms" ) )
	{
		SoundData_t *pAnnouncerSound = ClientModeShared::PlayTeamAnnouncerLine( "Inf_LastManAlive", TF_TEAM_RED, true );

		float flDelay = pAnnouncerSound ? (float)FMODManager()->GetSoundLength( pAnnouncerSound->pBaseFile ) : 0;

		int iSeed = CBaseEntity::GetPredictionRandomSeed() & 255;

		COFSoundParameters *pBase = OFSoundManifest()->GetSoundscript( "InfectionMusic.LastMercStanding" );
		if( pBase )
		{
			COFSoundParameters* pIntro = OFSoundManifest()->GetSoundscript( pBase->GetIntro() );

			if( pIntro )
			{
				RandomSeed(iSeed);
				const char *szIntroWav = pIntro->GetRandomWav();
				while( szIntroWav[0] == '#' || szIntroWav[0] == ')' )
					szIntroWav++;

				flDelay -= (float)FMODManager()->GetSoundLength(szIntroWav);
			}
		}

		flDelay /= 1000.0f;

		flDelay = max(0, flDelay);

		RandomSeed(iSeed);
		SoundData_t *pSound = FMODManager()->PlaySound( "InfectionMusic.LastMercStanding", OF_CHANNEL_MUSIC, flDelay );
...
bool CTFGameRules::HasPassedMinRespawnTime( CBasePlayer *pPlayer )
{
	...
	if (TFGameRules()->IsArenaGamemode() && pTFPlayer->m_Shared.GetTFLives() <= 0)
		return false;
...
int CTFGameRules::GetClassLimit(int iTeamNum, int iClassIndex)
{
	ConVar* cvClassLimit;

	// Always restrict the civilian class in the escort gamemode
	if (iClassIndex == TF_CLASS_CIVILIAN && IsESCGamemode())
		return 0;

	switch (iClassIndex)
	{
	case TF_CLASS_SCOUT:
		cvClassLimit = &of_classlimit_scout;
		break;
	case TF_CLASS_SOLDIER:
		cvClassLimit = &of_classlimit_soldier;
		break;
	case TF_CLASS_PYRO:
		cvClassLimit = &of_classlimit_pyro;
		break;
	case TF_CLASS_DEMOMAN:
		cvClassLimit = &of_classlimit_demoman;
		break;
	case TF_CLASS_HEAVYWEAPONS:
		cvClassLimit = &of_classlimit_heavy;
		break;
	case TF_CLASS_ENGINEER:
		cvClassLimit = &of_classlimit_engineer;
		break;
	case TF_CLASS_MEDIC:
		cvClassLimit = &of_classlimit_medic;
		break;
	case TF_CLASS_SNIPER:
		cvClassLimit = &of_classlimit_sniper;
		break;
	case TF_CLASS_SPY:
		cvClassLimit = &of_classlimit_spy;
		break;
	case TF_CLASS_MERCENARY:
		cvClassLimit = &of_classlimit_mercenary;
		break;
	case TF_CLASS_CIVILIAN:
		cvClassLimit = &of_classlimit_civilian;
		break;
	case TF_CLASS_JUGGERNAUT:
		cvClassLimit = &of_classlimit_juggernaut;
		break;
	default:
		return -1;
	}

	std::string strClassLimit = fmt::format(FMT_STRING("{:s}"), cvClassLimit->GetString());
	std::vector<std::string> vecClassLimits = UTIL_SplitSTDString(strClassLimit, ",");

	int iClassLimitAny = -1;
	int iClassLimitBlue = -999;
	int iClassLimitRed = -999;

	for (size_t i = 0; i < vecClassLimits.size(); i++)
	{
		std::string strLimit = vecClassLimits[i];
		...
		int iLimit = atoi(strLimit.c_str());

		switch (i)
		{
		case 0:
			iClassLimitAny = iLimit;
			break;
		case 1:
			iClassLimitBlue = iLimit;
			break;
		case 2:
			iClassLimitRed = iLimit;
			break;
		}
	}
	...
	if (iTeamNum == TF_TEAM_BLUE && iClassLimitBlue != -999)
	{
		iClassLimit = iClassLimitBlue;
	}
	else if (iTeamNum == TF_TEAM_RED && iClassLimitRed != -999)
	{
		iClassLimit = iClassLimitRed;
	}
	else
	{
		iClassLimit = iClassLimitAny;
	}

	return iClassLimit;
}

bool CTFGameRules::IsClassSelectAllowed(int iTeamNum)
{
	return GetForcedClassIndex(iTeamNum) == -1;
}

#ifdef GAME_DLL
bool CTFGameRules::IsClassAllowed( int iTeamNum, int iClassIndex )
{
	if (iClassIndex == TF_CLASS_RANDOM)
		return true;

	int iClassLimit = GetClassLimit(iTeamNum, iClassIndex);
	...
	for (int i = 0; i < pTeam->GetNumPlayers(); i++)
	{
		...
		if (pTeamPlayer && pTeamPlayer->GetPlayerClass()->GetClassIndex() == iClassIndex)
		{
			iClassCount++;

			if (iClassCount >= iClassLimit)
			{
				return false;
			}
		}
...
int CTFGameRules::GetForcedClassIndex(int iTeamNum)
{
	int iForcedClass = TF_CLASS_RANDOM;

	for (int i = TF_FIRST_NORMAL_CLASS; i < TF_CLASS_COUNT_ALL; i++)
	{
		int iClassLimit = TFGameRules()->GetClassLimit(iTeamNum, i);

		if (iClassLimit == -1 || iClassLimit > 0)
		{
			// If another class was already allowed, then we're not forcing a class
			if (iForcedClass != TF_CLASS_RANDOM)
			{
				return -1;
			}
	...
	// If the forced class is random (ie. all classes are blocked), pick a random (normal) class index and return it
	if (iForcedClass == TF_CLASS_RANDOM)
		return random->RandomInt(TF_FIRST_NORMAL_CLASS, TF_LAST_NORMAL_CLASS);
...
void CTFGameRules::BeginInfection( void )
{
	...
	// all teams
	BroadcastSound( TEAM_UNASSIGNED, "InfRoundStart" );
	BroadcastSound( TEAM_UNASSIGNED, "InfectionMusic.Begin", false );
}
...
void CTFGameRules::SelectInfector( void )
{
	...
	// This should never happen, since we force Waiting for players when under 2 players
	// However, in some edge case this *does* happen, reset ourselves
	if ( targets <= 0 )
	{
