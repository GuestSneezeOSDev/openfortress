...
void CTFViewModel::CalcViewModelView( CBasePlayer *owner, const Vector& eyePosition, const QAngle& eyeAngles )
{
	...
	if( pWeapon )
	{
		...
		if (bMinimized && bCentered)
		{
			x += pWeapon->GetTFWpnData().m_flMiniCenteredViewmodelOffsetX;
			y += pWeapon->GetTFWpnData().m_flMiniCenteredViewmodelOffsetY;
			z += pWeapon->GetTFWpnData().m_flMiniCenteredViewmodelOffsetZ;

			pitch += pWeapon->GetTFWpnData().m_flMiniCenteredViewmodelAngleX;
			yaw += pWeapon->GetTFWpnData().m_flMiniCenteredViewmodelAngleY;
			roll += pWeapon->GetTFWpnData().m_flMiniCenteredViewmodelAngleZ;
		}
	}
...
int CTFViewModel::DrawModel( int flags )
{
	...
	if ( pViewModelArms )
	{
		if ( viewmodel_hide_arms.GetBool() && !pViewModelArms->IsEffectActive( EF_NODRAW ) )
		{
			pViewModelArms->AddEffects( EF_NODRAW );
		}
		else if( pViewModelArms->IsEffectActive( EF_NODRAW ) )
		{
	...
	{
		if ( viewmodel_hide_arms.GetBool() && !pViewModelSleeves->IsEffectActive( EF_NODRAW ) )
		{
			pViewModelSleeves->AddEffects( EF_NODRAW );
		}
		else if( pViewModelSleeves->IsEffectActive( EF_NODRAW ) )
		{
