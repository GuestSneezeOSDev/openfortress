...
ConVar of_cs_blast_radius("of_cs_blast_radius", "1024", FCVAR_REPLICATED);
ConVar of_cs_bomb_time("of_cs_bomb_time", "45", FCVAR_REPLICATED);
ConVar of_cs_bomb_damage("of_cs_bomb_damage", "400", FCVAR_REPLICATED);
...
void CTFFlag::PlantBomb()
{
	...
	if( pOwner )
	{
		...
		hHandle = this;	
		if ( pOwner->GetWeaponInSlot( GetSlot(), GetPosition() ) == hHandle )
			pOwner->SetWeaponInSlot( NULL, GetSlot(), GetPosition());
#endif
	}
