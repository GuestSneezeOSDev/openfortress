...
IMPLEMENT_NETWORKCLASS_ALIASED(TFFlameThrowerDM, DT_WeaponFlameThrowerDM)

BEGIN_NETWORK_TABLE(CTFFlameThrowerDM, DT_WeaponFlameThrowerDM)
#if defined( CLIENT_DLL )
	RecvPropInt(RECVINFO(m_iWeaponState)),
	RecvPropInt(RECVINFO(m_bCritFire)),
#else
...
#if defined( CLIENT_DLL )
BEGIN_PREDICTION_DATA(CTFFlameThrowerDM)
	DEFINE_PRED_FIELD(m_iWeaponState, FIELD_INTEGER, FTYPEDESC_INSENDTABLE),
	DEFINE_PRED_FIELD(m_bCritFire, FIELD_INTEGER, FTYPEDESC_INSENDTABLE),
END_PREDICTION_DATA()
#endif

LINK_ENTITY_TO_CLASS(tf_weapon_flamethrower_mercenary, CTFFlameThrowerDM);
//PRECACHE_WEAPON_REGISTER( tf_weapon_flamethrower_mercenary );

BEGIN_DATADESC(CTFFlameThrowerDM)
END_DATADESC()

IMPLEMENT_NETWORKCLASS_ALIASED(TFFlameThrower_Retro, DT_WeaponFlameThrower_Retro)

BEGIN_NETWORK_TABLE(CTFFlameThrower_Retro, DT_WeaponFlameThrower_Retro)
#if defined( CLIENT_DLL )
RecvPropInt(RECVINFO(m_iWeaponState)),
RecvPropInt(RECVINFO(m_bCritFire)),
#else
...
#if defined( CLIENT_DLL )
BEGIN_PREDICTION_DATA(CTFFlameThrower_Retro)
DEFINE_PRED_FIELD(m_iWeaponState, FIELD_INTEGER, FTYPEDESC_INSENDTABLE),
DEFINE_PRED_FIELD(m_bCritFire, FIELD_INTEGER, FTYPEDESC_INSENDTABLE),
END_PREDICTION_DATA()
#endif

LINK_ENTITY_TO_CLASS(tfc_weapon_flamethrower, CTFFlameThrower_Retro);
//PRECACHE_WEAPON_REGISTER( tfc_weapon_flamethrower );

BEGIN_DATADESC(CTFFlameThrower_Retro)
END_DATADESC()
...
void CTFFlameThrower::WeaponSound( WeaponSound_t sound_type, float soundtime )
{
...
void CTFFlameThrower::ItemPostFrame()
{
	...
	if (
		CanPerformSecondaryAttack() && pOwner->IsAlive() && ((pOwner->m_nButtons & IN_ATTACK2)
		&& (iAmmo >= (GetTFWpnData().m_WeaponData[TF_WEAPON_SECONDARY_MODE].m_iAmmoPerShot)))
		)
	{
	...
	else if (pOwner->IsAlive() && ((pOwner->m_nButtons & IN_ATTACK) && iAmmo > 0) )
	{
		PrimaryAttack();
	}
	
	if ( m_iWeaponState > FT_STATE_IDLE && !(pOwner->m_nButtons & IN_ATTACK))
	{
...
void CTFFlameThrower::PrimaryAttack()
{
	...
	case FT_STATE_IDLE:
		{
			...
			//DevMsg("FT_STATE_IDLE to FT_STATE_STARTFIRING. DoAnimationEvent(PLAYERANIMEVENT_ATTACK_PRE)\n");
	...
	case FT_STATE_FIRING:
		{
			if ( gpGlobals->curtime >= m_flNextPrimaryAttackAnim )
			{
				...
				//DevMsg("FT_STATE_FIRING. DoAnimationEvent(PLAYERANIMEVENT_ATTACK_PRIMARY)\n");
	...
	m_flAmmoUseRemainder += (GetTFWpnData().m_WeaponData[TF_WEAPON_PRIMARY_MODE].m_iAmmoPerShot) * flAmmoRate;
	...
	pOwner->m_Shared.RemoveCond( TF_COND_SPAWNPROTECT );
#endif
...
void CTFFlameThrower::SecondaryAttack()
{
	...
	if ( !CanPerformSecondaryAttack() && CanSecondaryAttack() )
	{
		m_iWeaponState = FT_STATE_IDLE;
		return;
	}
...
{
	//DevMsg("Stop Flame! UpdateOnRemove\n");
	StopFlame();
	StopPilotLight();
...
{
	if( !TFGameRules()->CanEntityHitEntity(this, pOther) )
		return;
...
acttable_t CTFFlameThrowerDM::m_acttableFlameThrower[] =
{
	...
	{ ACT_MP_ATTACK_STAND_PREFIRE, ACT_MERC_FLAMETHROWER_ATTACK_STAND_PREFIRE, false },
	{ ACT_MP_ATTACK_STAND_POSTFIRE, ACT_MERC_FLAMETHROWER_ATTACK_STAND_POSTFIRE, false },
	{ ACT_MP_ATTACK_STAND_STARTFIRE, ACT_MERC_FLAMETHROWER_ATTACK_STAND_STARTFIRE, false },
	{ ACT_MP_ATTACK_CROUCH_PREFIRE, ACT_MERC_FLAMETHROWER_ATTACK_CROUCH_PREFIRE, false },
	{ ACT_MP_ATTACK_CROUCH_POSTFIRE, ACT_MERC_FLAMETHROWER_ATTACK_CROUCH_POSTFIRE, false },
	{ ACT_MP_ATTACK_SWIM_PREFIRE, ACT_MERC_FLAMETHROWER_ATTACK_SWIM_PREFIRE, false },
	{ ACT_MP_ATTACK_SWIM_POSTFIRE, ACT_MERC_FLAMETHROWER_ATTACK_SWIM_POSTFIRE, false },
};

//Act table remapping for Merc
acttable_t *CTFFlameThrowerDM::ActivityList(int &iActivityCount)
{
	if( !GetTFPlayerOwner() )
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFFlameThrowerDM::SecondaryAttack()
{
