									...
									CBaseCombatCharacter *pOwner, const CTFWeaponInfo &weaponInfo, CTFWeaponBase *pWeapon )
{
	...
	// UNDONE: Needed for proper self team checks, and in TF2 we can't shoot our own grenades anyways
	...
	if( pWeapon )
	{
		TFExplosion_Visuals_t visuals = pWeapon->GetExplosionVisualInfo();
		SetExplosionVisualInfo( &visuals );

		visuals = pWeapon->GetBombletExplosionVisualInfo();
		SetBombletExplosionVisualInfo( &visuals );

		SetDamageRadius( pWeapon->GetDamageRadius() );
	}
	else
	{
		m_ExplosionVisualInfo.m_iDamage = GetDamage();
		m_ExplosionVisualInfo.m_iTeam = GetTeamNumber();

		if( ToTFPlayer( GetThrower() ) )
			m_ExplosionVisualInfo.m_vecPlayerColor = ToTFPlayer( GetThrower() )->GetPlayerColor();

		TFExplosion_Visuals_t visuals = GetExplosionVisualInfo();
		SetBombletExplosionVisualInfo( &visuals );
...
void CTFWeaponBaseGrenadeProj::Explode( trace_t *pTrace, int bitsDamageType, int bitsCustomDamageType )
{
	...
	if ( UseImpactNormal() )
	{
		...
		{
			TE_TFExplosion( filter, 0.0f, vecOrigin, GetImpactNormal(), pTrace->m_pEnt->entindex(), GetExplosionVisualInfo() );
		}
		else
		{
			TE_TFExplosion( filter, 0.0f, vecOrigin, GetImpactNormal(), -1, GetExplosionVisualInfo() );
		}
	}
	else
	{
		if ( pTrace->m_pEnt && pTrace->m_pEnt->IsPlayer() )
		{
			TE_TFExplosion( filter, 0.0f, vecOrigin, pTrace->plane.normal, pTrace->m_pEnt->entindex(), GetExplosionVisualInfo() );
		}
		else
		{
			TE_TFExplosion( filter, 0.0f, vecOrigin, pTrace->plane.normal, -1, GetExplosionVisualInfo() );
		}
	}
	...
	if( m_pBombletInfo )
	{
		TFExplosion_Visuals_t visuals = GetBombletExplosionVisualInfo();
		SpawnBomblets( m_pBombletInfo, &visuals, pTrace, GetTeamNumber() );
	}
#endif
...
void CTFWeaponBaseGrenadeProj::Deflected( CBaseEntity *pDeflectedBy, Vector &vecDir )
{
	...
	CTFPlayer* pReflect = static_cast<CTFPlayer*>(pDeflectedBy);
	CBaseCombatCharacter* pReflecterCombat = static_cast<CBaseCombatCharacter*>(pReflect);

	SetOwnerEntity(pReflect);

	if (pReflecterCombat)
	{
		SetThrower(pReflecterCombat);
	}
...
#ifdef OF_DLL
void CTFWeaponBaseGrenadeProj::SetExplosionVisualInfo(TFExplosion_Visuals_t *pInfo )
{
	m_ExplosionVisualInfo = *pInfo;
	m_ExplosionVisualInfo.m_iTeam = GetTeamNumber();
}

void CTFWeaponBaseGrenadeProj::SetBombletExplosionVisualInfo(TFExplosion_Visuals_t* pInfo)
{
	m_BombletExplosionVisualInfo = *pInfo;
	m_BombletExplosionVisualInfo.m_iTeam = GetTeamNumber();
}
