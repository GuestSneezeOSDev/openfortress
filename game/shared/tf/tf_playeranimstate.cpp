	...
	case PLAYERANIMEVENT_ATTACK_PRIMARY:
		{
			...
			bool bIsSniperRifle = pWpn && (pWpn->GetWeaponID() == TF_WEAPON_SNIPERRIFLE || pWpn->GetWeaponID() == TF_WEAPON_RAILGUN || pWpn->GetWeaponID() == TFC_WEAPON_SNIPER_RIFLE);
...
bool CTFPlayerAnimState::HandleSwimming( Activity &idealActivity )
{
	...
	if ( bInWater )
	{
		if (m_pTFPlayer->m_Shared.InCond(TF_COND_AIMING) || m_pTFPlayer->m_Shared.InCond(TF_COND_AIMING_SCOPE_ONLY))
		{
			...
			else if (pWpn && (pWpn->GetWeaponID() == TF_WEAPON_SNIPERRIFLE || pWpn->GetWeaponID() == TF_WEAPON_RAILGUN || pWpn->GetWeaponID() == TFC_WEAPON_SNIPER_RIFLE))
			{
...
bool CTFPlayerAnimState::HandleDucking( Activity &idealActivity )
{
	if ( GetBasePlayer()->GetFlags() & FL_DUCKING )
	{
		...
		if ( m_pTFPlayer->m_Shared.IsLoser() || GetOuterXYSpeed() < MOVING_MINIMUM_SPEED )
		{
			...
			if ((m_pTFPlayer->m_Shared.InCond(TF_COND_AIMING) || (m_pTFPlayer->m_Shared.InCond(TF_COND_AIMING_SCOPE_ONLY))) || m_flHoldDeployedPoseUntilTime > gpGlobals->curtime)
			{
