...
bool IsPlayerClassName( char const *str )
{
	...
	{
		TFPlayerClassData_t *data = GetPlayerClassData( i, 0 );
...
const char *g_aGameTypePrefixes[] =
{
	"Undefined",
	"CTF",			
	"CP",			
...
const Vector g_vecFixedPatternTFC[] =
{
	Vector(0, 0, 0),			//1
...
const char *g_aWeaponNames[] =
{
	...
	"TF_WEAPON_LEAD_PIPE",
	"TF_WEAPON_FLAMETHROWER_MERCENARY",
	"TF_WEAPON_RIPPER",
...
uint g_aWeaponDamageTypes[] =
{
	...
	DMG_CLUB,																					// TF_WEAPON_LEAD_PIPE
	DMG_IGNITE | DMG_PREVENT_PHYSICS_FORCE | DMG_PREVENT_PHYSICS_FORCE,							// TF_WEAPON_FLAMETHROWER_MERCENARY,
	DMG_SLASH | DMG_PREVENT_PHYSICS_FORCE,														// TF_WEAPON_RIPPER
...
void LoadObjectInfos( IBaseFileSystem *pFileSystem )
{
	...
	for ( int iObj=0; iObj < ARRAYSIZE( g_ObjectInfos ); iObj++ )
	{
		...
		pInfo->m_DMSelectionSlot = pSub->GetInt( "DM_SelectionSlot", pInfo->m_SelectionSlot );
		pInfo->m_DMSelectionPosition = pSub->GetInt( "DM_SelectionPosition", pInfo->m_SelectionPosition );
