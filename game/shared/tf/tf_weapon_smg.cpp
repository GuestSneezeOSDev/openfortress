...
acttable_t CTFTommyGun::m_acttableTommyGun_Mercenary[] =
{
...
acttable_t *CTFTommyGun::ActivityList(int &iActivityCount)
{
	if( !GetTFPlayerOwner() )
		return BaseClass::ActivityList(iActivityCount);
	...
	if (GetTFPlayerOwner()->GetPlayerClass()->GetClassIndex() == TF_CLASS_MERCENARY)
	{
		iActivityCount = ARRAYSIZE(m_acttableTommyGun_Mercenary);
		return m_acttableTommyGun_Mercenary;
	}
	else
	{
		return BaseClass::ActivityList(iActivityCount);
