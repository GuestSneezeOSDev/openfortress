...
public:
	...
	// Voice commands - shared
	int ActiveVoiceCommandSetId() const override;

#if defined( OF_DLL ) || defined ( OF_CLIENT_DLL )
	// Get which slot a certain weapon takes up for a certain player
	void GetWeaponSlot( char *szName, int &iSlot, int &iPos, CTFPlayer *pPlayer = NULL );
	// Get which slot a certain weapon takes up for a certain player
	void GetWeaponSlot( WEAPON_FILE_INFO_HANDLE pHandle, int &iSlot, int &iPos, CTFPlayer *pPlayer = NULL );
	ETFInventoryType GetPlayerInventorySystem( CTFPlayer *pPlayer );

	int GetPlayerClassMod( CTFPlayer *pPlayer );
	int GetTeamClassMod( int iTeam );

	// Override on the SERVER side, new function on the CLIENT
	// This is because player relationship was only ever used on the Server for some reason
	// While on the client it was checked with various weird other functions
	virtual int PlayerRelationship( CBaseEntity* pPlayer, CBaseEntity* pTarget, bool bCheckDisguise = false );
	virtual bool CanEntityHitEntity( CBaseEntity* pPlayer, CBaseEntity* pTarget );
#endif
...
public:
	...
	virtual bool 	ShouldBalanceTeams( bool bBots = false )
	{
		if( InGametype( TF_GAMETYPE_INF ) )
			return false;

		return BaseClass::ShouldBalanceTeams( bBots );
	}
	...
	virtual void	PrecacheClass( int iClass );
...
public:
	...
	virtual void CreateInfectionRoundTimer();
	...
	virtual void CalcFFALead( CTFPlayer *pTFPlayerScorer );
	virtual void CalcTDMLead( void );
...
#ifdef GAME_DLL
	void		OnTeamLostPlayer( int iTeamNum, CBasePlayer *pPlayer );
#endif
...
protected:
	bool PlayVoiceCommand(CBaseMultiplayerPlayer* pPlayer, const SVoiceCommand& voiceCommand, bool bIsCalledFromBind) const override;
...
public:
	...
	bool m_bHasRedSpawns;
	bool m_bHasBlueSpawns;
#endif
	...
	bool	DoesArenaCountLivesAndFrags(void);
	...
	virtual int GetGameType() { return m_nGameType; }
	...
	bool	IsClassSelectAllowed(int iTeamNum);
	int		GetForcedClassIndex(int iTeamNum);
	int		GetClassLimit(int iTeamNum, int iClassIndex);
#ifdef GAME_DLL
	bool	IsClassAllowed( int iTeamNum, int iClassIndex );
#endif
	...
	bool HasRedSpawns(void) { return m_bHasRedSpawns; }
	bool HasBlueSpawns(void) { return m_bHasBlueSpawns; }
#endif
	
	CNetworkVar( bool, m_bUsesHL2Hull );
	CNetworkVar( bool, m_bForce3DSkybox );
	CNetworkVar( bool, m_bUsesMoney );
	CNetworkVar( bool, m_bArmorOnSpawn);
