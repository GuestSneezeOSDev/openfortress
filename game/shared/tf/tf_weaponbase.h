...
class CTFWeaponBase : public CBaseCombatWeapon
{
...
#if defined(OF_DLL)
	virtual TFExplosion_Visuals_t GetExplosionVisualInfo();
	virtual TFExplosion_Visuals_t GetBombletExplosionVisualInfo();
#endif
	...
	int GetProjectileType( void ) const;
	...
	virtual int  GetSlotOverride( void ) const { return m_iSlotOverride; };
	virtual int  GetPositionOverride( void ) const { return m_iPositionOverride; };
	...
	virtual float	GetBurstTotalTime(void) { 
	...
	virtual float GetProjectileSpeed( void ){ return 0; };
...
protected:
	...
	CUtlVector<HPARTICLEFFECT> m_hAnimParticles;
#endif
protected:
...
#ifdef OF_DLL
extern CTFWeaponBase *CreateWeaponNoGive( const char *pszName, int iSubType = 0 );
#endif
