...
bool g_bUsedDMWeaponSlots[MAX_WEAPON_SLOTS][MAX_WEAPON_POSITIONS] = { { false } };
#endif
...
//	memset(g_bUsedDMWeaponSlots, 0, sizeof(g_bUsedDMWeaponSlots));
//#endif
//
//#ifndef GAME_DLL
...
#if defined( OF_DLL ) || defined ( OF_CLIENT_DLL )
bool ReadWeaponDataFromKeyValuesForSlot( KeyValues *pKV, const char *szWeaponName, WEAPON_FILE_INFO_HANDLE *phandle, bool bReParse )
{
	if ( !phandle )
	{
		Assert( 0 );
...
void FileWeaponInfo_t::Parse( KeyValues *pKeyValuesData, const char *szWeaponName )
{
...
#if defined( OF_DLL ) || defined ( OF_CLIENT_DLL )
	int iOldSlot = iSlot;
	int iOldPosition = iPosition;
	int iOldSlotDM = iSlotDM;
	int iOldPositionDM = iPositionDM;
#endif
	...
	if ( iSlotDM > MAX_WEAPON_SLOTS ||
		iPositionDM > MAX_WEAPON_POSITIONS )
	{
		Error( "Invalid DM weapon slot or position [slot %d/%d max], pos[%d/%d max] in weapon %s!!!\n Grab a programmer and fix this or lower the slot/pos!\n",
			iSlotDM, MAX_WEAPON_SLOTS, iPositionDM, MAX_WEAPON_POSITIONS, szWeaponName );
	}
	else
	{
		if( g_bUsedDMWeaponSlots[iSlotDM][iPositionDM] && iSlotDM != iOldSlotDM && iPositionDM != iOldPositionDM )
		{
			DevMsg( "Duplicately assigned DM weapon slots in selection hud:  %s (%s, %d, %d)\n", szClassName, szPrintName, iSlotDM, iPositionDM );
		}
		g_bUsedDMWeaponSlots[iSlotDM][iPositionDM] = true;
	}

	if ( iSlot > MAX_WEAPON_SLOTS ||
		iPosition > MAX_WEAPON_POSITIONS )
	{
		Error( "Invalid weapon slot or position [slot %d/%d max], pos[%d/%d max] in weapon %s!!!\n Grab a programmer and fix this or lower the slot/pos!\n",
			iSlot, MAX_WEAPON_SLOTS, iPosition, MAX_WEAPON_POSITIONS, szWeaponName );
	}
	else
	{
		if( g_bUsedWeaponSlots[iSlot][iPosition] && iSlot != iOldSlot && iPosition != iOldPosition )
		{
			DevMsg( "Duplicately assigned weapon slots in selection hud:  %s (%s, %d, %d)\n", szClassName, szPrintName, iSlot, iPosition );
		}
	...
	gWR.LoadWeaponSprites( FindWeaponInfoSlot(szWeaponName), pKeyValuesData );
#endif
}
