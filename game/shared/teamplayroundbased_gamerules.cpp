...
#if defined( OF_DLL ) || defined( OF_CLIENT_DLL )
ConVar tf_bot_quota_mode("tf_bot_quota_mode", "normal", 
	...
	If 'balance', bots will maintain a 1:1 ratio between teams, letting human players join either without affecting balance.\n");

ConVar tf_bot_auto_balance( "tf_bot_auto_balance", "1", 
	FCVAR_NOTIFY | FCVAR_REPLICATED
#ifdef OF_CLIENT_DLL
	| FCVAR_HIDDEN
#endif
	, "If nonzero, bots will not be counted when balancing teams, and will be auto switched if a player joins the team with higher players." );
#endif
...
float CTeamplayRoundBasedRules::GetMinTimeWhenPlayerMaySpawn( CBasePlayer *pPlayer )
{
...
#if defined (OF_DLL) || defined (OF_CLIENT_DLL)
	if( TFGameRules()->IsDMGamemode() && !(TFGameRules()->IsArenaGamemode()) )
	{
		if( pTFPlayer->IsPressingRespawnButton() )
			flDeathAnimLength = 0.0f;
		else if( pTFPlayer->ShouldWaitForRespawn() )
	...
	{
		if (pTFPlayer->m_Shared.GetTFLives() > 0)
		{
			//DevMsg("Lives Above 0\n");
			// This won't even work I bet
...
#endif
				pTFPlayer->ShouldWaitForRespawn()
				)
				return gpGlobals->curtime;
		}
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
SERVERONLY_DLL_EXPORT void CTeamplayRoundBasedRules::SetInWaitingForPlayers( bool bWaitingForPlayers )
{
	...
	// Don't even TRY to get into waiting for players if we got it disabled
	// Though still keep going out of it enabled
	if( mp_disable_waitingforplayers.GetBool() && m_bInWaitingForPlayers == false )
		return;
...
void CTeamplayRoundBasedRules::CheckWaitingForPlayers( void )
{
	...
	bool bRestartWaitingForPlayers = mp_waitingforplayers_restart.GetBool();
	...
	if( TFGameRules()->IsInfGamemode() )
	{
		...
		bRestartWaitingForPlayers = bRestartWaitingForPlayers || targets < 2;
	}

	if( bRestartWaitingForPlayers )
	{
		if( m_bInWaitingForPlayers )
		{
			m_flWaitingForPlayersTimeEnds = gpGlobals->curtime + mp_waitingforplayers_time.GetFloat();
	...
	bool bCancelWait = ( mp_disable_waitingforplayers.GetBool() || mp_waitingforplayers_cancel.GetBool() || IsInItemTestingMode() ) && !IsInTournamentMode();

	if ( mp_developer.GetBool() )
		bCancelWait = true;

	if ( bCancelWait && !bRestartWaitingForPlayers )
	{
	...
	if( m_bInWaitingForPlayers )
	{
		...
		// Properly resets waiting for players to time set -Bryson
		// And bots will not affect the timer... - Brysondev 2023/02/09
		// UNDONE: Breaks testing with bots
		// use tf_bot_join_after_player if you don't want the game to start without a player
		if (TFGameRules()->IsDMGamemode() && (CountActivePlayers() < 1))
		{
			SetInWaitingForPlayers(false);
			State_Transition( GR_STATE_PREGAME );
			return;
...
void CTeamplayRoundBasedRules::State_Think_PREGAME( void )
{
	...
	if ( IsInCommentaryMode() )
		return;
#ifdef OF_DLL
	int iActivePlayers = CountRealPlayers();
...
void CTeamplayRoundBasedRules::State_Enter_PREROUND( void )
{
	...
	m_bInfectionLastManAlive = false;
	if( DMMusicManager() )
		DMMusicManager()->SetDisabled( false );
#endif
	...
	if ( IsInArenaMode() == true )
	{
		...
		if ( bEnoughPlayers )
#else
		if ( CountActivePlayers() > 0 && CountRealPlayers() > 0 )
#endif
		{
#ifndef CSTRIKE_DLL
			variant_t sVariant;
...
void CTeamplayRoundBasedRules::State_Think_RND_RUNNING( void )
{
...
#endif
	{
		...
		if ( TFGameRules()->IsDMGamemode() )
		{
			...
			//Fixes problem where map will end while "waitingforplayers" in DM -Bryson
			else if ( IsInWaitingForPlayers() || CountRealPlayers() <= 0 )
			{
				ResetMapTime(); 
			}
			else
...
void CTeamplayRoundBasedRules::State_Think_TEAM_WIN( void )
{
	...
	if( TFGameRules() && TFGameRules()->IsDMGamemode() && !TFGameRules()->DontCountKills()
		&& gpGlobals->curtime >= (m_flStateTransitionTime - ( GetBonusRoundTime() * ( 1 - of_winscreenratio.GetFloat() ) )) )
	{
			...
			else
			{
				...
				if( TFGameRules()->IsESCGamemode() )
				{
					for ( int iTeam = LAST_SHARED_TEAM+1; iTeam < GetNumberOfTeams(); iTeam++ )
					{
						...
						for( int player = 0; player < pTeam->GetNumPlayers(); player++ )
						{
							...
							if( pCiv && pCiv->IsConnected() )
							{
								...
								{
									if( !pCiv->IsClassAllowed( TF_CLASS_CIVILIAN ) )
									{
...
{
	if( TFGameRules()->IsInfGamemode() && !TFGameRules()->GetInfectionRoundTimer() )
	{
		TFGameRules()->CreateInfectionRoundTimer();
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
int CTeamplayRoundBasedRules::CountRealPlayers( void )
{
	int i;
	int count = 0;
	CBasePlayer *pPlayer;
	...
	for (i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		// 
		if ( pPlayer && (!pPlayer->IsFakeClient() || !pPlayer->IsBot()))
		{
			if( pPlayer->IsReadyToPlay() )
			{
				count++;
...
{
	// Stop infection LMS music
	if( TFGameRules()->IsInfGamemode() )
	{
		IGameEvent * event = gameeventmanager->CreateEvent( "stop_fmod_sound" );
		if ( event )
		{
			event->SetString( "soundscript", "InfectionMusic.LastMercStanding" );
...
void CTeamplayRoundBasedRules::PlayStalemateSong( void )
{
	...
	for ( int i = FIRST_GAME_TEAM; i < GetNumberOfTeams(); i++ )
	{
		BroadcastSound( i, GetStalemateSong( i ), false );
	}
}
...
#if defined( OF_DLL ) || defined ( OF_CLIENT_DLL )
	// bruh
	if ( TFGameRules()->IsArenaGamemode() && (of_lives.GetInt() == 0))
		return 99999;
#endif
...
//-----------------------------------------------------------------------------
bool CTeamplayRoundBasedRules::ShouldBalanceTeams( bool bBots )
{
	if ( IsInTournamentMode() == true )
		return false;
	...
	if ( !bBots && mp_teams_unbalance_limit.GetInt() <= 0 )
		return false;
...
bool CTeamplayRoundBasedRules::WouldChangeUnbalanceTeams( int iNewTeam, int iCurrentTeam, bool bIsBot )
#else
...
#endif
{
	...
	if( !bIsBot && UTIL_StringFieldToInt(tf_bot_quota_mode.GetString(), g_QuotaModes, TF_BOT_QUOTA_COUNT) == TF_BOT_QUOTA_BALANCE )
		return false;

#if defined( OF_CLIENT_DLL ) || defined( OF_DLL )
	CTFTeam* pNewTeam = GetGlobalTFTeam(iNewTeam);
#else
	...
	int iNewTeamPlayers = pNewTeam->GetNumPlayers() - pNewTeam->GetNumLockdownBots() + 1;
	...
	for ( pTeam = GetGlobalTFTeam(i); pTeam != NULL; pTeam = GetGlobalTFTeam(++i) )
#else
	CTeam* pTeam;
...
#endif
	{
		...
		int iNumPlayers = pTeam->GetNumPlayers() - pTeam->GetNumLockdownBots();
...
#if defined( OF_DLL ) || defined ( OF_CLIENT_DLL )
		int iUnbalanceLimit = mp_teams_unbalance_limit.GetInt();

		// All the bots the new team has can be used to balance out things
		if( IgnoreBotsWhenBalancing() )
			iUnbalanceLimit += pNewTeam->GetNumQuotaBots();

		if ( ( iNewTeamPlayers - iNumPlayers ) > iUnbalanceLimit )
#else
...
//-----------------------------------------------------------------------------
#if defined ( OF_DLL ) || defined ( OF_CLIENT_DLL )
bool CTeamplayRoundBasedRules::AreTeamsUnbalanced( int &iHeaviestTeam, int &iLightestTeam, bool bForceCountBots )
#else
...
#endif
{
	...
	if( !bForceCountBots && UTIL_StringFieldToInt(tf_bot_quota_mode.GetString(), g_QuotaModes, TF_BOT_QUOTA_COUNT) == TF_BOT_QUOTA_BALANCE )
		return false;
...
#if defined ( OF_DLL ) || defined ( OF_CLIENT_DLL )
	for ( CTFTeam *pTeam = GetGlobalTFTeam(i); pTeam != GetGlobalTFTeam(TF_TEAM_MERCENARY) ; pTeam = GetGlobalTFTeam(++i) )
#else
...
#if defined( OF_CLIENT_DLL ) || defined( OF_DLL )
const char *g_QuotaModes[TF_BOT_QUOTA_COUNT] =
{
	"normal",
	"fill",
	"match",
...
bool CTeamplayRoundBasedRules::IgnoreBotsWhenBalancing()
{
	if( tf_bot_auto_balance.GetBool() )
		return true;

	return UTIL_StringFieldToInt(tf_bot_quota_mode.GetString(), g_QuotaModes, TF_BOT_QUOTA_COUNT) == TF_BOT_QUOTA_BALANCE;
}
#endif
