...
bool CTraceFilterSimple::ShouldHitEntity( IHandleEntity *pHandleEntity, int contentsMask )
{
	...
	CBaseEntity *pPassEntity = NULL;
#endif
	if ( m_pPassEnt )
	{
#if defined( OF_DLL ) || defined ( OF_CLIENT_DLL )
		pPassEntity = const_cast<CBaseEntity*>( EntityFromEntityHandle(m_pPassEnt) );
#endif
	...
	if ( !pEntity )
		return false;
#if defined( OF_DLL ) || defined ( OF_CLIENT_DLL )
	if ( !pEntity->ShouldCollide( m_collisionGroup, contentsMask, pPassEntity ) )
#else
...
const char* ReadAndAllocStringName( KeyValues *pSub, const char *pFilename )
{
	const char *pValue = pSub->GetName();
	if ( !pValue )
	{
		if ( pFilename )
		{
			DevWarning( "Can't get key value name from file '%s'.\n", pFilename );
		}
		return NULL;
	}
...
#if defined( OF_DLL ) || defined ( OF_CLIENT_DLL )
const char* ReadAndAllocStringValue( KeyValues *pSub, const char *pName, const char *pFilename, const char *pDefault, bool bReturnNull )
#else
const char* ReadAndAllocStringValue( KeyValues *pSub, const char *pName, const char *pFilename )
#endif
{
#if defined( OF_DLL ) || defined ( OF_CLIENT_DLL )
	const char *pValue = pSub->GetString( pName, pDefault );
#else
