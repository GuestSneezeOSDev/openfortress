...
enum TF_BOT_QUOTA_MODE
{
	TF_BOT_QUOTA_NORMAL,
	TF_BOT_QUOTA_FILL,
	TF_BOT_QUOTA_MATCH,
	TF_BOT_QUOTA_BALANCE,

	TF_BOT_QUOTA_COUNT
};

extern const char* g_QuotaModes[TF_BOT_QUOTA_COUNT];
...
public:
	...
	virtual int	 CountRealPlayers( void );
protected:
...
#if defined ( OF_DLL ) || defined ( OF_CLIENT_DLL )
	bool WouldChangeUnbalanceTeams( int iNewTeam, int iCurrentTeam, bool bIsBot = false );
	bool AreTeamsUnbalanced( int &iHeaviestTeam, int &iLightestTeam, bool bForceCountBots = false );
	bool IgnoreBotsWhenBalancing();
#else
