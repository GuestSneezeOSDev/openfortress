...
void CTeamRoundTimer::RoundTimerSetupThink( void )
{
	...
	if ( flTime <= 0.0f && m_bFireFinished )
	{
		...
		// Has to be after SetState so OnTeamLosePlayer doesn't trigger waiting for players
		if ( this == TFGameRules()->GetInfectionRoundTimer() )
		{
...
#ifdef OF_DLL
		if( this == TFGameRules()->GetInfectionRoundTimer() && !TeamplayRoundBasedRules()->IsInWaitingForPlayers() )
		{
...
void CTeamRoundTimer::SetBaseTime( int iTimerSeconds )
{
	m_nTimerInitialLength = iTimerSeconds;
}
...
// Purpose: To set the initial timer duration
//-----------------------------------------------------------------------------
void CTeamRoundTimer::SetSetupTimeRemaining( int iTimerSeconds )
{
	if ( iTimerSeconds >= 0 )
	{
		m_nSetupTimeLength = iTimerSeconds;
	}
...
void CTeamRoundTimer::SetDisabled( bool bDisabled )
{
	...
	if( m_bShowInHUD )
	{
		SetActiveTimer( m_bIsDisabled ? NULL : this );
	}
}
