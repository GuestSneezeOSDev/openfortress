...
CVoiceStatus::~CVoiceStatus()
{
	...
	if( pGameDir )
	{
		if(m_bBanMgrInitialized)
		{
			...
			m_SprayBanMgr.SaveState( pGameDir );
#endif
		}
	}
}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CVoiceStatus::IsPlayerSprayBlocked(int iPlayer)
{
	player_info_t pi;
	...
	return m_SprayBanMgr.GetPlayerBan( pi.guid );
}

void CVoiceStatus::SetPlayerSprayBlockedState( int iPlayer, bool blocked )
{
	player_info_t pi;
	if ( !engine->GetPlayerInfo( iPlayer, &pi ) )
		return;

	m_SprayBanMgr.SetPlayerBan(pi.guid, blocked);
	UpdateServerState(false);
}
#endif