...
#endif // defined( OF_DLL ) || defined ( OF_CLIENT_DLL )
...
// Purpose: 
// Input  :
// Output : Returns identifier to be used in voice_command_registry to select correct menu set, can be overriden by implementation
//-----------------------------------------------------------------------------
int CMultiplayRules::ActiveVoiceCommandSetId() const
{
	return -1;
}
...
#endif
	// Loading registry will cause blocking load from filesystem

#if defined( OF_DLL ) || defined ( OF_CLIENT_DLL )
	m_pVoiceCommandRegistry = new COFVoiceCommandRegistry();
#else
	m_pVoiceCommandRegistry = new CVoiceCommandRegistry("scripts/voicecommands.txt");
#endif // defined( OF_DLL ) || defined ( OF_CLIENT_DLL )

	m_pVoiceCommandRegistry->Initialize();
}

CMultiplayRules::~CMultiplayRules()
{
	delete m_pVoiceCommandRegistry;
}
...
//=============================================================================
	...
	*/
	...
	bool CMultiplayRules::ClientCommand( CBaseEntity *pEdict, const CCommand &args )
	{
		...
		if ( FStrEq( pcmd, "voicemenu" ) )
		{
			...
			if ( pMultiPlayerPlayer )
			{
				if( args.ArgC() < 3 )
				{
					VoiceCommand( pMultiPlayerPlayer, args[1] );
					return true;
				}
	...
	const SVoiceCommand* CMultiplayRules::VoiceCommand( CBaseMultiplayerPlayer* pPlayer, const char *szGlobalIdentifier ) const
	{
		// Selects voice command by it's global identifier from current menu set and plays it
		const CVoiceCommandMenuSet& menuSet = m_pVoiceCommandRegistry->GetMenu(ActiveVoiceCommandSetId());

		const auto* pItem = menuSet.GetItem(szGlobalIdentifier);
		if (!pItem)
		{
			return nullptr;
		}

		return PlayVoiceCommand(pPlayer, *pItem, true) ? pItem : nullptr;
	}

	const SVoiceCommand* CMultiplayRules::VoiceCommand(CBaseMultiplayerPlayer* pPlayer, unsigned int uMenu, unsigned int uItem) const
	{
		// Selects voice command by menu and item index from current menu set and plays it
		const CVoiceCommandMenuSet& menuSet = m_pVoiceCommandRegistry->GetMenu(ActiveVoiceCommandSetId());

		const auto* pItem = menuSet.GetItem(uMenu, uItem);
		if (!pItem)
		{
			return nullptr;
		}

		return PlayVoiceCommand(pPlayer, *pItem, false) ? pItem : nullptr;
	}

	bool CMultiplayRules::PlayVoiceCommand(CBaseMultiplayerPlayer* pPlayer, const SVoiceCommand& voiceCommand, bool bIsCalledFromBind) const
	{
		// Processes all necessary steps to actually play voice command
		if (!pPlayer || !pPlayer->CanSpeakVoiceCommand())
		{
			return false;
		}
		...
		if (!pPlayer->SpeakConceptIfAllowed(voiceCommand.m_ConceptId, NULL, szResponse, AI_Response::MAX_RESPONSE_NAME))
		{
		...
		if (voiceCommand.m_ShowsSubtitle)
		{
			CRecipientFilter filter;

			if (voiceCommand.m_IsSubtitleDistanceBased)
			{
			...
			const CVoiceCommandMenuSet& menuSet = m_pVoiceCommandRegistry->GetMenu(ActiveVoiceCommandSetId());
			const auto voiceCommandPos = menuSet.GetItemPosition(voiceCommand.m_GlobalIdentifier);

			// Define global identifiers for your voice commands if you hit this assert
			Assert(voiceCommandPos.IsValid());

			// Send a subtitle to anyone in the PAS
			UserMessageBegin(filter, "VoiceSubtitle");
			WRITE_BYTE(pPlayer->entindex());
			WRITE_BYTE(voiceCommandPos.m_MenuIndex);
			WRITE_BYTE(voiceCommandPos.m_ItemIndex);
			MessageEnd();
		}
		...
		{
			botVector[i]->OnActorEmoted(pPlayer, voiceCommand.m_ConceptId);
		}
#endif // NEXT_BOT
...
const char* CMultiplayRules::GetVoiceCommandSubtitle(unsigned int uMenu, unsigned int uItem) const
{
	const CVoiceCommandMenuSet& menuSet = m_pVoiceCommandRegistry->GetMenu(ActiveVoiceCommandSetId());
	const SVoiceCommand* command = menuSet.GetItem(uMenu, uItem);

	Assert(command);

	return command->m_Subtitle;
}

// Returns false if no such menu is declared or if it's an empty menu
bool CMultiplayRules::GetVoiceMenuLabels(unsigned int iMenu, KeyValues *pKV ) const
{
	const CVoiceCommandMenuSet& menuSet = m_pVoiceCommandRegistry->GetMenu(ActiveVoiceCommandSetId());

	const auto* menu = menuSet.GetMenu(iMenu);
	if (!menu)
	{
		return false;
	}

	// Should have returned a vector, but input KeyValues* get passed into GUI down the line too
	for (const auto& item : *menu)
	{
		KeyValues* labelSubkey = new KeyValues(item.m_MenuLabel);
		pKV->AddSubKey(labelSubkey);
	}
