...
{
	COFSchemaWeaponInfo *pWeapon = GetItemSchema()->GetWeapon( szName );
	if( pWeapon )
	{
		if( pWeapon->m_szWeaponClass )
		{
			UTIL_PrecacheOther( pWeapon->m_szWeaponClass );

			if( !Q_strncasecmp( pWeapon->m_szWeaponClass, szName, Q_strlen(szName) ) )
				return;

			CBaseCombatWeapon *pEntity = (CBaseCombatWeapon*)CreateEntityByName( pWeapon->m_szWeaponClass );
			if ( !pEntity )
			{
			...
			// Making this a dev-msg, sorry not sorry.
			//Warning("Failed to find weapon class, falling back to util precache other\n");
			DevWarning(1, "Failed to find weapon class, falling back to util precache other\n");
	}
...
{
	int iBaseReserve = GetWpnData().iMaxReserveAmmo;
	GetAttributeValue_Int("set reserve ammo", iBaseReserve);

	float flReserveMultiplier = 1;
	GetAttributeValue_Float("reserve ammo multiplier", flReserveMultiplier);

	return iBaseReserve * flReserveMultiplier;
}
//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
...
{
	int iBaseReserve = GetWpnData().iDefaultReserveAmmo;
	GetAttributeValue_Int("set reserve ammo", iBaseReserve);
...
#if defined ( OF_DLL ) || defined ( OF_CLIENT_DLL )
	int iBaseClip = GetWpnData().iMaxClip1;
	GetAttributeValue_Int("set clip size", iBaseClip);

	float flClipMultiplier = 1;
	GetAttributeValue_Float("clip size multiplier", flClipMultiplier);

	return iBaseClip * flClipMultiplier;
#else
...
{
#if defined( OF_DLL ) || defined( OF_CLIENT_DLL )
	return const_cast<CBaseCombatWeapon*>(this)->GetSchemaName();
#else
...
int CBaseCombatWeapon::UpdateClientData( CBasePlayer *pPlayer )
{
	...
	// undone: Everything is parsed in the item schema so no need for this!
	// undone undone: We still need to do this, not to parse the weapon
	// but rather, to assign the correct handle
	// Since the schema name doesn't get transfered untill this
	// Don't worry, parsing doesn't happen twice!
	if( IsSchemaItem() && !m_bSetHandle )
	{
		ParseWeaponScript(false);
		m_bSetHandle = true;
	}
