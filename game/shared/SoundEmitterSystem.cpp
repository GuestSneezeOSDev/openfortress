...
public:
	...
	virtual void LevelInitPreEntity()
	{
		...
		if( V_stristr( mapname, "mvm" ) || V_stristr( mapname, "ld" )) // ld for lethal lockdown
		{
...
#if defined( OF_DLL ) || defined( OF_CLIENT_DLL )
		// Since there's no interface for adding sound files permanently post launch
		// we use this instead
		if( OFSoundManifest() )
			OFSoundManifest()->ReAddPermanentSoundFiles();
#endif
...
public:
	...
	void EmitSoundByHandle( IRecipientFilter& filter, int entindex, const EmitSound_t & ep, HSOUNDSCRIPTHANDLE& handle )
	{
		...
		enginesound->EmitSound( 
			...
			if( ep.m_pSoundFile )
			{
				Q_strcpy( (char*)ep.m_pSoundFile, params.soundname );
			}
#else
		enginesound->EmitSound( 
			filter, 
