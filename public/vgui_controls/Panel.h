...
#define AnimHandleHack(x) g_AnimHandles[g_AnimHandles.Find(x->ToHandle())]
...
void SetupAnimHandlingHackForPanel( vgui::Panel *pPanel );

class IAnimHandlingHack
{
public:
	IAnimHandlingHack();

	void SetAnimPos(int x, int y);	// sets animation offset of panel, in local space (ie. relative to parent's position)
	void GetAnimPos(int &x, int &y);	// gets animation offset of panel

	void RefreshPos(); // Applies the current Default + Anim Pos

	void SetAnimSize(int wide, int tall);	// sets animation size of panel
	void GetAnimSize(int &wide, int &tall);	// gets animation size of panel

	void RefreshSize(); // Applies the current Default + Anim Size
private:
	int m_iDefaultXPos;
	int m_iDefaultYPos;
	int m_iAnimXPos;
	...
	friend class Panel;
	friend void vgui::SetupAnimHandlingHackForPanel( vgui::Panel *pPanel );
};

} // namespace vgui

extern CUtlMap<vgui::HPanel, vgui::IAnimHandlingHack> g_AnimHandles;
